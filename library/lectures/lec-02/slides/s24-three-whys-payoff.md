...
## Body
[Gold-маркер сверху]
**Payoff Лекции 1 §5.3 — LO7**

[3 Ocean rounded boxes вертикально или 3-column grid]

**(1) Почему промпт с ролью работает лучше пустого?**
→ На уровне attention **role-токены получают высокий вес**, и модель опирается на них при выборе следующих токенов. См. s15.

**(2) Почему AI плохо считает буквы?**
→ Токенизатор объединяет несколько букв в один токен; модель не видит букв, видит токены. Слово `strawberry` — 3 токена, не 10 букв. См. s05–s07.

**(3) Почему один и тот же запрос даёт разные ответы?**
→ Сэмплинг — стохастический выбор из distribution при T > 0. Каждый запуск может выбрать разный токен. См. s18–s19.

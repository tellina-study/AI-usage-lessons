...
visual_brief: "2 Ocean rounded boxes side-by-side: «Человек» — «X произошло, потому что Y» = модель механизмов мира (уровни 1–3); «Модель (через внимание)» — «X следует за Y в текстах» = частотный паттерн (уровень 1, частично 2, не 3). Внизу — иллюстративная полоса «разбор инцидента»: плашка «связный нарратив ≠ каузальный вывод» с gold-акцентом. Gold callout про человека в контуре."
---
...
[Полоса-пример]
**Разбор инцидента:** модель строит связный нарратив «после деплоя X начались таймауты Y» — он **читается** как каузальный вывод, а является статистически правдоподобным рассказом. Проверка — откат, эксперимент — за инженером.

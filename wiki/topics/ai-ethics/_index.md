---
title: "AI Ethics and Regulation"
type: topic-index
sources_count: 12
sub_topics: [bias-fairness, regulatory-frameworks, data-privacy, responsible-ai]
lectures: [7]
coverage: medium
updated_at: 2026-04-07
---

# AI Ethics and Regulation

## Overview
Bias detection, fairness metrics, regulatory frameworks (EU AI Act, NIST AI RMF), data privacy, responsible AI principles. Primary lecture: 7.

## Sub-topics

### Data Privacy and Security
- Primary: [security-local-cloud.md](../../../notes/research/lecture-1/security-local-cloud.md)
- OpenAI/Anthropic/Google data policies, Samsung incident, open-source model risks
- NIST AI Risk Management Framework, EU AI Act Article 10

### Responsible AI
- LO3: Analyze ethical risks
- LO8: Formulate responsible AI principles
- Assessment: Sem 5, 10, 17 (via LO3)

## Lectures
- [Lecture 7](../../lectures/lec-07.md) *(planned)* -- AI Ethics and Regulation (primary)
- Lectures 3, 4, 6, 8 -- ethical considerations in industry context (via LO3)

## Normative Requirements
- PKS-4 -> LO3, LO7, LO8

## Related Topics
- [AI Fundamentals](../ai-fundamentals/_index.md) -- ethical dimensions of AI classification
- [AI Agents](../ai-agents/_index.md) -- autonomy levels raise ethical questions

## Backlinks
- [wiki/index.md](../../index.md) -- topic listing

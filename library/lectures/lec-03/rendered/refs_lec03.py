...
SLIDE_REFS = {
    ...
    "s05a": [
        ("1", "Zheng et al. 2024, Findings of EMNLP — персоны не улучшают точность фактов", "zheng_personas",
...
# ============================================================
ANCHORS = {
    "s01": [("1,2", "Трибунал: «бот — не отдельное юр. лицо» → компания компенсирует разницу")],
    "s05": [("1", "Не усложняй архитектуру без причины, выраженной в требованиях задачи")],
    "s05a": [("1", "Персоны НЕ улучшили точность ответа по сравнению с ответом без персоны вообще")],
    "s06": [("1", "23 − 7 = 16"),
            ("2", "доля случаев, где модель упомянула реально использованную подсказку")],
    ...
    "s13b": [("1", "Fine-tuning = «изменить саму модель»")],
    "s14": [("1", "Fine-tuning не умер"),
            ("2,3", "Дистилляция — отдельная техника, часто применяемая в связке с дообучением")],
    ...
    "s22": [("1", "Предсказуемая задача → workflow"),
            ("2", "ценность оправдывает кратные стоимость/риск → агент")],
    "s22b": [("1", "Пять типовых слотов")],
    "s22c": [("1", "Спектр — от плоского файла до graph-баз")],
    "s22d": [("1", "Независимый live-eval показывает: иногда — драматически нет")],
    ...
    "s23": [("1", "$4 200 за 63 часа"),
            ("2", "«меньше хопов + валидация между шагами» — сильный"),
    ...
    "s25": [("1", "ассистент выгрузил приватные репозитории в публичный PR"),
            ...
            ("4,5", "судебный приказ (NYT v. OpenAI) + third-party/MCP вне ZDR")],
...
# ============================================================
NOTES_ANCHORS = {
    "s01": [("1,2", "отвечающее за свои действия».")],

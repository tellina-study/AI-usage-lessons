...
def build_s04(p):
    ...
    header(s, "Раздел 0 · Сквозной кейс",
           "Репозиторий буквально пуст, а агент не задал ни одного уточняющего вопроса — и то, "
           "чего не знал, решил за вас молча",
           title_size=16)
    half_w = 5.85
    y1 = 1.85
    h1 = 1.55
...
def build_s05(p):
    ...
    header(s, "Раздел 0 · Первый вопрос классу",
           "Первый блок конфигурации выбирают из восьми вариантов на экране — и «ничего» среди "
           "них не менее весомый вариант, чем остальные семь",
           title_size=15)
...
def build_s10(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 1 · Сценарий-боль",
           "«Готово» для агента — это «собралось и тест зелёный»; «готово» для вас — «заявка "
           "доставлена на проде», и эти два определения разошлись молча",
           title_size=15)
    y = 1.85
    ocean_box(s, 0.55, y, 12.23, 1.05)
    text_box(s, 0.81, y, 11.7, 1.05,
             text="Новая сессия, неделю спустя. Заказчик: «отправил заявку с сайта — она не "
                  "пришла». Агент читает код, правит обработчик отправки формы в src/main.js, "
                  "запускает тест — тест зелёный — и отчитывается: «Готово, тест проходит».",
             size=12.3, color=DEEP, line_spacing=1.24, anchor=MSO_ANCHOR.MIDDLE)
    y += 1.05 + 0.12
    dashed_box(s, 0.55, y, 12.23, 0.46, fill=GOLD_TINT, stroke=GOLD_DARK)
    text_box(s, 0.81, y, 11.7, 0.46,
             text="а tests/form.spec.ts проверяет ровно одно — что форма НЕ отправляется с "
                  "пустыми обязательными полями",
             size=11.2, italic=True, color=GOLD_DARK, anchor=MSO_ANCHOR.MIDDLE)
    y += 0.46 + 0.12
    gold_callout(s, 0.55, y, 12.23, 1.2,
                 "«Агент починил форму, прогнал тест, тест зелёный, агент говорит ‘готово’. А "
                 "заявка на проде по-прежнему не доходит — потому что тест проверяет совсем "
                 "другое. Выберите, куда записать критерий ‘готово’ в этом проекте — из карточек "
                 "ниже — так, чтобы агент в следующей сессии не сделал ровно то же самое.»",
                 size=12.3)
    y += 1.2 + 0.16
    option_row(s, 0.55, y, 12.23, 0.85,
               ["ответить\nв чате", "README.md", "файл инструкций\nCLAUDE.md",
                "комментарий\nв тесте", "переписать\nсам тест"], size=12)
    y += 0.85 + 0.22
    text_box(s, 0.55, y, 12.23, 0.4, text="разбор — на следующем слайде", size=12,
...
def build_s11(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 1 · Разбор",
           "В файл инструкций идёт только невыводимое: критерий «готово», граница действий и "
           "способ проверки — и ничего, что агент допишет сам",
           title_size=15)
    y = 1.85
    text_box(s, 0.55, y, 12.23, 0.3,
             text="вопрос для контекста: «Куда записать критерий ‘готово’?»",
             size=11, italic=True, color=SLATE)
    y += 0.38
    headers = ["Вариант", "Где это работает", "Почему здесь ещё рано — и что вместо"]
    rows = [
        ["«Отвечу агенту в чате ещё раз»", "Внутри одной сессии — полностью достаточно",
         "Не переживает конец сессии: следующая начинается с пустого контекста. Вместо: "
         "записать туда, что читается само, без вашего участия"],
        ["«Напишу в README.md»", "README нужен людям, впервые открывшим репозиторий",
         "README не подгружается в начало каждой сессии. README читает человек, если захочет; "
         "файл инструкций читает агент, хочет он того или нет"],
        ["«Заведу файл инструкций с полным описанием архитектуры»",
         "Практически нигде: самый частый способ потратить контекст впустую",
         "Агент выведет структуру кода сам за секунды. Раздел устареет при первом рефакторинге. "
         "Контролируемый эксперимент: такие файлы подняли стоимость на 20–23% без прироста успеха"],
        ["«Допишу комментарий в tests/form.spec.ts»", "Когда правило касается ровно этого файла",
         "Агент откроет тест не в каждой задаче. Вместо: правило уровня проекта — в файл уровня проекта"],
        ["«Пусть агент перепишет тест так, чтобы проверял доставку»",
         "Правильный инженерный шаг — стоит сделать",
         "Не закрывает задачу раздела: даже идеальный тест не скажет, чего делать нельзя. Не "
         "«вместо», а вдобавок"],
        ["«Заведу файл инструкций, но только с тем, чего агент не выведет из кода»",
         "На этом шаге — именно то, что нужно",
         "Не рано: сигнал сработал. Перестаёт работать, когда файл начинает расти без обрезки"],
    ]
    reveal_table(s, 0.55, y, 12.23, 6.95 - y, headers, rows, [0.26, 0.26, 0.48],
                 row_highlight={5: "gold"}, header_size=9, cell_size=8.6)
...
def build_s17(p):
    build_stage_divider(p, "s17", stage_num=2, title="Ступень 2 · память",
...
def build_s18(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 2 · Сценарий-боль",
    ...
    text_box(s, 0.81, y, 11.7, 1.15,
             text="Третья сессия за две недели. Заказчик прислал заявку с опечаткой в домене "
                  "почты, и форма её приняла. Агент читает код и предлагает подключить стороннюю "
                  "библиотеку валидации форм — ту самую, которую уже обсуждали и отклонили: "
                  "хватает нативных required/pattern и двадцати строк собственного кода.",
             size=11.6, color=DEEP, line_spacing=1.22, anchor=MSO_ANCHOR.MIDDLE)
    y += 1.15 + 0.12
    dashed_box(s, 0.55, y, 12.23, 0.42, fill=GOLD_TINT, stroke=GOLD_DARK)
    text_box(s, 0.81, y, 11.7, 0.42,
             text="он предлагает её так, будто вопрос никогда не поднимался — для него он и не "
                  "поднимался",
             size=10.6, italic=True, color=GOLD_DARK, anchor=MSO_ANCHOR.MIDDLE)
    y += 0.42 + 0.12
    gold_callout(s, 0.55, y, 12.23, 1.15,
                 "«Вы второй раз за две недели объясняете агенту, почему в этой форме нет "
                 "сторонней библиотеки валидации — то самое решение, которое уже отклонили. "
                 "Выберите, куда записать это решение — из карточек ниже — так, чтобы не "
                 "объяснять его в третий раз, и обоснуйте, почему не в тот файл инструкций, "
                 "который мы только что завели.»", size=11.8)
    y += 1.15 + 0.16
    option_row(s, 0.55, y, 12.23, 0.85,
...
def build_s19(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 2 · Разбор",
           "Командное решение с обоснованием идёт в DECISIONS.md, личное предпочтение — в "
           "авто-память, выводимое из кода — никуда",
           title_size=16)
    y = 1.8
    text_box(s, 0.55, y, 12.23, 0.3,
             text="вопрос для контекста: «Куда записать отклонённое решение?»",
             size=11, italic=True, color=SLATE)
    y += 0.38
    headers = ["Вариант", "Где это работает", "Почему здесь ещё рано — и что вместо"]
    rows = [
        ["«Допишу в CLAUDE.md»", "Механически сработает: файл читается каждую сессию",
         "Смешивает два разных по природе слоя. На десятке решений файл выйдет за ориентир в "
         "200 строк. Вместо: развести слои по авторству"],
        ["«Ничего, агент сам запомнит»", "Нигде: это не настройка, которую забыли включить",
         "Модель не имеет состояния между вызовами по конструкции: без записи на диск ничего не "
         "переживает конец сессии"],
        ["«Скажу агенту ‘запомни это’»", "Для личного предпочтения — идеально",
         "Половина ответа. Уходит в авто-память: машинно-локальный слой без синхронизации и без "
         "ревью. Для командного решения этого мало"],
        ["«Заведу DECISIONS.md»", "Для командного решения с обоснованием — именно то, что нужно",
         "Вторая половина ответа. Требует дисциплины: писать «почему», а не «что»"],
        ["«Поставлю систему памяти (векторную/графовую)»",
         "На десятках-сотнях записей, при потребности в поиске по смыслу",
         "Рано на порядок: живые замеры показывают, что на малых корпусах простой файл "
         "регулярно конкурентен со сложными системами"],
    ]
    table_h = 3.85
    reveal_table(s, 0.55, y, 12.23, table_h, headers, rows, [0.26, 0.28, 0.46],
                 header_size=9.5, cell_size=9.3)
    y += table_h + 0.16
    gold_callout(s, 0.55, y, 12.23, 0.7,
                 "целевой ответ — оба механизма разом, с разделением ролей", size=14.5)
...
def build_s25(p):
    build_stage_divider(p, "s25", stage_num=3, title="Ступень 3 · хук",
...
def build_s26(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 3 · Сценарий-боль",
           "Правило было в файле инструкций — и агент всё равно его нарушил",
           title_size=20, title_h=1.05)
    y = 1.95
    ocean_box(s, 0.55, y, 12.23, 1.3)
    text_box(s, 0.81, y + 0.1, 11.7, 1.1,
             text="Завтра показ сайта заказчику. Накануне в файл инструкций добавлена ещё одна "
                  "строка — репозиторный этикет: «Перед показом правки идут через отдельную "
                  "ветку, прямых коммитов в main нет.» Вечером, в спешке, агент правит валидацию "
                  "формы и делает git commit прямо на main.",
             size=11.2, color=DEEP, line_spacing=1.2, anchor=MSO_ANCHOR.MIDDLE)
    y += 1.3 + 0.14
    gold_callout(s, 0.55, y, 12.23, 1.15,
                 "«Завтра показ. В файле инструкций написано: перед показом — только через "
                 "отдельную ветку, никаких прямых коммитов в main. Агент только что закоммитил "
                 "правку формы прямо в main. Текстом всё было сказано правильно. Выберите, как "
                 "сделать нарушение этого правила НЕВОЗМОЖНЫМ, а не просто маловероятным, — из "
                 "карточек ниже.»", size=11)
    y += 1.15 + 0.16
    option_row(s, 0.55, y, 12.23, 0.75,
               ["переписать\nправило капсом", "проверять руками\nпосле коммита",
                "защита ветки\nна сервере", "хук, который среда\nисполняет сама",
                "забрать у агента\nдоступ к git"], size=10.6)
    y += 0.75 + 0.18
    text_box(s, 0.55, y, 12.23, 0.4, text="разбор, доказательство и формула — на следующем слайде",
             size=12, italic=True, color=SLATE, align=PP_ALIGN.CENTER)
...
def build_s27(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 3 · Разбор",
           "Правила в промпте — это просьбы, правила в коде — это законы: хук делает нарушение "
           "невозможным, а не просто маловероятным",
           title_size=14.5, title_h=1.1)
    y = 1.9
    text_box(s, 0.55, y, 12.23, 0.28,
             text="вопрос для контекста: «Как сделать нарушение невозможным?»",
             size=10.5, italic=True, color=SLATE)
    y += 0.34
    headers = ["Вариант", "Где это работает", "Почему здесь ещё рано — и что вместо"]
    rows = [
        ["«Перепишу правило капсом, добавлю ‘ВАЖНО’»",
         "Нигде механически: эмфаза не создаёт барьера",
         "Было бы разумно, если бы агент не заметил инструкцию — но это не так "
         "(см. доказательство ниже)"],
        ["«Буду проверять руками после каждого коммита»",
         "Пока хватает дисциплины и коммитов немного",
         "Не масштабируется: вы приставили себя сторожем к инструменту, который взяли, чтобы "
         "не сторожить"],
        ["«Включу защиту ветки на сервере»", "Работает, и это правда нужный слой",
         "Не рано — но и не достаточно: обратной связи в момент действия нет. Не «вместо», а "
         "вдобавок"],
        ["«Заберу у агента доступ к git вообще»", "Когда вы готовы делать все коммиты руками",
         "Снимает риск вместе с пользой — это отказ от задачи, а не её решение"],
        ["«Поставлю хук, который блокирует коммит в main»", "Именно то, что нужно: исполняет среда",
         "Не рано: правило уже нарушено, сигнал сработал. Ломается тихо — при невалидном "
         "условии срабатывания или таймауте"],
    ]
    table_h = 2.75
    reveal_table(s, 0.55, y, 12.23, table_h, headers, rows, [0.26, 0.26, 0.48],
                 row_highlight={4: "gold"}, header_size=8.4, cell_size=8.2)
    y += table_h + 0.14
    dashed_box(s, 0.55, y, 12.23, 1.05, fill=GOLD_TINT, stroke=GOLD_DARK)
    text_box(s, 0.78, y + 0.08, 11.75, 0.9,
             text="«‘Перепишу капсом’ исходит из того, что агент правило не заметил. Вот "
                  "публично разобранный случай, где замечать было нечего: правило требовало "
                  "явного подтверждения перед доступом к любому пути вне рабочей директории. "
                  "Агент выполнил установочную команду с записью в системный каталог, "
                  "регистрацией служб и правкой реестра — без подтверждения "
                  "(anthropics/claude-code#42863, закрыт как ‘not planned’).»",
             size=9.8, italic=True, color=GOLD_DARK, line_spacing=1.16)
    y += 1.05 + 0.14
    gold_callout(s, 0.55, y, 12.23, 0.6,
                 "правила в промпте — это просьбы; правила в коде — это законы", size=13.5)
...
def build_s35(p):
    build_stage_divider(p, "s35", stage_num=4, title="Ступень 4 · скилл",
...
def build_s36(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 4 · Сценарий-боль",
           "Файл инструкций облагает налогом каждый запрос процедурой, которая нужна примерно "
           "в одной сессии из десяти",
           title_size=16)
    y = 1.85
    ocean_box(s, 0.55, y, 12.23, 1.1)
    text_box(s, 0.8, y + 0.1, 6.3, 0.9, text="14 строк → 54 строки", size=20, bold=True,
             color=DEEP, anchor=MSO_ANCHOR.MIDDLE)
    text_box(s, 7.0, y + 0.1, 5.5, 0.9,
             text="ориентир 200 · деплой ≈ 1 сессия из 10", size=13, italic=True, color=SLATE,
             anchor=MSO_ANCHOR.MIDDLE, line_spacing=1.3)
    y += 1.1 + 0.14
    numbered_card(s, 0.55, y, 12.23, 1.05, [
        "собрать → прогнать тест формы → опубликовать → открыть прод-URL и отправить заявку "
        "руками → записать в журнал, что задеплоено и когда"], size=12.5)
    y += 1.05 + 0.14
    gold_callout(s, 0.55, y, 12.23, 1.1,
                 "«В файле инструкций накопилась процедура деплоя на сорок строк. Она нужна раз "
                 "в две недели, а грузится в каждую сессию — даже когда вы правите отступы в "
                 "вёрстке. Выберите, куда её вынести — из карточек ниже — так, чтобы агент сам "
                 "понял, когда её открыть.»", size=11.8)
    y += 1.1 + 0.16
    option_row(s, 0.55, y, 12.23, 0.7,
               ["оставить\nкак есть", "отдельный файл\n+ импорт",
                "отдельный файл,\nнапоминать вручную",
                "скилл с\nописанием-триггером", "хук"], size=10.6)
    y += 0.7 + 0.16
    text_box(s, 0.55, y, 12.23, 0.4, text="разбор — на следующем слайде", size=12,
             italic=True, color=SLATE, align=PP_ALIGN.CENTER)
...
def build_s37(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 4 · Разбор",
           "Скилл — знание, которое агент сам подгружает по описанию-триггеру, состоящему из "
           "трёх частей: что делает, когда использовать, когда не использовать",
           title_size=14.5, title_h=1.1)
    y = 1.9
    text_box(s, 0.55, y, 12.23, 0.3,
             text="вопрос для контекста: «Куда вынести процедуру деплоя?»",
             size=11, italic=True, color=SLATE)
    y += 0.38
    headers = ["Вариант", "Где это работает", "Почему здесь ещё рано — и что вместо"]
    rows = [
        ["«Оставлю в файле инструкций — вдруг понадобится»",
         "Нигде: ровно тот антипаттерн, который ступень называет по имени",
         "Постоянный налог на контекст независимо от релевантности. Вместо: вынести туда, что "
         "грузится по требованию"],
        ["«Вынесу в отдельный файл и сошлюсь импортом»", "Организационно: файл станет читаемее",
         "Импорт не экономит контекст — импортированный файл всё равно грузится целиком при "
         "старте. Налог не решён, он переехал"],
        ["«Вынесу в файл и буду сам напоминать агенту его прочитать»", "Пока вы помните",
         "Требует человека в каждом цикле. Вместо: отдать решение о релевантности модели, но по "
         "явному описанию"],
        ["«Сделаю это хуком»", "Там, где правило нельзя нарушить ни разу",
         "Путаница механизмов, и полезная: хук — механика, которую исполняет среда; скилл — "
         "знание, которое агент может учесть"],
        ["«Заведу скилл с описанием, по которому он срабатывает»", "Именно то, что нужно",
         "Не рано: файл инструкций уже платит за процедуру в девяти сессиях из десяти. "
         "Ломается, если описание написано для человека, а не как триггер"],
    ]
    reveal_table(s, 0.55, y, 12.23, 6.95 - y, headers, rows, [0.26, 0.26, 0.48],
                 row_highlight={4: "gold"}, header_size=9.3, cell_size=9)
...
def build_s43(p):
    build_stage_divider(p, "s43", stage_num=5, title="Ступень 5 · доступ наружу (MCP)",
...
def build_s44(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 5 · Сценарий-боль",
           "Каждая сессия начинается с копипаста issue в чат и заканчивается копипастом ответа "
           "обратно — несколько раз в неделю",
           title_size=16)
    y = 1.85
    ocean_box(s, 0.55, y, 12.23, 1.35)
    text_box(s, 0.8, y + 0.12, 11.7, 1.1,
             text="Сайт живёт третий месяц. Заказчик перестал писать в мессенджер и заводит "
                  "задачи прямо в репозитории. Почти каждая сессия начинается одинаково: открыть "
                  "браузер, скопировать текст задачи, вставить в чат — и скопировать результат "
                  "обратно в комментарий. Несколько раз в неделю.",
    ...
    gold_callout(s, 0.55, y, 12.23, 1.15,
                 ...
                 "черновики правок по ним. Вы почти готовы подключить официальный сервер доступа "
                 "к трекеру. Выберите, что вы проверите ДО того, как это сделать, — из карточек "
                 "ниже, — и в каком порядке.»", size=12.3)
    y += 1.15 + 0.18
    option_row(s, 0.55, y, 12.23, 0.85,
    ...
    text_box(s, 0.55, y, 12.23, 0.4,
             text="разбор и порядок проверок — на следующем слайде", size=12,
             italic=True, color=SLATE, align=PP_ALIGN.CENTER)
...
def build_s45(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 5 · Разбор",
           "Правильный ответ — не одна проверка, а последовательность из трёх, и порядок в ней "
           "имеет значение",
           title_size=17)
    y = 1.75
    text_box(s, 0.55, y, 12.23, 0.28,
             text="вопрос для контекста: «Что вы проверите до подключения?»",
             size=10.5, italic=True, color=SLATE)
    y += 0.34
    headers = ["Вариант", "Где это работает", "Почему здесь ещё рано — и что вместо"]
    rows = [
        ["«Ничего — сервер официальный, вендорский»",
         "Как эвристика снижения риска — да; как единственная проверка — нигде",
         "Официальность не иммунитет: у эталонного файлового сервера самого разработчика "
         "протокола нашли две уязвимости выхода за пределы песочницы"],
        ["«Посмотрю, сколько у сервера звёзд»",
         "Нигде: популярность и безопасность не коррелируют",
         "Сканирования находят проблемы у трети-двух третей случайных выборок. Вместо: смотреть "
         "на права, а не на звёзды"],
        ["«Посчитаю, сколько токенов это добавит»", "Всегда стоит: величина измеримая",
         "Не рано — но это третий вопрос, а не первый: можно отлично оптимизировать стоимость "
         "того, что вообще не следовало подключать"],
        ["«Проверю, какие права даю токену»", "Всегда: главная из проверок",
         "Не рано — но не отвечает на вопрос, нужно ли подключение вообще"],
        ["«Спрошу, не хватит ли обычной команды в терминале»", "Именно то, с чего надо начинать",
         "Не рано: это первый вопрос из трёх. Перестаёт хватать, когда доступ нужен регулярно, "
         "из нескольких сессий, с сохранением состояния"],
    ]
    table_h = 2.95
    reveal_table(s, 0.55, y, 12.23, table_h, headers, rows, [0.26, 0.28, 0.46],
                 row_highlight={4: "gold"}, header_size=8.6, cell_size=8.4)
    y += table_h + 0.16
    text_box(s, 0.55, y, 12.23, 0.3, text="ПОСЛЕДОВАТЕЛЬНОСТЬ ИЗ ТРЁХ ПРОВЕРОК",
             size=11, bold=True, color=TEAL, align=PP_ALIGN.CENTER)
    y += 0.34
    numbered_card(s, 0.55, y, 12.23, 1.3, [
        "нужно ли подключение вообще", "с какими правами", "по какой цене в контексте"], size=13)
...
def build_s53(p):
    build_stage_divider(p, "s53", stage_num=6, title="Ступень 6 · кастомный субагент",
...
def build_s55(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 6 · Такт 3",
           "Роль приходится переизобретать при каждом вызове, а у универсального субагента есть "
           "права, которых ревьюеру не нужно",
           title_size=15)
    hw = 5.85
    y = 1.85
    failure_card(s, 0.55, y, hw, 1.7, icon_name="repeat",
                 header_text="1. Роль собирается заново каждый раз",
                 body="Что проверять, на что смотреть, в каком виде отвечать — вы пишете это в "
                      "третий раз за неделю, каждый раз чуть иначе. Три вердикта несравнимы.",
                 body_size=11.3)
    rx = 0.55 + hw + 0.33
    failure_card(s, rx, y, hw, 1.7, icon_name="alert-triangle",
                 header_text="2. Полный набор инструментов — включая правку",
                 body="Один раз субагент не просто нашёл проблему, а сам её поправил — "
                      "по-своему. Разработчик узнал об этом из diff.",
                 body_size=11.3)
    y += 1.7 + 0.2
    gold_callout(s, 0.55, y, 12.23, 1.2,
                 "«Вы третий раз за неделю просите проверить правку отдельным агентом — и "
                 "каждый раз заново пишете, что проверять и в каком виде ответить. А в прошлый "
                 "раз проверяющий не просто нашёл проблему, а сам её поправил, по-своему. "
                 "Делегировать вы уже умеете, это ничего не стоит и ничего не требует. Выберите, "
                 "что теперь нужно завести — из карточек ниже — и обоснуйте зачем.»", size=11.6)
    y += 1.2 + 0.18
    option_row(s, 0.55, y, 12.23, 0.75,
...
def build_s56(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 6 · Разбор",
           "Кастомный субагент решает обе проблемы сразу: имя и фиксированная роль исключают "
           "переизобретение, а отнятые права делают «не правь сам» механической гарантией",
           title_size=13.5, title_h=1.15)
    y = 1.95
    text_box(s, 0.55, y, 12.23, 0.3,
             text="вопрос для контекста: «Что теперь нужно завести — и зачем?»",
             size=11, italic=True, color=SLATE)
    y += 0.38
    headers = ["Вариант", "Где это работает", "Почему здесь ещё рано — и что вместо"]
    rows = [
        ["«Ничего не надо — делегирование уже есть»",
         "Для разовой проверки — полностью достаточно (наполовину верно)",
         "Ломается на двух вещах: роль приходится переизобретать каждый вызов, и у "
         "универсального субагента есть права, которых ревьюеру не нужно"],
        ["«Буду писать запрос подробнее каждый раз»", "Пока помните и формулируете одинаково",
         "Формулировка каждый раз чуть другая — вердикты несравнимы между собой. И вопрос прав "
         "не решён вовсе"],
        ["«Сохраню этот запрос как скилл»",
         "Когда нужно переиспользовать знание внутри той же сессии",
         "Скилл подгружается в тот же контекст и выполняется той же сессией: изоляции он не "
         "даёт — ни контекстной, ни по правам"],
        ["«Позову коллегу»", "В команде, если коллега реально доступен",
         "Наш разработчик работает один — решение, зависящее от наличия второго человека, здесь "
         "не решение, а пожелание"],
        ["«Заведу субагента с именем и урезанным набором инструментов»", "Именно то, что нужно",
         "Не рано: роль повторилась трижды, и права уже сработали против вас. Перестаёт "
         "работать, если «независимость» объявлена, но не проверена"],
    ]
    reveal_table(s, 0.55, y, 12.23, 6.95 - y, headers, rows, [0.26, 0.28, 0.46],
                 row_highlight={4: "gold"}, header_size=8.8, cell_size=8.5)
...
def build_s61(p):
    ...
    text_box(s, 0.55, 6.18, 12.23, 0.6,
             text="κ=0,88 — шкала каппа Коэна от 0 до 1, чем ближе к 1, тем выше согласие экспертов; "
                  "доля провальных трасс — 41–87%; ни одна категория не про «модель ошиблась в факте»",
...
def build_s63(p):
    build_stage_divider(p, "s63", stage_num=7, title="Ступень 7 · процесс",
...
def build_s64(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 7 · Сценарий-боль",
           "Каждое отдельное утверждение в отчёте агента может быть правдой — и вместе они "
           "складываются в «готово», которого нет",
           title_size=15)
    y = 1.8
    dashed_box(s, 0.55, y, 12.23, 0.75, fill=GOLD_TINT, stroke=GOLD_DARK)
    text_box(s, 0.75, y, 11.8, 0.75,
    ...
    rows = [["какие именно, когда, с каким выводом?", "на проде или только локально?",
             "по чьему критерию?"]]
    reveal_table(s, 0.55, y, 12.23, 1.1, headers, rows, [0.34, 0.34, 0.32],
                 header_size=11, cell_size=10.5)
    y += 1.1 + 0.18
    gold_callout(s, 0.55, y, 12.23, 1.2,
                 "«Утро показа. Агент написал: ‘Готово, форма отправляет заявку, все тесты "
                 "прошли, можно показывать’. Выберите, что из этого вы проверите ПРЯМО СЕЙЧАС, "
                 "не читая переписку, — из карточек ниже, — и что для этого должно было "
                 "существовать до того, как агент это написал.»", size=11.8)
    y += 1.2 + 0.18
    option_row(s, 0.55, y, 12.23, 0.7,
               ["поверю — он явно\nзакончил", "прогоню тесты\nсам", "спрошу\n«ты уверен?»",
                "открою прод и\nпроверю руками", "ссылка на коммит +\nжурнал по ходу"], size=10.3)
    y += 0.7 + 0.16
    text_box(s, 0.55, y, 12.23, 0.4, text="разбор — на следующем слайде", size=12,
             italic=True, color=SLATE, align=PP_ALIGN.CENTER)
...
def build_s65(p):
    s = blank(p)
    set_slide_bg(s, WHITE)
    header(s, "Ступень 7 · Разбор",
           "Целевой ответ — не один артефакт, а два вместе: проверяемая ссылка на коммит и "
           "журнал, который вёлся по ходу работы, а не восстановлен по памяти",
           title_size=13.5, title_h=1.2)
    y = 1.95
    text_box(s, 0.55, y, 12.23, 0.3,
             text="вопрос для контекста: «Что вы проверите прямо сейчас — и что должно было "
                  "существовать до?»",
             size=10.5, italic=True, color=SLATE)
    y += 0.38
    headers = ["Вариант", "Где это работает", "Почему здесь ещё рано — и что вместо"]
    rows = [
        ["«Поверю — он же явно закончил»", "Нигде как единственная проверка",
         "Измерено: 75,8% провальных траекторий кодинг-агентов, оценивающих себя сами, "
         "уверенно отчитаны как «готово». Вместо: проверять факт, а не тон"],
        ["«Прогоню тесты сам ещё раз»", "Ловит прямое расхождение с заявлением — делать стоит",
         "Не ловит два случая: тесты зелёные при неподключённой правке и подмену самого теста. "
         "Не «вместо», а вдобавок"],
        ["«Спрошу у него самого: ты уверен?»", "Нигде: иллюзия проверки",
         "Самокоррекция без внешнего сигнала часто ухудшает точность; плюс подхалимство. "
         "Вместо: спрашивать не модель, а систему"],
        ["«Открою прод и отправлю заявку руками»",
         "Всегда: ровно тот критерий «готово», который мы записали в разделе 1",
         "Не рано — но не масштабируется: одну форму проверить руками можно, двадцать "
         "изменений за неделю нет"],
        ["«Ссылка на коммит плюс журнал, который вёлся по ходу»", "Именно то, что нужно",
         "Не рано: работу уже принимают на слово. Перестаёт работать, если процесс раздувается "
         "до состояния, когда его дешевле обойти, чем соблюсти"],
    ]
    reveal_table(s, 0.55, y, 12.23, 6.95 - y, headers, rows, [0.26, 0.28, 0.46],
                 row_highlight={4: "gold"}, header_size=8.8, cell_size=8.5)
...
def build_s66(p):
    ...
    speaker_notes(s, load_notes("s66"))
...
def build_s67(p):
    ...
    speaker_notes(s, load_notes("s67"))
...
def build_s68(p):
    ...
    text_box(s, 0.75, 5.38, 11.8, 1.15,
             ...
             size=13, italic=True, color=GOLD_DARK, line_spacing=1.32, anchor=MSO_ANCHOR.MIDDLE)
    speaker_notes(s, load_notes("s68"))
...
def build_s69(p):
    ...
    text_box(s, 0.75, 5.38, 11.8, 1.15,
             text="ни одна из пяти проверенных конфигураций модели-судьи не превысила AUROC 0,65 "
                  "(0,5 — случайное угадывание, 1,0 — идеальный судья). Верификация должна "
                  "проверять ФАКТ — файл существует, команда выполнена, тест прошёл, — а не "
                  "спрашивать другую модель, выглядит ли это завершённым.",
             size=12.5, italic=True, color=GOLD_DARK, line_spacing=1.3, anchor=MSO_ANCHOR.MIDDLE)
    speaker_notes(s, load_notes("s69"))
...
def build_s70(p):
    ...
    speaker_notes(s, load_notes("s70"))
...
def build_s71(p):
    ...
    speaker_notes(s, load_notes("s71"))
...
def build_s72(p):
    ...
    text_box(s, badge_x + 0.62, badge_y, badge_w - 0.84, badge_h,
             ...
             line_spacing=1.2)
    speaker_notes(s, load_notes("s72"))
...
SLIDE_BUILDERS = [
    ...
    build_s09, build_s10, build_s11, build_s12, build_s13, build_s14, build_s15, build_s16,
    build_s17, build_s18, build_s19, build_s20, build_s21, build_s22, build_s23, build_s24,
    build_s25, build_s26, build_s27, build_s28, build_s29, build_s30, build_s31, build_s32,
    build_s33, build_s34, build_s35, build_s36, build_s37, build_s38, build_s39, build_s40,
    build_s41, build_s42, build_s43, build_s44, build_s45, build_s46, build_s47, build_s48,
    build_s49, build_s50, build_s51, build_s52, build_s53, build_s54, build_s55, build_s56,
    build_s57, build_s58, build_s59, build_s60, build_s61, build_s62, build_s63, build_s64,
    build_s65, build_s66, build_s67, build_s68, build_s69, build_s70, build_s71, build_s72,
]
...
def main():
    ...
    for fn in SLIDE_BUILDERS:
        print(f"--- {fn.__name__} ---")
        fn(p)
    assert len(p.slides) == 72, f"expected 72 slides, got {len(p.slides)}"

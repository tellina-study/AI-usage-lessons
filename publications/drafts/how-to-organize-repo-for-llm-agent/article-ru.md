...
> **TL;DR** — Ответьте на два вопроса: сколько у вас источников и что это — код или документация?
>
...
Небольшое пояснение: под текстовым поиском (grep) я имею в виду поиск по точному совпадению текста в файлах. Это базовый инструмент, встроенный во все операционные системы и AI-кодинг-агенты. Когда я говорю «агент грепает» — он ищет конкретное слово или фрагмент по всем файлам проекта.
...
На уровне 0 агент видит всё — но не знает, что важно. CLAUDE.md даёт приоритеты. Текстовый поиск позволяет агенту читать только нужные файлы, а не загружать все 500 в контекст.
...
Текстовый поиск объективно превосходит семантический для точных совпадений. Как [отмечает ast-grep](https://ast-grep.github.io/blog/code-search-design-space.html): `ERROR_4532` в векторном пространстве неотличим от `ERROR_4533` — а это совершенно разные ошибки. Текстовый поиск находит ровно то, что нужно. Мой курс по AI перешёл на этот уровень, когда источников стало больше двадцати — текстовый поиск по экспортированным документам работал быстро и точно.
...
Этот уровень — для проектов, где документация создаётся людьми для людей, а AI-агент получает качественную навигацию бесплатно. Stripe docs, Kubernetes (3000+ страниц), Django, Terraform — обслуживают миллионы разработчиков без RAG и не собираются переходить.

Ключевая идея: документация организована по типу контента. [Фреймворк Diátaxis](https://diataxis.fr/) делит её на 4 типа — обучение, инструкции, справочник, объяснение. Когда текстовый поиск находит слово «authentication» в 15 файлах, агент без типизации вынужден читать все 15. С Diátaxis — сразу идёт в `how-to/configure-oauth.md`. Фреймворк [принят](https://ubuntu.com/blog/diataxis-a-new-foundation-for-canonical-documentation) Cloudflare, Ubuntu, Django, Gatsby.
...
Чтобы начать: рассортируйте существующие документы по 4 типам [Diátaxis](https://diataxis.fr/), добавьте навигационный index.md. Один день для среднего проекта. Docusaurus, MkDocs или Sphinx — все поддерживают автоматическую навигацию из файловой структуры.
...
Если вам нужен семантический поиск по разнородным источникам, но без вики-компиляции — можно просто загрузить документы в локальный RAG и получить поиск по смыслу за один вечер. Это быстрая альтернатива для тех случаев, когда компилировать вики избыточно, а текстовый поиск уже не справляется.

Чтобы начать с вики: создайте `raw/` и `wiki/` директории, добавьте CLAUDE.md с конвенциями из [гиста Карпати](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f). Не компилируйте всё сразу — загружайте по 10–20 документов за сессию. Вики растёт органически.
...
Предприниматель Вамши Редди написал Карпати: «У каждого бизнеса есть директория raw/. Никто её ещё не скомпилировал. Вот и продукт». Карпати согласился. Я сам потратил спринт на четырёхслойную систему с онтологией и SPARQL-запросами — а потом открыл граф знаний и обнаружил, что он пуст. Рядом лежал CLAUDE.md на 40 строк, через который агент уже неделю находил всё нужное.

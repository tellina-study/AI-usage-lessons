"""
Full 43-slide build of Лекции 10 «AI в сельском хозяйстве».

Source-of-truth: deck.yaml v1 (status=finalized, 43 slides) + chapter v3.1
(status=finalized, ~31960 слов, 3 parts) + slides/*.md (43 файла readable speaker notes).

Issue #126 · downstream-каскад от chapter (book-first).
...
Lec-N-1 pattern compliance: match lec-09 (cover + lecture-map + 7 section dividers +
dedicated Q&A; top progress bar только на dividers + cover).

Build via: python3 build_lec10.py — generates lec-10.pptx (43 slides).
"""
import re
...
# 7 sections: 0 Открытие, 1 Поле, 2 Робот, 3 Животное, 4 Цепочка, 4-bis Среда, 5 Полка
SECTIONS = ["Открытие", "L1 Поле", "L2 Робот", "L3 Животное", "L4 Цепочка", "Среда", "L5 Полка"]
...
def load_speaker_notes(slide_id):
    """Extract speaker notes from slides/sNN-*.md file."""
...
def main():
    ...
    # Import slide builders from modular parts to keep each file <600 lines
    from build_lec10_p1 import build_part1_full  # s01-s14 (14 slides — Раздел 0 + L1)
    from build_lec10_p2 import build_part2  # s15-s21+s17 (12 slides — Р2 L2 «Робот»)
    from build_lec10_p3 import build_part3  # s22-s38 (17 slides — L3 + L4 + Среда + L5 + closing + Q&A)

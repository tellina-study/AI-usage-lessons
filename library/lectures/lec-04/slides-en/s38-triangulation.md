...
**GitClear** — two independent measurements: (211M lines, 2020–24) refactoring **~25% → <10%**, duplicates **8.3% → 12.3%**; (623M changes, 2023–26, a separate sample) refactoring **21% → 3.8%** (-70%), duplicates **40.3 → 73.0 per million lines** (+81%), churn **+15%**. Three markers of one process on both samples. *(Both are correlation, not an RCT.)*
...
DORA is the largest system-level program, about five thousand respondents. Its picture is dual and honest: in roughly ninety percent of cases throughput grows, but the link of AI adoption to delivery stability is negative, and already for the second year running [1]. Hence their load-bearing lens: AI amplifies what is already there, it is a multiplier, not a source of quality. GitClear looks at the code itself, and here there are two independent measurements: over two hundred eleven million lines (2020–2024) the share of refactored code fell from twenty-five to under ten percent and the share of duplicates grew; on a separate, more recent sample of six hundred twenty-three million changes (2023–2026) refactoring fell by seventy percent, duplication rose by eighty-one percent, and churn rose by fifteen [2]; the caveat is honest — both samples are correlation, not a controlled experiment. And METR, which we saw at the very start: sixteen experts on familiar code, objectively minus nineteen percent to speed while believing in a speed-up — the perception gap [3]; the mandatory baseline — on unfamiliar code the effect differs.

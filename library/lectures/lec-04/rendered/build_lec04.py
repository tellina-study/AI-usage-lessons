...
def build_s01(p):
    ...
    text_runs(s, 0.55, 5.0, 5.9, 1.7, [
        ...
        {"newpara": True, "text": "~3 sec inference", "size": 15, "color": GOLD, "bold": True},
        {"text": "  ·  ", "size": 15, "color": DEEP},
        {"text": "локально в браузере", "size": 15, "color": TEAL, "bold": True},
        {"newpara": True, "text": "18 патологий: pneumonia, cardiomegaly, atelectasis…",
    ...
    text_box(s, x=box_x, y=box_y + box_h + 0.05, w=box_w, h=0.4,
             text="Chester AI · mlmed.org/tools/xray/ · heatmap + 18 классов",
             size=11, italic=True, color=LIGHT, align=PP_ALIGN.CENTER)
    text_box(s, x=0.55, y=7.0, w=12.3, h=0.35,
             text="Cohen et al. 2019 (arXiv:1901.11210) · backup PNG (live demo при наличии интернета)",
...
def build_s04(p):
    ...
    text_box(s, x=info_x + 0.3, y=info_y + 0.25, w=info_w - 0.6, h=0.45,
             text="mosmed.ai — 5 лет production",
             size=18, bold=True, color=DEEP, line_spacing=1.15)
    text_box(s, x=info_x + 0.3, y=info_y + 0.75, w=info_w - 0.6, h=0.30,
             text="ДЗМ Москвы → федеральный MosMedAI (май 2024)",
    ...
    text_box(s, x=0.55, y=7.0, w=12.3, h=0.35,
             text="FDA AI/ML list end-2025 (theimagingwire.com 10.12.2025) · Remedium 2025 · mos.ru AI Leaders Award.",
             size=10, italic=True, color=SLATE, align=PP_ALIGN.LEFT)
...
def build_s05(p):
    ...
    text_box(s, x=rm_x + 0.3, y=rm_y + 0.2, w=rm_w - 0.6, h=0.4,
             text="Roadmap — 4 раздела",
             size=14, bold=True, color=MID)
    roadmap = [
        ("1", "Карта AI в медицине"),
        ("2", "AI-диагностика как зеркало"),
        ("3", "Drug discovery — обещания vs реальность"),
...
def build_s06(p):
    s = blank(p)
    slide_title(s, "AI в медицине — 4 разные индустрии, не один tools-set.", size=24)
    text_box(s, x=0.55, y=1.20, w=12.3, h=0.40,
             text="Modality × Scope = design-driven taxonomy",
             size=15, italic=True, color=MID)
    grid_x, grid_y, grid_w, grid_h = 1.40, 1.95, 10.7, 4.6
    cell_w = grid_w / 2
    cell_h = grid_h / 2
    cells = [
        ("AI-диагностика", "CT · MRI · X-ray · дермато-скан",
         "mosmed.ai · IDx-DR · Aidoc",
         "lucide-scan-blue.png", MID, 0, 0),
        ("Population imaging", "Скрининговые программы",
         "MASAI (Sweden) · BreastScreen",
         "lucide-heart-pulse-blue.png", LIGHT, 1, 0),
        ("Personalized medicine", "Genomic AI · clinical decision",
         "Tempus · Foundation Med · Webiomed",
         "lucide-pill-blue.png", LIGHT, 0, 1),
        ("Drug discovery + эпид.", "Generative chemistry · structures",
    ...
    for title, sub, exs, icon, color, col, row in cells:
        ...
        text_box(s, x=x + 1.15, y=y + 0.30, w=cell_w - 1.35, h=0.55,
                 text=title, size=18, bold=True, color=color, line_spacing=1.15)
        text_box(s, x=x + 0.3, y=y + 1.20, w=cell_w - 0.5, h=0.45,
                 text=sub, size=13, color=DEEP, italic=True)
        text_box(s, x=x + 0.3, y=y + 1.70, w=cell_w - 0.5, h=0.50,
    ...
    text_box(s, x=grid_x, y=grid_y + grid_h + 0.1, w=grid_w, h=0.35,
             text="◄ single patient        SCOPE        population / pharma ►",
...
def build_s07(p):
    s = blank(p)
    slide_title(s, "За 10 лет — от 6 до 1 451 AI-устройств в FDA-list.", size=24)
    text_box(s, x=0.55, y=1.20, w=12.3, h=0.40,
    ...
    text_box(s, x=chart_x + 0.3, y=chart_y + chart_h - 0.40, w=chart_w - 0.6, h=0.35,
             text="Cumulative AI/ML-enabled medical devices (FDA), end-2025.",
             size=11, italic=True, color=LIGHT, align=PP_ALIGN.CENTER)
    ...
    text_box(s, x=info_x + 0.3, y=info_y + 1.45, w=info_w - 0.6, h=0.35,
             text="cumulative · end-2025", size=11, italic=True, color=SLATE)
    ...
    text_box(s, x=info_x + 0.3, y=info_y + 3.95, w=info_w - 0.6, h=0.65,
             text="Pivot — 2022–2024:\nexponential acceleration",
             size=11, italic=True, color=GOLD, line_spacing=1.20)
    text_box(s, x=0.55, y=7.0, w=12.3, h=0.35,
             text="FDA AI/ML list (fda.gov, end-2025) · JAMA Network Open systematic review 2025 · The Imaging Wire 10.12.2025.",
             size=10, italic=True, color=SLATE, align=PP_ALIGN.LEFT)
...
def build_s08(p):
    s = blank(p)
    slide_title(s, "Медицина — инструктивный case для инженера.", size=24)
    text_box(s, x=0.55, y=1.20, w=12.3, h=0.40,
             text="Высокие ставки + строгое регулирование + прозрачная operational scale.",
             size=15, italic=True, color=MID)
    cards = [
        ("Высокие ставки",
         "Ошибка модели → ошибка диагноза → вред пациенту.",
         "Калибровка confidence, audit-trail, fallback.",
        ...
        ("Прозрачная operational scale",
         "mosmed.ai: 14M+ исследований · 74 региона · 70 сервисов.",
...
def build_s09(p):
    s = blank(p)
    slide_title(s, "AI-диагностика — это computer vision, не LLM.", size=24)
    text_box(s, x=0.55, y=1.20, w=12.3, h=0.40,
             text="image → label с confidence score. Это не LLM.",
             size=15, italic=True, color=MID)
    pipe_y = 2.0
    pipe_h = 2.6
    ...
    text_box(s, x=sample_x + 0.3, y=sample_y + 0.15, w=sample_w - 0.6, h=0.35,
             text="Пример: CheXNet — 121-layer DenseNet, 14 патологий грудной клетки (Rajpurkar et al. 2017)",
...
def build_s10(p):
    s = blank(p)
    slide_title(s, "Для медицинского AI «accuracy» недостаточна.", size=24)
    text_box(s, x=0.55, y=1.20, w=12.3, h=0.40,
             text="Sens · Spec · Prevalence · PPV. Sens/spec не зависят от prevalence; PPV — зависит.",
    ...
    text_box(s, x=mt_x + 0.3, y=mt_y + 0.15, w=mt_w - 0.6, h=0.35,
             ...
             size=14, bold=True, color=DEEP)
    metrics = [
        ("Sensitivity", "TP / (TP+FN)", "доля больных, которых поймал AI"),
        ("Specificity", "TN / (TN+FP)", "доля здоровых, не напуганных"),
        ("Prevalence",  "(TP+FN) / Total", "как часто болезнь в популяции"),
        ("PPV",         "TP / (TP+FP)", "если AI сказал болен, вер-ть"),
    ]
    row_y = mt_y + 0.65
    row_h = 0.78
    for i, (name, formula, meaning) in enumerate(metrics):
        ry = row_y + i * row_h
        is_ppv = (name == "PPV")
        text_box(s, x=mt_x + 0.3, y=ry, w=1.7, h=0.40,
                 text=name, size=16, bold=True,
...
def build_s11(p):
    s = blank(p)
    slide_title(s, "Imaging — AI+врач > врач. Reasoning — augmentation gap.", size=22)
    text_box(s, x=0.55, y=1.10, w=12.3, h=0.40,
             text="Вопрос «AI или врач» поставлен неправильно. Правильный — «какая задача и какой workflow».",
             size=13, italic=True, color=MID)
    rows = [
        ("Liu et al. 2019",
         "Lancet Digital Health — meta-analysis 14 prospective",
         "Imaging meta-analysis",
         "pooled sens AI 0.87  ·  врач 0.85",
         "Близко к паритету, исторический baseline",
         MID, False),
        ("MASAI 2024–2025",
         "Lancet Digital Health (2024) + Lancet (2025)",
         "Mammography RCT, n > 100 000 (Sweden)",
         "sens 80.5% (AI) vs 73.8% (стандарт)  ·  workload −44%  ·  interval cancer −12%",
         "AI+радиолог → значимо лучше каждого alone",
         MID, True),
        ("Goh et al. 2024",
    ...
    for i, (study, journal, domain, result, takeaway, color, is_gold) in enumerate(rows):
        ...
        text_box(s, x=0.75, y=ry + 0.55, w=3.0, h=0.45,
                 text=journal, size=10, italic=True, color=SLATE, line_spacing=1.20)
        text_box(s, x=4.0, y=ry + 0.15, w=2.7, h=0.45,
        ...
        text_box(s, x=6.95, y=ry + 0.40, w=result_w, h=0.55,
                 text=result, size=13, bold=is_gold, color=DEEP, line_spacing=1.30)
...
def build_s12(p):
    s = blank(p)
    slide_title(s, "mosmed.ai — обещание сбылось в operational форме.", size=24)
    text_box(s, x=0.55, y=1.20, w=12.3, h=0.40,
             text="5 лет production · 14M+ исследований · 74 региона · 70 AI-сервисов.",
             size=14, italic=True, color=MID)
    pipe_y = 1.85
    pipe_h = 1.3
    stages = [
        ("Снимок", "КТ / МРТ / X-ray", LIGHT),
        ("mosmed.ai\ncloud", "federated AI-platform", MID),
        ("AI-анализ", "70 services\n43 области", MID),
        ("Врач + 2nd opinion", "decision\n+ verify", GOLD),
    ]
    n = len(stages)
    arrow_w = 0.5
    ...
    cells = [
        ...
        ("74", "региона РФ", False),
        ("18 млн+", "изображений processed", False),
        ("70", "AI-сервисов на 43 областях", False),
        ("11", "нац. стандартов · 300+ datasets", False),
    ]
    ...
    text_box(s, x=0.55, y=6.85, w=12.3, h=0.40,
             text="mos.ru AI Leaders Award · Remedium 2025 · Healthcare ME 2026. Federated platform: Сбер AI Lab, Care Mentor AI, Третье Мнение, Webiomed.",
             size=10, italic=True, color=SLATE, align=PP_ALIGN.LEFT)
...
def build_s13(p):
    s = blank(p)
    slide_title(s, "Bias = consequence design choices, не bug.", size=24)
    text_box(s, x=0.55, y=1.20, w=12.3, h=0.40,
             text="AI хорошо работает в распределении обучения. Outside — может проваливаться unfairly.",
             size=14, italic=True, color=MID)
    cards = [
        ("Dermatology — skin tone",
         ["Mechanism: training datasets (ISIC) перепредставляют светлую кожу",
          "Evidence: Daneshjou 2022 — sens ↓ 20–30% на коже Фитцпатрика V–VI",
          "Fix: fine-tuning на DDI закрыл gap; AI > дерматолога на тёмной коже"],
         "lucide-eye-blue.png", "Daneshjou 2022, Science Advances"),
        ("Pulse oximeter — racial bias",
         ["Mechanism: sensor systematically overestimates SpO2 на dark skin",
          "Evidence: Sjoding 2020 NEJM — гипоксия пропускается чаще у Black",
          "AI implication: модели с SpO2 input feature наследуют sensor bias"],
    ...
    gold_callout(s, 0.55, 6.55, 12.3, 0.55,
                 "→ Validation set должен покрывать deployment population. Не academic point — профессиональная ответственность.",
                 size=14)
...
def build_s14(p):
    ...
    text_box(s, x=0.55, y=0.95, w=12.3, h=0.60,
             text="AI-диагностика — обещание сбылось (mosmed: 14M+ исследований, 74 региона).",
    ...
    text_runs(s, qbox_x + 0.4, qbox_y + 0.4, qbox_w - 0.8, qbox_h - 0.8, [
        {"text": "Drug discovery — ", "size": 28, "color": DEEP, "bold": True},
        {"text": "обещали 10× быстрее.", "size": 28, "color": GOLD, "bold": True},
        {"newpara": True, "text": "Что реально к 2026 году?",
         "size": 28, "color": DEEP, "bold": True, "align": PP_ALIGN.CENTER},
...
def build_s15(p):
    s = blank(p)
    slide_title(s, "AI ускоряет discovery, не clinical trials.", size=24)
    text_box(s, x=0.55, y=1.20, w=12.3, h=0.40,
             text="10–15 лет · $1–2 млрд · ~6.7% success rate. AI ускоряет stages 1–3.",
             size=14, italic=True, color=MID)
    pipe_y = 2.65
    pipe_h = 2.6
    stages = [
        ("1. Target ID", "AlphaFold\nAlphaProteo", "lucide-target-blue.png", LIGHT, True),
        ("2. Hit discovery", "Insilico\nExscientia\nGenerate Bio", "lucide-flask-conical-blue.png", MID, True),
        ("3. Lead opt.", "Simulation\n+ ML", "lucide-sparkles-blue.png", MID, True),
        ("4. Preclinical", "Toxicity\npredict", "lucide-microscope-blue.png", DEEP, False),
        ("5. Clinical I/II/III", "Patient\nstratification", "lucide-users-blue.png", DEEP, False),
    ]
    n = len(stages)
    arrow_w = 0.42
    stage_w = (12.3 - (n - 1) * arrow_w) / n
    ...
    text_box(s, x=pipe_x, y=banner_y, w=ai_banner_w, h=0.40,
             text="AI ускоряет значительно (4–5 лет → 12–18 мес)",
    ...
    text_box(s, x=grey_x, y=banner_y, w=grey_w, h=0.40,
             text="AI помогает marginally — биология",
             size=12, bold=True, color=DEEP, align=PP_ALIGN.CENTER,
             anchor=MSO_ANCHOR.MIDDLE)
    ...
    gold_callout(s, 0.55, 5.55, 12.3, 0.55,
                 "→ ~90% clinical attrition rate — AI не меняет. Биология, не алгоритм.",
                 size=14)
    text_box(s, x=0.55, y=6.25, w=12.3, h=0.6,
             text="«AI ускорил design» (verified) ≠ «AI ускорил approval» (не verified).",
             size=15, bold=True, color=DEEP, align=PP_ALIGN.CENTER, line_spacing=1.25)
    text_box(s, x=0.55, y=6.95, w=12.3, h=0.35,
...
def build_s16(p):
    s = blank(p)
    slide_title(s, "AlphaFold 200M+; AlphaProteo проектирует binders de novo.", size=22)
    text_box(s, x=0.55, y=1.10, w=12.3, h=0.40,
             text="Решённая задача 50-летней давности · Нобель по химии 2024 (Hassabis + Jumper + Baker).",
             size=13, italic=True, color=MID)
    card_x = 0.55
    card_w = 7.5
    card_h = 1.55
    cards = [
        ("AlphaFold DB",
         "200M+",
         "structures",
         "alphafold.ebi.ac.uk · open, free", MID, True),
        ("AlphaProteo (Sep 2024)",
         "88%",
         "success rate BHRF1",
         "First AI binder для VEGF-A · 3–300× affinity", MID, True),
        ("AlphaFold 3 (Nature 2024)",
         "+50%",
         "accuracy PoseBusters",
         "Protein–ligand interactions · diffusion-based", DEEP, False),
    ]
    ...
    text_box(s, x=img_x + 0.2, y=img_y + img_h - 0.55, w=img_w - 0.4, h=0.40,
             text="3D protein structure (schematic)",
             size=11, italic=True, color=LIGHT, align=PP_ALIGN.CENTER)
...
def build_s17a(p):
    s = blank(p)
    slide_title(s, "Rentosertib — первый AI-designed drug с peer-reviewed Phase IIa.", size=22)
    text_box(s, x=0.55, y=1.10, w=12.3, h=0.40,
    ...
    tl_h = 2.5
    events = [
        ("2020–2022", "AI-driven target ID\n→ preclinical candidate",
         "~18 мес vs 4–5 лет", LIGHT, False),
        ("Окт 2024", "Topline Phase IIa\nresults announced",
         "Insilico press release", MID, False),
        ("Июнь 2025", "Nature Medicine\npeer-reviewed RCT n=71",
         "21 China sites · IPF", GOLD, True),
    ]
    n = len(events)
    arrow_w = 0.50
    total_w = 12.3
...
def build_s17b(p):
    s = blank(p)
    slide_title(s, "DSP-1181 — reality check: AI ускорил design, не clinical.", size=22)
    text_box(s, x=0.55, y=1.10, w=12.3, h=0.40,
             text="«Первый AI-designed drug в clinical trials» (2020) → Phase 1 discontinued (2022).",
             size=13, italic=True, color=MID)
    tl_y = 2.4
    tl_h = 2.5
    events = [
        ("Январь 2020", "Phase 1 entry\nExscientia + Sumitomo",
         "OCD · Japan · 12 мес vs 4–5 лет", LIGHT, False),
        ("2022", "Phase 1\nDISCONTINUED",
         "Cause not specified", DARK_GREY, True),
        ("Май 2026", "Synapse status:\nDiscontinued",
         "Current R&D status", DEEP, False),
    ]
    n = len(events)
    arrow_w = 0.50
    total_w = 12.3
    ...
    text_runs(s, 0.85, insight_y + 0.20, 11.7, insight_h - 0.4, [
        ...
        {"newpara": True,
         "text": "«AI drug = быстро + эффективно»", "size": 14, "italic": True, "color": SLATE},
        {"text": "  — две объединённые в рекламе claim'ы.",
...
def build_s18(p):
    s = blank(p)
    slide_title(s, "Medical AI = high-risk во всех 3 крупных юрисдикциях.", size=24)
    text_box(s, x=0.55, y=1.20, w=12.3, h=0.40,
             text="Approaches отличаются процессами, не principles.",
             size=14, italic=True, color=MID)
    cols = [
        ("США (FDA)",
         "lucide-flag-blue.png",
         [("Framework", "SaMD + AI/ML-specific"),
          ("PCCP final", "4 декабря 2024"),
          ("До PCCP", "каждое обновление = new submission (12–18 мес)"),
          ("С PCCP", "vendor pre-declares допустимые updates")],
         False),
        ("EU (AI Act)",
         "lucide-flag-blue.png",
         [("Regulator", "EU Commission + Notified Bodies"),
          ("Базис", "Article 6 + Annex III (high-risk)"),
          ("⚠ 2 авг 2026", "Annex III high-risk → 2.5 мес после лекции"),
          ("⚠ 2 авг 2027", "MDR full compliance для medical AI")],
         True),
        ("РФ (Росздравнадзор)",
         "lucide-flag-blue.png",
         [("Registered", "57 AI-медизделий (52 RF + 5 foreign)"),
          ("Expedited", "1 марта 2025 (ПП РФ № 1684)"),
          ("Первый AI", "Webiomed, 3 апреля 2020"),
          ("Дата-локализация", "ФЗ-23, 1 июля 2025")],
         False),
    ]
    col_y = 1.95
    col_h = 4.8
...
def build_s19(p):
    ...
    steps = [
        ("Step 1  ·  3 мин",
         "Открой web-chat (ChatGPT / Claude / YandexGPT / GigaChat). Введи промпт ниже:",
         MID),
        ("Step 2  ·  3 мин",
         "Отметь карандашом: 1 неточность ИЛИ 1 unverifiable claim ИЛИ 1 слишком абстрактное место.",
         LIGHT),
        ("Step 3  ·  4 мин — reveal",
         "Лектор спросит — 2–3 студента читают (1 мин). Лектор покажет control-ответ.",
         DEEP),
    ]
    step_y = card_y + 0.35
    ...
    text_box(s, x=card_x + 0.45, y=prompt_y + 0.40, w=card_w - 0.90, h=0.75,
             text="«Объясни мне, что такое sensitivity и specificity для AI-диагностики на конкретном примере (mammography screening). Объясни как для студента 2 курса техн. вуза, со знанием базовой probability.»",
             size=12, italic=True, color=DEEP, font=FONT_MONO, line_spacing=1.40)
    ...
    text_box(s, x=ctrl_x + 0.30, y=ctrl_y + 0.30, w=ctrl_w - 0.60, h=0.40,
             text="Control-ответ (готовится накануне)",
    ...
    lines = [
        "Sensitivity — это доля...",
        "...правильно поймал AI.",
        "TP / (TP + FN).",
        "",
        "Specificity — это доля...",
        "...правильно отпустил.",
        "TN / (TN + FP).",
        "",
        "Mammography примеры:",
    ...
    gold_callout(s, 0.55, 6.95, 12.3, 0.40,
                 "Цель — paragraph-level critique. AI отлично объясняет — паттерн. AI даёт цифры без citation — анти-паттерн.",
                 size=12)
...
def build_s20(p):
    ...
    items = [
        ("Bias в medical AI",
         "Obermeyer 2019 deep-dive",
         "lucide-scale-blue.png"),
        ("LLM anti-pattern",
         "NEDA Tessa + adversarial + 40M self-dx",
         "lucide-message-circle-warning-blue.png"),
        ("Безопасность данных + responsibility",
         "Change Healthcare + 4-actor framework",
    ...
    gold_callout(s, 0.55, 6.85, 12.3, 0.50,
                 "→ Думать про границы сразу, на стадии design — не post-hoc после первого инцидента.",
                 size=14)
...
def build_s21(p):
    ...
    text_box(s, x=0.55, y=1.10, w=12.3, h=0.40,
             text="Commercial AI для 200M Americans systematically underestimated severity для black patients.",
             size=13, italic=True, color=MID)
    pipe_y = 1.85
    pipe_h = 1.65
    boxes = [
        ("Goal", "Identify patients\nneeding\nadditional care", LIGHT),
        ("Proxy used", "Spending on\nprevious care\n(не severity)", GOLD),
        ("Bias source", "Black: −$1 800/y\n(access disparities)\n→ AI «менее больны»", DARK_GREY),
    ]
    n = 3
    arrow_w = 0.50
    ...
    text_box(s, x=rc_x + 0.3, y=rc_y + 0.20, w=rc_w - 0.6, h=0.40,
             text="При одинаковом risk score у Black:",
             size=13, bold=True, color=DEEP)
    ...
    text_box(s, x=rc_x + 0.3, y=rc_y + rc_h - 0.40, w=rc_w - 0.6, h=0.35,
             text="Science 2019 · n ≈ 200M американцев",
             size=11, italic=True, color=LIGHT)
    ...
    text_box(s, x=fix_x + 0.3, y=fix_y + 0.20, w=fix_w - 0.6, h=0.40,
             text="Fix: hybrid proxy (cost + chronic conditions)",
             size=13, bold=True, color=DEEP)
    ...
    text_box(s, x=fix_x + 0.3, y=fix_y + 1.65, w=fix_w - 0.6, h=0.50,
             text="доля Black patients в high-risk care management programs",
             size=12, color=DEEP, line_spacing=1.30)
    text_box(s, x=fix_x + 0.3, y=fix_y + rc_h - 0.40, w=fix_w - 0.6, h=0.35,
             text="−84% bias  ·  не теоретический fix — реальные пациенты",
...
def build_s22(p):
    s = blank(p)
    slide_title(s, "LLM в медицине ≠ medical AI — 3 documented cases.", size=22)
    text_box(s, x=0.55, y=1.10, w=12.3, h=0.40,
             text="Vendor accountability · adversarial hallucination · mass self-diagnosis (2025–2026).",
             size=13, italic=True, color=MID)
    cards = [
        ("NEDA Tessa scandal",
        ...
        ("Adversarial hallucination",
         ["6 ведущих LLM, 300 clinical vignettes",
          "С подсаженной фейк-деталью → расширяют в 83% случаев",
          "Mitigation prompt → halves but ≠ zero"],
         "lucide-alert-octagon-blue.png", "83% halluc. rate",
         "Communications Medicine 2025 (Nature)"),
        ("Self-diagnosis at scale",
         ["~40M Americans use ChatGPT для health past 3 months",
          "3 из 5 US adults",
...
def build_s23(p):
    ...
    grid_h = 1.7
    cells = [
        ("190M", "Americans affected (~57% US pop)", False),
        ("$2.457B", "recovery cost UHG Q3 2024", True),
        ("6 TB", "exfiltrated", False),
        ("Multi-week", "US claims disruption", False),
        ("$22M", "ALPHV/BlackCat ransom", False),
    ]
    n = len(cells)
    gap = 0.12
    ...
    text_box(s, x=0.85, y=bridge_y + 0.20, w=11.7, h=0.40,
             text="AI connection — medical AI training datasets inherit risk",
             size=14, bold=True, color=DEEP)
    text_box(s, x=0.85, y=bridge_y + 0.65, w=11.7, h=0.65,
             text="mosmed.ai = 18M+ images → ransomware target scope. Анонимизация ≠ anonymity (Sweeney 2002 re-identification governor MA).",
...
def build_s24(p):
    s = blank(p)
    slide_title(s, "Врач решает. AI подсказывает. Final responsibility undivided.", size=22)
    text_box(s, x=0.55, y=1.10, w=12.3, h=0.40,
             text="4 actors с разной комбинацией technical control × legal liability.",
             size=13, italic=True, color=MID)
    ...
    for title, sub, exs, icon, color, col, row, is_gold in cells:
        ...
        text_box(s, x=x + 0.30, y=y + 1.10, w=cell_w - 0.5, h=0.40,
                 text=sub, size=12, color=DEEP, line_spacing=1.25)
        text_box(s, x=x + 0.30, y=y + 1.50, w=cell_w - 0.5, h=0.30,
                 text=exs, size=11, italic=True, color=SLATE)
    text_box(s, x=grid_x, y=grid_y + grid_h + 0.1, w=grid_w, h=0.30,
             text="◄ low control · TECHNICAL CONTROL · high control ►",
    ...
    cl_y = 6.0
    cl_h = 1.0
    ocean_box(s, 0.55, cl_y, 12.3, cl_h)
    text_runs(s, 0.85, cl_y + 0.20, 11.7, cl_h - 0.4, [
        {"text": "Врач ставит диагноз. AI подсказывает. Final clinical responsibility — ",
         "size": 19, "color": DEEP, "bold": True},
        {"text": "undivided.", "size": 19, "color": GOLD, "bold": True},
...
def build_s26(p):
    s = blank(p)
    slide_title(s, "3 takeaways — медицинский AI к 2026 году.", size=24)
    text_box(s, x=0.55, y=1.20, w=12.3, h=0.40,
...
def build_s27(p):
    ...
    text_box(s, x=ph_x + 0.4, y=ph_y + ph_h - 0.50, w=ph_w - 0.8, h=0.40,
             text="callback to central question — payoff",
             size=12, italic=True, color=LIGHT, align=PP_ALIGN.LEFT)
    text_box(s, x=0.55, y=6.85, w=12.3, h=0.40,
             text="Transparency · validated population · audit-trail — три инженерных принципа в копилку Lec 9.",
...
def build_s28(p):
    ...
    text_runs(s, 0.85, card_y + 2.05, card_w - 0.5, 0.50, [
        ...
        {"text": "  эффективности (Russian context)",
    ...
    text_box(s, x=0.85, y=card_y + 2.60, w=card_w - 0.5, h=0.35,
             text="Predictive maintenance · QC · physical AI",
             size=11, italic=True, color=LIGHT)
...
def build_s29(p):
    ...
    backups = [
        ...
        "3. Medical AI стартап — первый вопрос о validation?",
    ]
    chip_y = 4.85
    chip_h = 0.65
    chip_w = 12.3
    for i, b in enumerate(backups):
        ...
        text_box(s, x=0.85, y=cy, w=chip_w - 0.6, h=chip_h,
                 ...
                 anchor=MSO_ANCHOR.MIDDLE, line_spacing=1.20)
    text_box(s, x=0.55, y=7.10, w=12.3, h=0.30,
             text="Курс «AI в разных индустриях» · sem-04 case study · office hours",
             size=10, italic=True, color=LIGHT, align=PP_ALIGN.CENTER)

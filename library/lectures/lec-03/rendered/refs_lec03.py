...
URLS = {
    ...
    # RAG cases / deep-research (v6 D1 #196) — §2.12
...
SLIDE_REFS = {
    ...
    "s-rag-chunk2": [
        ...
        ("2", "firecrawl — chunking eval (recall@k в изоляции может врать)", "firecrawl_chunking",
         "recall@k 91,9% при 54% end-to-end ответов — оценивать chunking и через end-to-end", True),
    ],
    "s-rag-design": [
    ...
    "s-rag-chunk3": [
        ("1", "Anthropic — Contextual Retrieval (таблицы — тихий провал retrieval)", "anthropic_contextual",
         "расплющивание таблицы рушит связи строк/столбцов без ошибки — лечить table-aware нарезкой"),
        ("2", "firecrawl — Chunking Strategies (re-chunk при смене эмбеддера, k-чувствительность)", "firecrawl_chunking",
         "размер чанка привязан к окну модели; оценка chunking только через recall@k И end-to-end", True),
    ],
    "s-rag-cases": [
        ("1", "Barnett et al. 2024 — Seven Failure Points (RAG-архетипы)", "barnett_7fail",
         "«вернул что-то ≠ вернул правильное»: точки отказа поперёк типовых RAG-задач"),
        ("2", "Stanford HAI/RegLab — Legal-AI галлюцинирует 17–33% (Lexis/Westlaw)", "stanford_legalai",
         "коммерческие legal-AI на RAG галлюцинируют 1 из 6+ запросов, несмотря на «0%»-маркетинг", True),
        ("3", "Sourcegraph Cody — контекст без эмбеддингов (классический поиск)", "sourcegraph_search",
         "Cody на Enterprise убрал эмбеддинги → лексический/структурный поиск дешевле, безопаснее, масштабнее", True),
    ],
    "s-rag-research": [
        ("1", "Anthropic — Multi-Agent Research System (агентный цикл RAG)", "anthropic_multiagent",
         "reasoning-LLM + агентная петля: рассуждение направляет поиск, находки уточняют рассуждение"),
        ("2", "arXiv:2604.03173 — фабрикация цитат в retrieval-augmented (3–13%)", "citation_fabrication",
         "3–13% URL сфабрикованы; deep research хуже обычного поиска 10,7% vs 4,8% на запрос", True),
    ],
    "s11": [
    ...
    "s14": [
        ("1", "Hinton, Vinyals, Dean 2015 — Distilling the Knowledge", "hinton_distill",
         "дистилляция обучает student матчить распределение teacher (soft targets, KL); это дообучение маленькой модели"),
        ("2", "Stanford Alpaca 2023 / Sanh et al. 2019 DistilBERT", "peft_survey",
         "Alpaca: LLaMA-7B на 52k выходов GPT <$600; DistilBERT: −40% размера / 97% качества GLUE", True),
    ],
    "s17": [
    ...
    "s-agent-when": [
        ("1", "Anthropic — Multi-Agent Research System (spend enough tokens)", "anthropic_multiagent",
         "мульти-агент выигрывает research-задачи объёмом токенов; параллелить чтения, не решения", True),
        ("2", "Kim et al. 2026 (arXiv:2512.08296) — топология: рой vs координатор", None,
         "децентрализованный «рой» амплифицирует ошибки сильнее координатора — детали на след. слайде", True),
    ],
    "s22a_multi": [
        ...
        ("2", "Kim et al. 2026 (arXiv:2512.08296) — рой 17,2× vs координатор 4,4×", None,
         "децентрализованный «рой» равноправных агентов амплифицирует ошибки сильнее координатора", True),
        ("3", "Anthropic — Multi-Agent Research System («spend enough tokens»)", "anthropic_multiagent",
    ...
    "s-mcp-api": [
        ("1", "MCP-Universe (arXiv:2508.14704) — верхний результат 43,7%", "mcp_universe",
         "верхняя модель на реальных MCP-серверах даёт 43,7% задач; >56% реальных задач провалено", True),
        ("2", "DEV/theopslog audit 2026 — ~25% серверов реестра непригодны", None,
         "≈25% серверов реестра непригодны (пол, не потолок); injection 43% / path-traversal 82%", True),
    ],
    "s-agent-cases": [
        ("1", "McCarthy Tétrault — Air Canada (бот выдумал политику)", "aircanada_mccarthy",
         "галлюцинация политики стала юридически обязывающей; свободному агенту не давать говорить за политику"),
        ("2", "OSWorld / WebArena / ITBench — бенчмарки агентных задач", None,
         "Operator ~38% OSWorld; SRE ~14% ITBench — сузить пространство поиска, не заменить человека", True),
    ],
    "s21": [
...
ANCHORS = {
    ...
    "s-rag-hybrid": [("1", "vs BM25 0,6983 / вектор 0,6953 → ~7,4% (скромно)"),
    ...
    "s-rag-chunk2": [("1", "recursive 512-token №1; semantic дал ~43-токенные фрагменты"),
                     ("2", "Recall@k в изоляции может врать (91,9% recall при 54% ответов)")],
    "s-rag-design": [("1", "семантическое кэширование ~69% экономии LLM (vs без кэша)"),
                     ("2", "метки: recall@k ~0,8, nDCG")],
    "s-rag-chunk3": [("1", "Основной выигрыш дало не «умную» нарезку, а уважение структуры."),
                     ("2", "сменил эмбеддер → re-chunk всего корпуса, не «строчка в конфиге».")],
    "s-rag-cases": [("1", "Retrieval-дизайн выводится из природы данных, а не копируется из чужого блога."),
                    ("2", "17–33% галлюцинаций несмотря на «0%»; верификация цитат + человек-в-петле"),
                    ("3", "Cody УБРАЛ эмбеддинги, вернул лексический поиск")],
    "s-rag-research": [("1", "Рассуждение направляет поиск, находки уточняют рассуждение — закрытая петля (agentic RAG)"),
                       ("2", "фейковых цитат на запрос, потому что генерирует их кратно больше.")],
    ...
    "s14": [("1", "матчит РАСПРЕДЕЛЕНИЕ учителя"),
            ("2", "DistilBERT: −40% размера / 97% качества")],
    "s17": [("1", "гибрид — норма")],
    ...
    "s-ft-cost": [("1", "7B-LoRA <$10 — дешевле фронтир-претрейна ($61–92M) на ~7 порядков"),
                  ("2", "65B влезает в одну 48-ГБ карту")],
    "s-ft-eval": [("1", "контаминация: MMLU ~29% вопросов"),
                  ("2", "GSM8K→GSM1k −13 п.п.")],
    "s-agent-frameworks": [("1", "многие паттерны — несколько строк прямых вызовов API"),
                           ("2", "LangGraph")],
    "s-agent-when": [("1", "параллельте независимые ЧТЕНИЯ"),
                     ("2", "быстрее коллапс")],
    "s22a_multi": [("1", "стабильно на всех восьми прогонах лишь в четверти случаев."),
                   ("2,3", "17,2×")],
    "s19": [("1", "N+M")],
    "s-mcp-api": [("1", "верхний результат на MCP-Universe"),
                  ("2", "серверов реестра непригодны")],
    "s-agent-cases": [("1", "Air Canada: бот выдумал политику"),
                      ("2", "OSWorld ~38%")],
...
NOTES_ANCHORS = {
    ...
    "s14": [("1", "мягкое распределение учителя"),
            ("2", "DistilBERT")],
    "s17": [("1", "гибрид")],
    "s15": [("1", "адаптеров"),
            ("3", "98")],
    ...
    "s-agent-frameworks": [("1", "несколько строк прямых вызовов API"),
                           ("2", "stateful-граф")],
    "s-agent-when": [("1", "магии координации"),
                     ("2", "быстрее коллапс, а конкретные множители")],
    "s19": [("1", "подключение стало операцией на минуты")],
    "s-mcp-api": [("1", "сорок три и семь"),
                  ("2", "четверть серверов реестра")],
    "s-agent-cases": [("1", "Air Canada"),
                      ("2", "тридцать восемь процентов")],
    ...
    "s29": [("1", "unfaithful"),
            ("2", "learning gap")],
    "s-rag-chunk3": [("1", "уважение структуры"),
                     ("2", "пере-нарезка всего корпуса")],
    "s-rag-cases": [("1", "проваливает ретрив"),
                    ("2", "семнадцать-тридцать три процента галлюцинаций"),
                    ("3", "вернулся к классическому лексическому")],
    "s-rag-research": [("1", "агентный цикл RAG"),
                       ("2", "хуже обычного поиска")],
}

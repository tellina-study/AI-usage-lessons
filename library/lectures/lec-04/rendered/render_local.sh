#!/bin/bash
set -e
REND=/tmp/lec04-block5-closing/library/lectures/lec-04/rendered
...
OUT=/tmp/claude-999/lec04-b5-snap
rm -rf "$OUT"; mkdir -p "$OUT" "$REND/snapshots"
timeout 300 $SOFF --headless -env:UserInstallation=file:///tmp/claude-999/loprofile_lec04b5 \

...
s04 задаёт; 5 точек возврата (s08/s13/s17/s23/s27 — «где человек обязателен»); payoff s29–s31 (матрица уровней + «когда не/опасно» + чек-лист).

### 2.2 Арка (32 слайда, 75 мин) — несущая ось A→D

| Раздел | Слайды | Бюджет* | Функция |
|---|---|---|---|
| 0. Открытие+recap Л3+ЦВ | s01–s04 | 8 | Hook (GATE0-Q1); cover+roadmap; s03 **мэппинг Л3-лестница→A–D**; ЦВ+disclaimer |
| 1. A+B автодоп/мелкие | s05–s09 | 11 | A (автодоп +55%лаб/+7–22%поле); B (чат); единый паттерн A/B; 70/80%-проблема; «почти правильный» (66%) |
...
- **s03 Recap Л3 → A→D (2, `comparison`)** — **явная таблица-мэппинг**: 6 ступеней лестницы сложности Л3 (код→промпт→RAG→workflow→агент→multi-agent) → 4 уровня A–D разработки. Тезис §1.1 виден.
- **s04 Центральный вопрос (2.5, `assertion_visual`)** — ЦВ крупно + 5 «где человек обязателен» якоря. **Disclaimer:** «лестница A–D — карта лекции (course-scaffold), не отраслевой стандарт; каждый уровень разберём».

**Раздел 1 — A+B (11):**
- **s05 Единый паттерн уровня (1.5, `assertion_visual`)** — рамка для всех A→D: «что делает AI / кто решает / где человек обязателен / типичный риск». Применяется на s06–s17.
- **s06 Уровень A: автодополнение (2.5, `case_study`)** — Copilot-класс; +55% лаб (CI,p=.0017)/+7–22% поле; человек=постоянный фильтр; риск автопринятия (клоны/уязвимости).
- **s07 Уровень B: мелкие задачи чат/inline (2, `assertion_visual`)** — генерит функцию/фикс; человек ставит+ревьюит. Явная граница A↔B (A=строка-в-потоке/человек-в-цикле каждый токен; B=задача-фрагмент/человек-ревью-после).
...
**Slides honest recompute (partial→out, Решение #78):** solid IN — s08, s12, s13, s16, s17, s21, s22, s23, s24, s29, s30 = **11/32 ≈ 34%**; усиление до ~42% — s25 (vibe-coding-критерий явный), s26 (DORA-провал «amplifies слабость» как failure-якорь), s27 (solo single-point-of-failure) → +3 = 14/32 ≈ 44%. Минутная доля ≈ **~45%** (честная метрика). Partial→out: s01(hook), s05–s07/s11/s15 (калибровка уровней), s28(docs частично), s31(payoff), s32(мост).
...
**Forbidden:** код>3 строк на слайде; vendor/benchmark-числа как незыблемые (→`[VFY]`/notes); CVE-номера на видимом слое (→chapter); «Лектору»/«Вы здесь»/тайминг/subtitle-инициатива; slide add/del без request (REPORT); локальный binding в chapter; «магия/10× инженер» хайп; deep SDK/IDE-UI; полная математика SWE-bench/METR.

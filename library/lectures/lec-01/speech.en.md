...
- **[WPM cleanup]** On recount after the edits, 5 fragments >95 WPM were found (s05a 194→80, s07a 126→88, s12 113→92, s19a 119→79, s22 129→83 words/min) — all tightened without loss of content, no fragment exceeds 95 WPM.
- **[Pacing]** Active speech 60.5 → 61.5 min (+1.0 from the 2 new 0.5-min dividers s05c/s07a). Buffer 14.5 → 13.5 min. Total of 75 min preserved. Section 1 "What is AI" 6.5→7.0 min, section 2 "Where we are now" 6→6.5 min (sync with deck.yaml pacing).
...
- **[s12/s13]** Reconciled against outdated visual details (removed gold highlight on YOLO, labels under the quadrant points) — there were no spoken mentions of these details in the speech, no edits required.
- **[s15–s19a]** Added mentions of the eyebrow headings in sync with the new visual; the phrase "Not magic" at the very end of s16 was removed (replaced by a neutral ending); "Returning to..." was removed from s17.
...
- **[Pacing]** Active speech 62.5 → 60.5 min (recomputed from the sum of duration_min of all 34 slides in deck.yaml v3.3). Buffer 12.5 → 14.5 min. Total of 75 min preserved.
...
- **[Fix-A]** [s07] — removed the repeat of "AI Effect" (it stays in [s06]). Expanded the fragment on Vaswani-2017: 8 co-authors by name, self-attention replaced RNN/LSTM, as of May 2026 — more than 160 thousand citations in Google Scholar.
- **[Fix-B]** [s09] — the 4 episodes were rewritten for the new layout of slide v3.2: Mistral 7B → DeepSeek R1 → OpenClaw → Kimi K2.5. Llama-3 and MCP removed. Added: OpenClaw (P. Steinberger, November 2025, >100K★ in a quarter, February 14, 2026 — move to OpenAI and a non-profit foundation) and Kimi K2.5 (Moonshot AI, January 2026, swarm mode with 100 sub-agents).
- **[Fix-C]** [s16] — rewritten for the new visual: dialog cycle (USER ↔ Message ↔ LLM ↔ Response ↔ USER + system prompt on top + ⋮ at the bottom), rather than a 6-step linear flow. The same tone: "we remove the magic", two consequences — the lever of the system prompt and the limitation of the context window.
- **[Fix-D]** [s17] — added a production disclaimer (~30 sec): "pure chats are almost never used in production — everywhere there are agents with RAG for long-term memory and search over the corporate knowledge base; the boundary 'chat vs agent' is blurred on the production side".
- **[Fix-E]** [s19] split — now 2 blocks. [s19] = "Agent at work: 200 PDFs" — 7 numbered steps with an explicit tool at each (file system / PDF reader / extraction / vector DB / search+LLM / Sheets API / orchestrator loop). NEW [s19a] = "Levels of autonomy + Human-in/on/out-of-the-loop" — 5 levels of Feng/McDonald/Zhang 2025 in parallel with 4 frames for including the human in the loop. Total time for the 2 slides — 3 min (was 2.5).
- **[Fix-F]** [s21] — updated the description of the visual for the new layout: Q1 on the left scale (vertically), Q2 on the bottom scale (horizontally) — as axis markers of the quadrant, not as cards on top. Worked examples preserved.
- **[Fix-G]** [s28] — the assignment was simplified: the phrase "defense before the group" was removed, leaving "bring → run through the 2-question quadrant → a one-page analysis in any format".
- **[Fix-H]** [s29] — updated the spoken description of the modules for the new structure of chapter §5.2: M1 = lectures 1-5+7 (6 lectures: introduction / architecture / agents+RAG / software / finance and retail / medicine); M2 = lectures 6, 9-12 (5 lectures: engineering design / aerospace / agriculture / manufacturing / digital twins); M3 = lectures 8, 13-17 (6 lectures: creative / logistics / telecom+cybersecurity / science / oil-and-gas / synthesis). Midterm 1 on S8, midterm 2 on S12, midterm 3 on S17.
- **[Fix-I]** [s30] — removed the callback to YOLO/s01 ("the camera in s01 recognized in 30 ms — we'll break down how in lecture 2"). What remains is clean: 4 concepts + a 1-phrase frame.
- **[Pacing]** Active speech 62.0 → 62.5 min (+0.5 from the s19 split). Buffer 13 → 12.5 min. Total of 75 min preserved.
...
- **[P0-1]** Removed the orphan pre-flight "[s26 ARC-AGI]" (s26 is now = "AGI forecasts: 4 speakers, 4 material interests", the ARC-AGI slide was removed in v3.1). Replaced with a pre-flight for the current AGI quotes.
- **[P1-1]** "Robot application" → "Application in automation mode" (sync chapter §3.6/§3.7 + s21 speaker notes + speech [s21]).
- **[P1-2]** Added bridge phrases "Section N of 5" in [s10/s22/s27] (continuous carry-over from the lecture map s02a).
- **[P1-3]** deck.yaml s13 axis labels updated (X=Delegation from the user, Y=Developer's control — sync with Fix-16).
- **[P1-4]** s09 Mistral founders "Meta and DeepMind" → "Meta and Google DeepMind" (to avoid confusion with pre-2014 DeepMind).
- **[P1-5]** Pearl reference removed from s28 speaker notes (orphan reference: the Pearl slide was removed, it is absent from the speech).
- **[P1-6]** s09 Llama-3 MMLU without rounding: "seventy-nine and a half versus sixty-eight point nine" (was "sixty-nine").
- **[P1-7]** s17 "Le Chat" → "Mistral Le Chat" (with the brand prefix for consistency with the chapter and notes).
...
- **[s24 pre-flight for hallucinations]** The day before the lecture — run a fresh hallucination test: ask ChatGPT and Claude to name three 2024–2025 articles with a DOI on a narrow topic ("seismic resistance of small-diameter underground pipelines"). Record the three invented DOIs. If today both chats give only real links — switch to the backup story "how we caught a hallucination in GPT-4 on a corporate case".
- **[s26 pre-flight for AGI forecasts]** Open fresh AI news (June–August 2026) and check whether new public statements by Altman / Amodei / Hassabis / LeCun about AGI timing have appeared. If so — replace the spoken quote with a current one, keeping the structure of the "speaker — affiliation — forecast — material interest" table.
...
[Minimize the camera, move to s00a.]
...
[third group, gold point on 2017]
...
[first point — Model in the bottom-left]
...
[second point — Chat in the center]
...
[third point — Agent in the top-right, gold]
...
[Now I'll show all three in parallel — three windows on the projector. If the demo exceeds two minutes — switch to screenshots and summarize in words.]
...
[staircase on the left, bottom to top]

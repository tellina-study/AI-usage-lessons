...
- s06: после worked-example с яблоками выдержать паузу перед переходом к faithfulness— теперь это один слитный фрагмент, следить за темпом, слайд плотный.
...
Полная пересборка под новую структуру Лекции 3. Старая речь использована как стилистическая база (тон, риторические приёмы, обращения к аудитории); контент и разбивка на фрагменты пересобраны целиком под новую деку.
...
- **Раздел 1**— добавлены s05a (роли в промпте, миф «персона=точность» опровергнут через Zheng et al. 2024 + arXiv:2605.29420) и s05b (структура/разделители, параллель со structured output и prompt injection); s06 объединил старые CoT-worked-example и faithfulness-limit в один фрагмент с одним timing-блоком; добавлен s08a (чек-лист 8 пунктов).

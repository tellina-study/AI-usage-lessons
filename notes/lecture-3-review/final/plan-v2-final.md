...
**Slide count: 36** = s01–s30 monotonic + **6 suffix/append: s04a, s13a, s13b, s23a, s25a, s31** (owner-структурная ревизия после GATE-A QA, 2026-05-16). Suffix-ID схема (как lec-07 s05b/s08a/s13a) — намеренно, чтобы НЕ перенумеровывать s01–s30 и НЕ ломать `[for-slide-sNN]` маркеры главы (cascade-safe: глава не правится, новые слайды мапятся на существующие §).
...
**USER структурная обратная связь после GATE-A QA (2026-05-16) → deck v2→v3:**
- [U-1] Перед Разделом 1 «Промпт» не хватает слайда-заголовка → **+s04a** divider.
- [U-2] Нужен слайд-ОПРЕДЕЛЕНИЕ fine-tuning ДО его критики/применимости → **+s13b** «Что такое fine-tuning» (после divider s13a, перед s14).
- [U-3] Перед Разделом 3 «Fine-tune» не хватает слайда-заголовка → **+s13a** divider.
- [U-4] В кейсах (особенно в speaker notes) не хватает референсов — времени, места, названия компании → обогатить notes конкретными атрибуциями из finalized chapter (book-first; fact-checker уже verified): Air Canada (BC CRT 14.02.2024), $4,200 loop (постмортем S. Jain, 2026-04), reliability compounding (MindStudio 2025-26), forgetting (Luo et al. arXiv:2308.08747, 2023), NYT v. OpenAI (May 2025, Bloomberg Law), ZDR (Anthropic live-doc 2026), GitHub MCP heist (May 2025, Invariant Labs/Docker), Barnett et al. RAG (arXiv:2401.05856).
- [U-5] Подзаголовок-divider для **Безопасности** (внутри Раздела 4, перед s24) → **+s23a**; и для **итогового обобщения** (Раздел 5) → **+s25a** divider «Как выбрать: фреймворк решения».
- [U-6] НЕ выводить ЦЕЛЬ/функцию слайда в title (как s30 «Мост к отраслям»). Ретайтл s30 на контент, аудит прочих title на function-as-title. (Section-divider'ы, называющие тему раздела — норма канона lec-02/04, не нарушение U-6.)
- [U-7] Отдельный последний слайд **Q&A** → **+s31** (выделен из s30; s30 = мост+ДЗ контентным заголовком, s31 = Q&A, последний).
- [U-8] Сторителлинг-проход по всей колоде (нарративные мосты на divider'ах, точки возврата ЦВ, переходы).
- [U-9] deck.yaml >600 строк (с +6 слайдами тем более) → **split** `deck.yaml`+`deck-part2.yaml` (или по секциям) с кросс-ссылками + обновить loader в build script (владелец ранее: «если deck не укладывается в 600 — делай несколько»).
...
Задаётся s04; возвращается 5 раз — «когда НЕ» (s08/s12/s17/s22) + s23 (5-я точка, провалы агентов) — и закрывается payoff s26–s28. [D2-fix Phase 10: 5 точек, не 4 — синхрон с chapter §1.5…§4.5 + speech.]

### 2.2 Арка (36 слайдов, 75 мин)
...
| **div** | **s04a** | 0.3 | Divider Раздел 1 «Промпт и его границы» (нарративный мост) |
...
| div | s09 | 0.3 | Divider Раздел 2 «RAG» |
| 2. RAG | s10–s13 | 12 | Принцип (retrieval поверх Л2-эмбеддингов + LLM); когда; **когда НЕ**; провал на масштабе + Air Canada как grounding |
| **div** | **s13a** | 0.3 | Divider Раздел 3 «Fine-tune vs промпт vs RAG» (нарративный мост) |
| 3. Fine-tune vs промпт vs RAG | **s13b**, s14–s17 | 9 | **s13b: что такое fine-tuning (определение)** → FT не «ушёл» — сузился; PEFT; провал catastrophic forgetting; критерии (гибрид=норма) |
| div | s18 | 0.3 | Divider Раздел 4 «API · tools · MCP · агенты» |
| 4. API·tools·MCP·агенты | s19–s23 | 13 | API-слой; MCP; agent loop plan→act→check→iterate; workflow vs agent; провалы агентов |
| **sub-div** | **s23a** | 0.3 | Sub-divider «Безопасность: кто видит данные в цепочке» |
| 4-sec. Безопасность | s24–s25 | 6 | Данные/retention (NYT/ZDR); атаки через инструменты (GitHub MCP heist/tool poisoning) |
| **div** | **s25a** | 0.3 | Divider Раздел 5 «Как выбрать: фреймворк решения» (итоговое обобщение) |
| 5. Фреймворк решения | s26–s29 | 11 | Лестница; матрица+когда-НЕ-ИИ; чек-лист (LO7 payoff)+LO4 apply; человек-валидатор+NANDA |
| Финал | s30 | 1.5 | Рамка как основа отраслевых лекций + ДЗ Семинар 3 (контентный title, U-6) |
| **Q&A** | **s31** | ≤5 буфер | Отдельный финальный слайд Q&A (U-7) |
...
|---|---|---|---|---|
| **LO7** | Обосновать выбор архитектуры AI (чат/агент/RAG/API/модель) для задачи | Лестница+матрица+критерии «когда НЕ»+чек-лист; каждый раздел даёт «когда НЕ применять» | s04,s08,s12,s17,s22,s23,**s26–s28** | Студент по задаче называет архитектуру И ≥2 причины по осям матрицы И ≥1 «когда это было бы НЕ так» |
...
## 4. Slide list (36: s01–s30 + s04a/s13a/s13b/s23a/s25a/s31)

> Divider-слайды: тип `section_divider`, ~0.3 мин, roadmap-бар + gold-маркер активного раздела + 1 нарративная строка-мост (НЕ function-as-title). Все — **partial→out** из strict-in (структурные, не failure/judgment).
...
- **s04a Divider Раздел 1 (0.3, `section_divider`)** [U-1] — «Раздел 1. Промпт и его границы»; нарративный мост: «прежде чем усложнять — что умеет один вызов и где его потолок». roadmap gold-маркер Раздел 1.
...
- **s13a Divider Раздел 3 (0.3, `section_divider`)** [U-3] — «Раздел 3. Fine-tune vs промпт vs RAG»; нарративный мост: «знание мы решили через RAG — а если проблема не в знании, а в поведении модели?». roadmap gold-маркер Раздел 3.

**Раздел 3 (9 мин):**
- **s13b Что такое fine-tuning (1.5, `assertion_visual`)** [U-2] — ОПРЕДЕЛЕНИЕ до критики: «fine-tuning (дообучение) — продолжение обучения готовой модели на своих данных; меняет ВЕСА (≠ промпт/RAG, что меняют контекст)». Мини-схема: предобученная модель + ваш датасет → дообученные веса. Закрывает gap пререквизита на видимом слое (раньше был только inline в s14). *Partial→out (определение, не failure/judgment).*
- **s14 «Fine-tuning ушёл»? — нет, сузился (2.5, `assertion_visual`)** — опирается на определение s13b. 2026: FT = поведение/стиль/формат/политика/дистилляция, НЕ знания. *Partial→out (калибровка области, не провал/критерий).*
...
- **s23a Sub-divider Безопасность (0.3, `section_divider`)** [U-5a] — «Безопасность: кто видит данные в цепочке»; нарративный мост: «агент действует через инструменты и внешние API — теперь вопрос не "работает ли", а "кто видит ваши данные и кто может прислать команду"». *Partial→out.*
...
- **s25a Divider Раздел 5 (0.3, `section_divider`)** [U-5b] — «Как выбрать: фреймворк решения» (итоговое обобщение); нарративный мост: «разобрали все архитектуры и их провалы — соберём в инструмент выбора». roadmap gold-маркер Раздел 5. *Partial→out.*
...
- **s30 «AI-архитектура в отраслевых лекциях» + ДЗ (1.5, `summary`)** [U-6 ретайтл: контент, не функция «мост»] — «в каждой индустрии Лекций 4–17 будет тот же вопрос: какая архитектура и почему». ДЗ = Семинар 3 (архитектурный выбор, 3 кейса). НЕ совмещать с Q&A.
- **s31 Q&A (≤5 буфер, `qa_minimal`)** [U-7] — отдельный финальный слайд: «Вопросы». Минимальный, как dedicated Q&A в lec-02. Title — «Вопросы» (контент, не «Q&A-слайд»).
...
s07, s08, s12, s13, s16, s17, s22, s23, s24, s25, s27, s28 = **12 / 36 ≈ 33% по слайдам**; **по минутам ≈ 30/70 ≈ ~43%** (divider'ы по 0.3 мин почти не двигают минутную долю — она и есть честная метрика; слайдо-доля просела из-за +6 структурных слайдов, но ≥30% сохраняется). chapter strict-in ~58% (не затронут — глава не правится). Holistic ≥30% в каждом артефакте удержан.
**Partial → OUT (честно НЕ засчитаны):** s01 (hook), s14 (калибровка FT), s26 (лестница-payoff), s29 (mixed) + **6 структурных: s04a, s13a, s13b, s23a, s25a, s31** (divider/определение/Q&A — не failure/judgment).

...
def s14_aurora(p):
    ...
    footer(slide, "Паттерн выживания: поэтапный ввод (ползком-шагом-бегом) · узкий ODD · не переобещание")
...
def s19_survivor_consolidation(p):
    ...
    text_box(slide, 0.7, 6.6, 11.9, 0.45,
             "Паттерн выживания: поэтапный ввод (ползком-шагом-бегом) · узкий ODD · не переобещание · терпеливый капитал · уважение к среде.",
...
def s21_highway_failure_matrix(p):
    ...
    rows = [
        ...
        ("SPAC IPO пузырь\n2021-2022", "Embark 16 месяцев SPAC→банкротство\nTuSimple early days",
         "Не вкладывать в до выручки SPAC merger без 5+ лет коммерческий history. Публичный надзор убьёт ажиотажа."),
...
def s22_starsky_quote(p):
    ...
    # Three lessons
    lessons = [
        ("ML не оправдывает ожиданий", "Каждый новый краевой случай требует размеченных данных. На до выручки масштаб денег не хватает."),
...
def s24_waymo(p):
    ...
    footer(slide, "Паттерн выживания: поэтапный ввод (ползком-шагом-бегом) · узкий ODD · не переобещание · терпеливый капитал Alphabet")
...
def s37_trucker_shortage_structural(p):
    ...
    set_slide_bg(slide, WHITE)
    text_box(slide, 0.5, 0.4, 12.33, 1.2,
             "Дефицит дальнобойщиков — структурная трудовой политика проблема, не AI.\nРешается политикой, переподготовкой, условиями труда.",
             size=22, bold=True, color=DEEP, line_spacing=1.1)
    # Left: цифры

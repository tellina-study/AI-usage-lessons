...
ANCHORS = {
    ...
    "s06": [("1", "23 − 7 = 16"),
            ("2", "the real cause of the answer")],
    "s08": [("1,2", "a new term, not a new entity"),
            ("3", "this is an engineering requirement, not aesthetics")],
    "s10": [("1", "is a correct answer from a RAG system")],
    "s11": [("1", "the signal on the features below is strong"),
            ("2", "One feature is a reason to take a look")],
    "s12": [("1", "RAG is redundant if ANY of the three holds"),
            ("2", "A fixed policy / value")],
    "s13": [("1,2", "RAG has no"),
            ("3", "Air Canada — architecture walkthrough")],
    "s13b": [("1", "change the model itself")],
    "s14": [("1", "Fine-tuning changes the model's behavior"),
            ("2,3", "first fine-tune the teacher for the task, then distill into the s")],
    "s15": [("1,2", "low-rank adapter matrices; QLoRA — the same on top of a quantized "),
            ("3", "of PEFT-tagged models are LoRA")],
    "s16": [("1", "degradation of the model")],
    "s19": [("1", "N+M")],
    "s21": [("1", "works in a loop, determining the sequence of steps itself"),
            ("2", "validation against an EXTERNAL criterion — not the model")],
    "s22": [("1", "Predictable task → workflow"),
            ("2", "the value justifies a multiple-fold increase")],
    "s22b": [("1", "Five typical slots")],
    "s22c": [("1", "The spectrum runs from a flat file to graph knowledge")],
    "s22d": [("1", "sometimes — dramatically not")],
    "s22e": [("1", "the mere presence of an instruction file gives NO significant gain"),
             ("2", "did NOT prevent its recurrence 25 days later")],
    "s23": [("1", "$4,200 in 63 hours"),
            ("2", "fewer transitions + a check between steps"),
            ("3", "parallel subagents make conflicting")],
    "s25": [("1", "dumped private repositories into a public PR"),
            ("2,3", "a surface appears that a single call never had"),
            ("4,5", "third-party services/MCP outside ZDR")],
    "s25b": [("1", "which equipment slots are filled and where the agent physically lives")],
    "s26": [("1", "Every climb is a TRADE-OFF")],
    "s27": [("1", "stop at the first question that fires"),
            ("2", "if the task is deterministic and verifiable — plain code, STOP here")],
    "s29": [("1", "not by the plausibility of the reasoning"),
            ("2", "the root is in the learning gap and the integration failure, not model quality")],
}

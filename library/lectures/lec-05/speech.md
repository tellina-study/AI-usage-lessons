...
issue: 100
changelog:
  - "v1 (2026-05-17): первый draft Phase 9 — устная развёртка chapter v2 по 33 слайдам deck v2. ЦВ символьно из §0.3/s04; 5 точек возврата (s09/s14/s18/s23/s28); failure-нить Zillow→fraud-FP→Apple Card→Air Canada/Klarna→Wendy's, Knight = callback; 5 divider bridge phrases; pre-flight verify-day-of по cbr/Сбер/X5/vendor."
  - "v1 (2026-05-17, same-session WPM/strip/inclusive hardening): mandatory trim-pass — 16 фрагментов >95 WPM (P0 в первом проходе) сжаты до ВСЕХ ≤95 (max s28=94.7); inline `[FACT-CHECK]`/`[VERIFY-DAY-OF]`-cue удалены из тела (strip 0/0/0); «мы с вами» доведён до 10, распределён по всем 7 разделам; «онбординг»→«подключение клиента» (forbidden anglicism)."
---

# Речь лектора · Лекция 5 «AI в финансовом секторе и ритейле»

**Длительность:** 75 мин (~70 активная + ~5 буфер Q&A).
**Версия:** v1 (Phase 9 draft).
...
Модель прогнозировала цену дома, и по прогнозу Zillow автоматически скупала жильё. В 2020–2021 рынок пережил пандемийный шок. Модель, обученная на стабильном рынке, систематически переоценивала. Полмиллиарда потерь.

Введём термин с нуля. **Дрейф распределения** — когда данные в реальности перестают быть похожими на обучающие. Одной фразой: модель училась на одном мире, а работает в другом.

[пауза] Но дрейф сам по себе компанию не разоряет — модели дрейфуют постоянно. Разорило то, к чему подключён выход. Ключевое понятие: **асимметрия цены ошибки**. Та же ошибка в рекомендации стоит около нуля; в автопокупке дома — десятки тысяч долларов, необратимо.

[понизить голос] Короткий callback. Knight Capital в 2012 за сорок пять минут потеряла четыреста сорок миллионов — автоматика выставляла заявки без контроля. Обычный алгоритм, не ML. Но класс ошибки тот же: автоматика необратимого без аварийного выключателя.
...
«Центральный вопрос возвращается второй раз. Посмотрим на провал-сторону — она важнее рекламных цифр.

Возьмём оптимистичную картину: ложных срабатываний доли процента, одобрение выше девяноста девяти. Кажется отлично. Но умножьте на масштаб. Миллиарды транзакций в год — даже половина процента это десятки миллионов заблокированных честных операций. За каждой конкретный человек.

Почему это нельзя убрать настройкой? Работает компромисс — **точность против полноты**. Разведём две идеи. Стоимостная оценка — про то, по какой цене выбирать точку срабатывания. Компромисс точность-полнота — про то, что сам компромисс неизбежен. Подозрительнее модель — больше заблокированных честных. Терпимее — больше пропущенного фрода. Лучшая модель сдвигает кривую, но не отменяет выбор точки на ней.
...
Кейс первый — Air Canada. Это callback Лекции 3, не дубль. Чат-бот авиакомпании сообщил пассажиру несуществующую политику возврата. Пассажир действовал на основании ответа. В феврале 2024 канадский трибунал постановил: компания отвечает за информацию своего чат-бота, и обязал выплатить компенсацию. Класс ошибки: галлюцинация финансово значимого факта равна юридической ответственности организации. Перенос в банк прямой: чат-бот, назвавший неверную ставку, создаёт такую же ответственность.

[пауза] Кейс второй — Klarna, дуга 2023 до 2025. Финтех развернул языкового ассистента поддержки. Он закрывал около двух третей обращений, сократил время решения с примерно одиннадцати минут до менее двух. Заявленная экономия — около сорока миллионов долларов в год. Это подавалось как «ИИ заменяет поддержку».

[понизить голос] Развязка. К середине 2025 Klarna зафиксировала падение удовлетворённости клиентов и вернулась к найму людей. По сообщениям СМИ, генеральный директор признал, что ставка на снижение издержек дала более низкое качество.

Почему это не «ИИ плох», а тоньше. Ассистент работал — экономия реальна. Провал в категориальной ошибке: задачу «отвечать на обращения» спутали с системой «клиентский сервис». Сервис — это ещё и редкие, эмоционально заряженные, спорные случаи, где на кону доверие. Их мало, но именно по ним клиент решает — остаться или уйти. Полная замена оптимизировала среднюю стоимость контакта и обрушила тяжёлый хвост. Тот же класс, что стоимостная оценка из раздела про фрод.
...
Почему это чистая иллюстрация, а не «неудачный пиар». Технически surge-цена — решаемая оптимизационная задача. Поставите задачу так — оптимизатор честно решит и приведёт ровно к тому, что вызвало бойкот. Ошибка не в модели, а в постановке: «справедливость» и «лояльность» неявно отброшены, потому что плохо измеримы, а измеримая выручка входила в функцию.

[понизить голос] И главное. Это тот же механизм, что информационный пузырь. Там прокси — клик, цель — долгосрочная ценность. Здесь прокси — выручка за смену, цель — доверие к бренду на годы. В обоих случаях патология не выглядит как сбой: метрика растёт, провал виден только если смотреть на то, что в метрику не попало. У прогноза и скоринга провал громкий, у рекомендаций — тихий.
...
[обращение к залу] Тридцать секунд про себя: где в знакомом сервисе вы замечали информационный пузырь или несправедливую цену — что там прокси, а что цель? [пауза 30 сек] Это пятая, замыкающая точка возврата, и здесь мы с вами видим главное: даже безобидный тип ИИ требует человека и рамки вокруг себя.»
...
«Раздел шестой, без дивайдера — это сборка. Соберём с вами пять разделов в один аппарат — ответ на центральный вопрос.

Матрица. Строка — задача, колонки — правильный тип, почему именно он, типичный провал. Это компактная упаковка пяти уже доказанных фактов, не новый материал. Пройдём по строкам.
...
Поддержка и объяснение — языковая модель с grounding. Почему именно она: текстово-диалоговая задача — ровно для LLM. Провал — галлюцинация финфакта как юридическая ответственность, замена не равна augmentation. Критерий — grounding, проверяемость, путь к человеку.
...
«Сворачиваем пять разделов в чек-лист — применять до выбора ИИ. Восемь вопросов: какой тип диктует структура задачи; почему не LLM здесь; можно ли вообще без ИИ; обратимо ли действие на выходе; как проверить факт; регулируемо ли решение; кто отвечает; есть ли персональные данные или биометрия?

[пауза] Этот аппарат — линза для всех отраслевых лекций дальше. Парный Семинар 5 доводит навык до практики: команды проверяют пять заявлений о ИИ в банках по первоисточникам Банка России и ВЦИОМ. Граница строгая: лекция учит распознавать класс и называть принцип, семинар — самостоятельно верифицировать.

[понизить голос] И последнее. ИИ здесь даёт огромную измеримую ценность — X5, Сбер, Stripe. И ошибочное суждение стоит полмиллиарда или доверия миллионов. Между восторгом и отрицанием — инженер, который умеет сказать: какой тип, почему он, где ломается и кто отвечает. Этот навык, который мы с вами собрали, не устаревает с выходом следующей модели.
...
**Word count:** ~5800 слов произносимого текста с pacing-cue-маркерами; ~5650 при stripped-cue подсчёте (frontmatter, pre-flight, самооценка не считаются). В диапазоне 4.5–6k. ✓

**Pacing / WPM (hard cap ≤95, target 70–80) — PROVEN PASS:** сумма slide-таймингов произносимых фрагментов = 70.0 мин (по deck totals) + ~5 буфер Q&A = 75 мин. Per-fragment WPM пересчитан скриптом (слова в `« »` минус `[cue]`-вставки / duration_min): **ВСЕ 33 фрагмента ≤95**, максимум = s28 (94.7 WPM). Первый draft имел 16 фрагментов >95 (P0); проведён mandatory trim-pass — filler удалён, предложения упрощены, критерии/точки возврата/альтернативы/аналогии сохранены. Граничные после trim: s09 (~95), s28 (94.7), s32 (~94) — в пределах cap, Phase 10 рекомендуется прогнать секундомером (preflight уже инструктирует лектора). Зеро-tolerance соблюдён: ни один fragment не >95.

**ЦВ символьно идентичен:** центральный вопрос на s04 произносится дословно из chapter §0.3 / deck s04: «Финансы и ритейл — отрасли максимального внедрения ИИ. Под какую задачу — какой тип ИИ, почему именно он, а не LLM везде, и где этот тип ломается?» ✓

**5 точек возврата ЦВ:** ✓ s09 («возвращается первый раз»), s14 («второй раз»), s18 («третий раз»), s23 («четвёртый раз»), s28 («пятый, последний раз»). Все названы живым языком, без §-кодов.

**Сквозная failure-нить:** Zillow (s01 hook → s09 payoff) → fraud false positives (s14) → Apple Card (s18) → Air Canada + Klarna (s23) → Wendy's (s28). Knight Capital — только callback (s09, s14, упомянут в критерии s19). Just Walk Out — иллюстрация s31. ✓

**Аналогии проговорены живо:** товаровед (s08), облако точек + выброс (s11), кредитный инспектор показывает расчёт построчно (s16), убрал графу «пол» но десяток коррелирующих граф (s18), студент со справочником = grounding (s22), «спроси похожих» vs «похожее по описанию» (s26), пароль сменить можно — лицо нет (s31). ✓

**5 divider bridge phrases:** ✓ «Раздел первый/второй/третий/четвёртый/пятый из пяти» на s04a, s10, s15, s20, s25. Раздел 0 (открытие) и Раздел 6 (сборка) без дивайдера — по deck.yaml (5 дивайдеров, plan §2.2).

**Forward-pointer'ы правильного направления:** ✓ Л7 §2.2 sensitivity/specificity (s13: «построим в Лекции 7»), Obermeyer Л7 §4.3 (s18: «увидим в Лекции 7»), bias CV Л7 §2.5 (s31: «углубим в Лекции 7»). Семинар 5 = Apply (s22, s32: «лекция учит распознавать, семинар — самостоятельно верифицировать»). Все — «введём здесь, углубите/доведёте позже». ✓

**Universality:** без локального binding — «наша страна», «наша отрасль», «вы будете работать»; нет «студент ИУ6». ✓

**Anti-hype:** s32 заключение — «между восторгом и отрицанием»; «провалов много намеренно». Без «магической пилюли». ✓

**ENFORCED strip (0 в теле) — PROVEN:** скрипт-проверка произносимого тела (от `## [s01` до `## [Резерв`): `[FACT-CHECK]`=0, `[VFY]`=0, `[VERIFY-DAY-OF]`=0, LO-коды=0, `§\d.\d`=0, `→sNN`=0. Все verify-day-of / fact-check инструкции вынесены ИСКЛЮЧИТЕЛЬНО в pre-flight секцию (actionable, с URL/файлами/командами); inline-cue-брекеты удалены из тела полностью (строже lec-04 reference). РПД «>90%» — ТОЛЬКО как обучающий пример подмены базы (s21, класс 5), НЕ как факт. ✓

**Англицизмы (forbidden_anglicisms glossary):** «пайплайн» не использован (нет конвейерных контекстов; «конвейер» употреблён в прямом смысле s17/s19); «фоллбэк» → «запасной вариант» (pre-flight, Q&A backup); «эдж-кейс»/«краевой случай» — не понадобился; «инсайт» не использован. Расширенный blacklist (стейкс/оверран/онбординг/юзкейс и т.д.) — grep-чисто. Whitelist-термины сохранены как канон: AI/LLM/RAG/grounding/KYC/liveness/AML/SHAP/ML/CV/augmentation/iBuying. ✓

**Glossary-консистентность:** канон-термины строго по glossary.yaml — «прогнозирование временных рядов», «дрейф распределения», «асимметрия цены ошибки», «поиск аномалий», «ошибка первого/второго рода», «матрица ошибок», «стоимостно-взвешенная оценка», «точность ↔ полнота», «кредитный скоринг», «коды причин» (reason codes), «прокси-предвзятость», «коллаборативная/контентная фильтрация», «холодный старт», «смещение к популярному», «чрезмерная специализация», «гибрид», «информационный пузырь», «динамическое ценообразование», «прокси≠цель», «grounding», «KYC», «liveness», «скрытый человеческий труд», «аварийный выключатель», «augmentation vs замена», «ПДн/закон о персональных данных». «единый патторн карточки» подан как course-scaffold (teach-not-defend, s04). ✓

**0 orphan refs:** все 33 слайда покрыты (s01–s32 + s04a), порядок по deck.yaml/deck-part2.yaml; слайды НЕ удалялись (LOCKED=33). ✓

## Остаточные риски для Phase 10

1. **WPM граничные fragments (methodology/consistency).** ВСЕ 33 фрагмента ≤95 (проверено скриптом, max s28=94.7). Граничные s09 (~95), s28 (94.7), s32 (~94) — Phase 10 рекомендуется прогнать секундомером для запаса; при реальном превышении снять одно пояснительное предложение, НЕ резать критерий/точку возврата/альтернативу (preflight уже инструктирует лектора). Зеро-tolerance соблюдён.
2. **Fact-checker.** Все числа сверены с chapter v2 (Zillow $300M/кв + $500M+/~2000/−25%; X5 >70%/+5 млрд ₽/−2%; Сбер ~100%/5000/+350 млрд; Stripe ~32%/>99%; JPMorgan −30%; Visa ~$40 млрд FY2023; Klarna ~$40M/2/3/−11→<2 мин; Amazon ~35%/Netflix ~75% историч.; Just Walk Out >1000). Атрибуции/даты — как в chapter. Fact-checker должен подтвердить vendor-disclaimer'ы и cbr.ru-формулировки; verify-day-of items в pre-flight синхронизированы с deck-part2 verify_day_of_items/fact_check_items.
3. **Consistency chapter↔slides↔speech.** Speech derive строго по deck-арке (book/deck-first). Один speech-фрагмент на слайд; s29 покрывает §6.1 целиком (матрица). Расхождений deck↔chapter не обнаружено — все 33 [for-slide-sNN] маркера chapter имеют соответствующий speech-фрагмент. Consistency-checker сверяет терминологию по locked glossary.yaml (terminology-only mode).
4. **strict-in устная доля.** Failure/judgment произносится развёрнуто в s06, s09, s13, s14, s16, s18, s19, s22, s23, s24, s28, s29, s30, s31 (критерий + урок + альтернатива каждый раз) — по минутам ≈ s09(2)+s13(3)+s14(3)+s16(3)+s18(3)+s19(2)+s22(3)+s23(3)+s24(2)+s28(3)+s29(3)+s30(2)+s31(3) ≈ 35 из ~55 активных контент-минут ≈ ≥30% holistic с запасом, распределено по всем разделам (Р1 s09 · Р2 s13/s14 · Р3 s16/s18/s19 · Р4 s22/s23/s24 · Р5 s28 · Р6 s29/s30/s31), не single-cluster. Methodology-critic должен подтвердить strict-in (partial→out).

Не коммитил содержимое сам — оркестратор коммитит. Speech derive не падал; расхождений deck↔chapter не обнаружено.

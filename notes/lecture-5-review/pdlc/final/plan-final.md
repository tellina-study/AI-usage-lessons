...
**GATE-0 blockers:** (1) owner-approve ✅ (2026-09-06 «давай в производство»); (2) course-curator LO3 sign-off ✅ **SIGN-OFF-WITH-NOTES** (`final/lo3-curator-signoff.md`); (3) бэкап finance/retail ✅ (`library/lectures/_archive/lec-05-finance-retail/`).

**Curator-корректировки (ENFORCED в Phase 2):**
- **LO3 «data security» facet усилить:** добавить в главе явное предложение «что безопасно передавать в сторонний AI» (PII в reference datasets, тикеты в LLMOps-трейсинг) — сейчас governance/guardrails/drift покрыты, data-security тонок.
- **Seminar naming FIX:** по seminar-shift (#152) «Семинар №5» пары с Л4. Парный семинар для Л5 — **Семинар 7** («Проверка фактов…»), сейчас finance-scoped → **re-scope под PDLC = follow-up** (не блокер производства главы). В плане/главе НЕ писать «Семинар №5» как пару Л5; писать «парный семинар (Сем7, re-scope под PDLC — follow-up)».
- **Stale cross-refs** в Л7/Л8/Л9/Л10 на старую finance-Л5 → отдельный non-blocking follow-up issue.
- **Reuse Zillow / Air Canada / Klarna** из архивной finance-Л5 под новой PDLC-рамкой — валидно; пометить fact-checker/consistency-checker (не случайный дубль).
...
**Phase-6 owner additions (2026-09-06, «дожимай сам»):** (1) заголовки/лейблы фаз — **на русском** (Исследование/Дизайн/Сборка и запуск/Измерение/Поддержка/Управление); (2) **реальные мемы** — на обложке + каждом дивайдере + 1-2 контент-слайда в каждом разделе (метод как в lec-02 `gen_memes`, каждый резолвится в тезис слайда); (3) **каждый раздел открывается слайдом-обзором «для чайников»** — метод/подход простым языком ДО классической базы (+6 слайдов: s07b/s14b/s21b/s28b/s36b/s44b, cascade-safe). Итого ~56 слайдов.
...
## 3. Learning Outcomes (Bloom Лекция ↔ парный семинар (Сем7, re-scope под PDLC — follow-up))
...
- **s49 Keystone payoff + чек-лист + мост Л6/парный семинар + Q&A (2.5 + ≤8 буфер, `summary`+`qa_minimal`, hero, LO1/LO6)** — сборка бесплатна → дефицит = сигнал/шум + удержание намерения; чек-лист «прежде чем делать фазу AI-first» (классика на месте? стоит ли доверие цены? eval/reference dataset? guardrail-метрика? staged+rollback? human-escalation? кто отвечает?); мост Л6; ДЗ парный семинар (Сем7, re-scope под PDLC — follow-up). Q&A отдельным beat'ом.

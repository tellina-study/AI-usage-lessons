...
status: draft
version: v1.0
derives_from: [chapter.md v1.1, deck.yaml v1.6 (36 slides), plan-v2-final.md v2.1]
slides_covered: [s01, s02, s02a, s03, s04, s04a, s04b, s05, s06, s07, s08, s08a, s09, s09a, s10, s12, s13, s13a, s14, s15, s16, s17, s17a, s18, s19, s20, s21, s22, s22a, s23, s24, s25, s26, s27, s28, s29]
---

# Речь лектора · Лекция 2 v1.0

**Длительность:** 75 минут (62 мин активной речи по 36 слайдам + 6 мин retrieval-моментов + 2 мин на переходы + 5 мин Q&A-буфер).
**Версия:** v1.0 — первый draft на основе chapter v1.1 и deck v1.6 (36 слайдов).
...
- [ ] Открыть `huggingface.co/playground` и убедиться, что модель `Meta-Llama-3-8B-Instruct` доступна и слайдер температуры работает. Это нужно для ДЗ на s27. Если HF Playground закрыт — переключиться на запасной: `together.ai` playground или локальный Ollama с Llama 3 (есть на ноутбуке).
...
## [s01 · 2 мин] — Как модель видит ваш запрос

[Включаю первый слайд. На экране — три строки с цветными токен-чанками.]

«Здравствуйте. Лекция вторая — мы с вами разбираем, что происходит внутри большой языковой модели между запросом и ответом. И начинаем с простой, но контринтуитивной мысли.

[пауза 2 секунды]

Посмотрите на экран. Три фразы — на английском, на русском, и кусочек кода. Каждая фраза разрезана на цветные кусочки. Эти кусочки называются токенами. И когда вы пишете в ChatGPT, в Claude или в GigaChat — модель видит не ваши буквы, не ваши слова, а вот эти токены. Только их.

Английская фраза `tokenization is fascinating` режется на пять токенов. Русская `Привет, как дела?` — на шесть, хотя по символам она короче. Кусочек кода `def hello(name):` — тоже шесть, причём скобки и двоеточие — это отдельные токены.

[обратить внимание на gold callout]

Главное практическое следствие — внизу, золотом. Один и тот же смысл стоит разное число токенов. Английский почти в два раза дешевле русского. Это инженерный факт: тот же запрос в API на русском обходится примерно вдвое дороже, чем на английском.

Это первая из четырёх внутренних операций, которые мы сегодня разберём.»
...
## [s03 · 1.5 мин] — Углубляем слой «модель» из Лекции 1
...
## [s04 · 1 мин] — Главный вопрос лекции
...
Первое: к концу лекции мы с вами поймём, почему промпт с ролью работает лучше пустого. Второе: почему AI плохо считает буквы. Третье — с золотой меткой payoff: почему один и тот же запрос даёт разные ответы. Эти три «почему» Лекция 1 оставила открытыми. Сегодня закроем все три.»

[Переход на s04a.]

---

## [s04a · 30 сек] — Раздел 1: Токенизация

[На слайде — большое «Раздел 1», под ним «Токенизация».]

«Первый раздел из шести. Токенизация — как модель видит ваш текст. Пять слайдов, одиннадцать с половиной минут.»

[Переход на s04b.]

---

## [s04b · 1 мин] — Поток данных в LLM
...
Сегодня мы с вами разбираем все эти этапы по очереди. Под конвейером — четыре подкарточки: Раздел 1 — токенизация, Раздел 2 — векторы, Раздел 3 — то, что LLM делает с векторами внутри, Раздел 4 — обратное преобразование в токены через сэмплинг.»

[Переход на s05.]

---

## [s05 · 2 мин] — Токен — id из словаря модели
...
## [s08 · 2 мин] — Один и тот же текст: дороже по-русски
...
## [s08a · 30 сек] — Раздел 2: Эмбеддинги
...
## [s09a · 1.5 мин] — Пространство эмбеддингов
...
## [s10 · 3 мин] — Семантическое сходство на предложениях
...
## [s12 · 2 мин] — Эмбеддинги — фундамент понимания LLM

[На слайде — слева вертикальный pipeline, справа три карточки.]
...
## [s13 · 30 сек] — Раздел 3: Механизм внимания

[На слайде — большое «Раздел 3».]

«Третий раздел из шести. Самый плотный — восемнадцать минут, пять слайдов. Механизм внимания — как модель решает, что важно сейчас. Это центральная операция трансформера и самая важная для понимания, как работают современные модели.»

[Переход на s13a.]

---

## [s13a · 2 мин] — Внимание — это матрица

[На слайде — 7×7 attention matrix.]
...
На слайде — упрощённая семь на семь attention-матрица для предложения «Кот съел мышь, потому что она была голодна». Каждая строка и каждый столбец — это токен. Цвет ячейки кодирует вес: тёмный — высокий вес, светлый — низкий. Ячейка «она → мышь» подсвечена золотом — она сейчас имеет самый высокий вес.

[пауза]

Главное здесь — каждый токен смотрит на все остальные одновременно. Не последовательно, как читали бы мы с вами. Одновременно. Это и есть «attention is all you need» из знаменитой статьи Vaswani 2017 года.
...
## [s14 · 3 мин] — Attention — распределение весов
...
Это и есть attention. Распределение света по сцене. И это распределение меняется в зависимости от того, что мы сейчас спрашиваем.
...
Первое — на вход attention получает все токены контекста. Не один и не часть. Все. Если контекст 10 токенов — смотрит на 10. Если 100 тысяч — на 100 тысяч.
...
В реальной модели голов внимания не одна, а десятки параллельных, и слоёв десятки. Но для нашего с вами уровня понимания достаточно одной фразы: attention выдаёт распределение весов на токены контекста.»

[Переход на s15.]

---

## [s15 · 5 мин] — Рабочий пример и эффект роли
...
Когда модель доходит до токена «она», ей нужно определить, к чему этот токен относится. К мыши или к кошке. На уровне attention это видно. Над токеном «она» оказывается распределение весов. Наибольший вес — на токене «мышь»; так предложение и понимается. Средний вес — на «была». Тонкий — на «голодна». На слайде это три стрелки разной толщины.

Важная оговорка. Картинка с тремя стрелками — сильное упрощение. Реальный attention map содержит сотни связей сразу, и в каждом из десятков слоёв рисунок свой. Мы агрегируем сотни значений в одну толстую стрелку для интуиции.
...
На уровне attention второй даёт качественно другую картину. Когда модель доходит до момента генерации первого токена ответа, её внимание распределяется на весь предшествующий контекст. В первом случае контекст короткий — почти только слово «асинхронность». Модель опирается на самую общую статистику. Выдаёт обобщённый ответ.
...
Рабочее объяснение. Role-токены получают повышенный вес в attention при генерации первых токенов ответа. Роль в промпте — не просьба «поверь мне». Это явный input-сигнал, прямо влияющий на распределение внимания. Это первое из трёх «почему», и его мы только что объяснили через механизм.»

[Переход на s16.]

---

## [s16 · 2 мин] — Контекстное окно
...
Первая — стоимость растёт квадратично. Базовая версия attention требует, чтобы каждый из N токенов посмотрел на каждый. Это N в квадрате операций. Удваиваем длину контекста — учетверяется стоимость. Миллион токенов входа — в шестнадцать раз дороже, чем сто тысяч. В цене API это видно прямо.
...
Точность около 70–80% когда факт в начале. Проваливается до 30% в середине. Снова поднимается до 70–80% в конце.

[пауза]

Природа эффекта связана с тем, как модели учатся работать с длинным контекстом. В типичных документах важные утверждения находятся либо в начале — тезис, — либо в конце — вывод. Статистика положения важных токенов имеет U-форму. Модель усваивает эту статистику и переносит её на работу. Даже если важный токен лежит в середине, модель по привычке меньше его взвешивает.
...
## [s18 · 2.5 мин] — Распределение вероятностей
...
## [s20 · 2 мин] — 4 ручки API под сценарий
...
## [s21 · 2 мин] — Авторегрессионный цикл

[На слайде — замкнутый цикл из пяти шагов.]

«Соберём все четыре этапа inference в единый цикл.

[показать на цикл]

Лекция 1 описывала модель как stateless-функцию: вход — данные, выход — предсказание, между вызовами состояния нет. Но в чате с LLM мы наблюдаем длинный ответ — фразу, абзац, страницу. Откуда длинный ответ из stateless-вызовов?
...
## [s22 · 1 мин] — Локально vs в облаке
...
## [s22a · 30 сек] — Раздел 5: Финал

[На слайде — большое «Раздел 5».]

«Пятый, последний раздел. Финал. Закрываем три «почему», три кросс-сюжетных фрейма, задание к семинару, мост к Лекции 3. Девять минут.»

[Переход на s23.]

---

## [s23 · 1 мин] — Конвейер inference

[На слайде — горизонтальный pipeline из 4 стадий.]
...
Внимание — десятки слоёв attention строят распределения весов на токены контекста.
...
Первое. Почему промпт с ролью лучше пустого. Ответ: role-токены получают высокий вес в attention при генерации первых токенов ответа. Распределение смещается в сторону роли. Слайд s15.
...
## [s25 · 1.5 мин] — Когда не LLM

[На слайде — дерево решений с тремя ветками.]

«Первый из трёх кросс-сюжетных фреймов финала. Когда LLM — не правильный инструмент.
...
Вторая. Нужна интерпретируемость или регулируемость. Финансы, медицина, юридическая сфера. Классические методы с прозрачной структурой — феатуры, дерево решений, правила. LLM — чёрный ящик в смысле объяснимости отдельного предсказания.

Третья. Критическое время отклика меньше 100 миллисекунд. Real-time, антифрод, edge-устройства. Специализированная маленькая модель, а не LLM с задержкой 200-500 мс.
...
## [s26 · 1 мин] — Внимание ≠ причинность

[На слайде — Человек vs AI side-by-side.]

«Второй фрейм. Возврат к трём уровням причинности Перла из Лекции 1.
...
## [s27 · 2 мин] — Домашнее задание

[На слайде — 3-шаговая инструкция.]

«Третий кросс-сюжетный фрейм — задание к семинару 2. Тема семинара — «Эффект температуры: как стохастичность сэмплинга меняет ответ LLM».

[показать на три шага]

Шаг первый. Возьмите типовую задачу из своей предметной области. Что-то, что вы уже спрашиваете у LLM или хотели бы автоматизировать. Это должен быть конкретный, воспроизводимый запрос — не «помоги мне думать», а «классифицируй жалобу клиента на одну из категорий».

Шаг второй. Откройте Hugging Face Inference Playground. Модель — `Meta-Llama-3-8B-Instruct`. Один запрос — три значения температуры: 0, 0.7, 1.5. По три запуска каждой температуры. Итого девять ответов.

Шаг третий. Принесите одностраничный разбор. Любой формат. Что изменилось от `T=0` к `T=1.5`. Что осталось одинаковым. И какую температуру вы выбрали бы для production-использования с обоснованием.

Если HF недоступен — Together.ai playground или локальная Ollama. Не подойдут UI ChatGPT Free и Claude.ai — у них нет регулятора температуры в интерфейсе.

[gold callout]

Бонусный челлендж — на слайде, не буду читать. Это про слепоту к буквам на трёх моделях.»

[Переход на s28.]

---

## [s28 · 1.5 мин] — Мост к Лекции 3

[На слайде — 4 концепта Лекции 3.]

«Что в Лекции 3. Тема — «Агенты, RAG, API: как AI выходит за пределы чата».

В нашем конвейере есть жёсткое ограничение: модель видит только контекст, наружу пойти не может. Лекция 3 покажет, как это обходится через четыре класса инструментов.
...
- 2 минуты на переходы и regulаторку темпа (распределены между разделами).
...
- Если идёт медленнее — сокращать s12 (целиком reformulation, можно за минуту), s22 (один экран, можно 30 секунд), s26 (callback на Лекцию 1, можно сжать).

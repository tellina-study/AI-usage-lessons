...
## v2.1 — owner-прожарка #183 (2026-09-05)

Brief: Часть A (глобальная зачистка тона — уверенный тон без оправданий),
Часть B (12 содержательных переделок s01–s14), Часть C (мелкие syncs).

### Изменённые слайды (источники + builder + pptx)

- s01 REDESIGN: постер-приманка «Шесть утверждений о LLM. Вы верите минимум
  в одно.» + gold «Все шесть — ложь.» + 6 полноширинных строк 16pt поверх
  hero (полупрозрачные white rows, новый helper `set_fill_alpha`).
- s02: subtitle без «проверяем ментальную модель»; notes — тон-формула
  owner'а (единственное место).
- s03: «Объект сегодня — слой „модель“» (вместо «фиксируем рамку»).
- s04: «Центральный вопрос» → «Цель лекции», gold на «важными деталями».
- s04b: петля авторегрессии (teal elbow-line Токен→Токены + подпись
  «токены генерируются по одному; каждый добавляется ко входу»); notes.
- s05/s19/s31/s35/s12/s12a: тон-зачистка notes (без визуальных изменений).
- s07: тезис «chat-формат — соглашение поверх плоского потока» + карточка
  «протокольные роли ≠ „роли“ из текста» + блок «Что с этого».
- s08: callout «рваный интеллект» с объяснением (навык = данные обучения)
  + строка «Что делать: cranberry своей предметки».
- s09: строка-мотивация (зачем инженеру) + блок «Что делать» (был).
- s10: блок «Что на практике» (glitch-гипотеза+плейсхолдер; санитизация);
  «системная особенность» → footer-строка.
- s11: gold-блок «Что делать» (калибровка лимитов на своём языке; когда
  выгоден перевод на EN) вместо одиночного callout.
- s13 REFRAME: «Эмбеддинги — не только внутренность инференса, но и
  инструмент поиска»; карточки с пометками «внутри инференса» /
  «самостоятельный инструмент»; gold-карта «это и есть ваш поиск/RAG».
- s14: возврат осей-признаков (X: тематика веб↔кулинария; Y:
  инфраструктура↔фронтенд) + подпись «каждая из 1536+ осей — выученный
  признак»; борщ перенесён вправо (кулинария).
- s38: «Точная ментальная модель…» → «Знать инструмент — значит знать его
  границы».

### Визуальный цикл

- Iter 1: полный rebuild + inspect 19 затронутых слайдов. Найдено:
  s01 — перенос заголовка на 2 строки с наездом на gold-строку (P1);
  s13 — title 24pt переносился и наезжал на строку-тезис (P0).
- Iter 2: s01 заголовок 30pt одной строкой; s13 title 21.5pt. Re-render —
  оба PASS (5-sec тест).
- Iter 3: контрольные нетронутые s06/s20/s42 — без регрессий; s38 callout
  PASS.

### Верификация

- Anti-leak grep (visible+notes из pptx): 0 hits.
- Тон-grep (visible+notes из pptx): 0 hits (1 false positive —
  «экспериментальная» в notes s37 содержит подстроку «ментальн»);
  owner-формула присутствует ровно 1 раз (notes s02).
- deck.yaml валиден; deck.yaml↔файлы: 0 расхождений по id/section/assertion
  (синхронизировано 24 assertion-паравразы batch-2 + s13 + Раздел 4 rename
  + frame-фразы дивайдеров s18a/s26a/s35a).
- lec-02.pptx (46 слайдов), lec-02.pdf, lec-02-notes.pdf (50 страниц)
  перегенерированы.

---

## v3.0 — rework round 2, полный дек 47 слайдов / 100 мин (#183, 2026-09-05)

Spec: `notes/lecture-2-review/final/rework-round2.md`. Wave 1 (контент
slides/*.md + мем-ассеты `gen_assets_v3.py`) → Wave 2 (deck.yaml v3.0 +
builder v3.0 + рендер + верификация).

### deck.yaml v3.0

- 47 слайдов (spec заявлял «48» — расхождение задокументировано в шапке,
  канонично 47), 7 разделов, порядок Р3: s18→s19→s21→s22→s20→s23→s25.
- `misconceptions_map` удалена целиком; упоминания M-кодов из комментариев
  убраны.
- Тайминг: секционные суммы = spec-таблице 9/15/11/21/20/12/12 = 100.0
  ровно; буферы 0 (Q&A внутри Р6/s42). duration_min синхронизированы в
  deck.yaml и frontmatter 27 слайдов (единственная разрешённая правка
  slides/*.md).
- Drift-фиксы против источников: s02 learning_goal + subtitle («шесть
  границ» → «границы»), s20 chapter_ref (+FACT-CHECK tag), s42
  learning_goal, interaction_summary s20 («он…»).

### Builder v3.0 (`build_lec02.py`)

- BUILDERS: s07 удалён; +s33a/+s33 (НОВЫЕ); канонический порядок 47;
  page numbers `N/46` (знаменатель = число нумеруемых слайдов, s42 без
  footer; вычисляется, не захардкожен).
- `pipeline_bar`: v3.0 passed-стадии (teal-tint) + muted_gold режим;
  дивайдеры s05a/s12a/s18a/s26a — passed-подсветка; s33a — вся лента
  приглушённый gold (раздел «над» конвейером), s35a — полный gold +
  рамка («всё пройдено», визуально сильнее s33a).
- Дивайдеры: мини-иллюстрации (нож/карта/фонарик/кости/матрёшка/пазл),
  угловое размещение top-right.
- s01 мем-хук (двухпанельный T=0, hero ≈40%+), s02a 7 карточек без
  M-чипов, s04 7 промис-чипов (4+3), s08 «гонка патчей» (timeline
  5.2→5.5→5.6+StrawberryBench), s09 авто-роутинг строка, s10 «на что
  влияет», s13 карточка 2, s14 +«звёздная карта», s18 матрица «он»→«Кот»
  (gold 0,7) + callout «на что влияют веса», s19 Q/K/V worked-пример +
  новый chart (лидер «Кот» gold, QuickChart), s20 контраст GIL без
  спуфинга + research-оговорка, s21 +вендорная строка кэша, s22 схема
  трёх запросов (mini-poll снят), s23 «до 1M — фронтир-стандарт», s25
  два яруса (U-кривая → плоская + NoLiMa), s27 формула argmax/P^(1/T) +
  «почему T влияет», s28 вывод про детерминизм, s29 таблица 5 параметров
  (effort gold), s31 стоп-условия + «горшочек, не вари!», s33 4-колоночная
  матрица (свои иконки s33-laptop/gpu/server/cloud, gen_icons_s33.py),
  s35 recap-фрейм + Glitch-токены overlay, s38 таблица «механизм →
  граница → что делать» (9 строк, без M-кодов), s41 якорь без спуфинга.
- `load_notes`: strip-safe маркеры `[VFY-day-of]`/`[FACT-CHECK…]`
  вырезаются при выпечке notes (pre-gate grep speaker_notes = 0).

### Визуальный цикл

- Iter 1 (полный рендер 47): s01/s33a/s33/s35a/s38/s02a/s08/s19/s20/s21 —
  PASS 5-sec; найдено: s31 — title 25pt на 2 строки, наезд на chip «вход»
  (P0); s25 — U-кривая вылезала за низ верхнего бокса (P1); s33 —
  колонки полупустые (P2, visual mass).
- Iter 2: s31 title 21pt одной строкой; s25 img w 7.4→6.2 + сдвиг
  текста; s33 шрифт 12.5 + интервалы. Re-render — все PASS.
- Iter 3: контроль s22/s27/s29/s41/s35/s04/s42 + нетронутые — без
  регрессий; s42 без номера страницы, s41 = 46/46.

### Верификация

- deck.yaml валиден; deck↔slides: 0 расхождений (id/type/section/
  duration_min); секционные суммы 9/15/11/21/20/12/12 = 100.0.
- pptx = 47 слайдов; lec-02.pdf 47 стр; lec-02-notes.pdf 62 стр.
- Anti-leak grep (visible + notes из pptx): все паттерны 0 («Лектору»,
  «Вы здесь», VERIFY/VFY, FACT-CHECK, LO-коды, §N, →sNN, (sNN), M1–M6,
  «N мин», ⏱/Тайминг, методическ/педагогическ) — 0/0 по обоим слоям.
- Тон-grep: 0 истинных hits (3 false positive: «оправдывает» ×2 как
  глагол цены/пользы в notes s11/s32, «экспериментальная» в notes s37).
- Snapshots: 47 файлов sNN.png в новом порядке; stale v21-* и старый
  s07.png удалены.

---

## v3.0.1 — surgical fix wave после QA (3 фикса: s29 P1, s14 P2, s25 P2) (#183, 2026-09-05)

Brief: ровно 3 правки, ничего сверх (No Extra Content). Rebuild: python-pptx
full rebuild; convert — канонический portable-LO (isolated HOME/profile) →
pymupdf 150dpi; notes-PDF — `tools/presentation-build/notes_pages_pdf.py`.

### Правки

- **s29 (P1, header/count mismatch):** таблица содержит 5 строк, заголовок
  обещал «шесть». Снято обещание числа: title bar → «Ручки API: к
  случайности и длине добавилась глубина рассуждения» (builder size 26→24 —
  профилактика переноса, DejaVu шире Arial); assertion → «К классическим
  ручкам „случайность + длина“ добавилась ось „глубина рассуждения +
  многословие“» синхронно в slides/s29-api-knobs-six.md + deck.yaml.
  Таблица и остальной слайд не тронуты.
- **s14 (P2, marker/label overlap):** маркеры точек касались подписей.
  Сдвиг label-боксов: «Установка HTTPS-сертификата» x 2.78→2.90, «Сборка
  React-приложения» x 3.02→3.14 (зазор ~0.10–0.11" от края точки).
  Контента не менял.
- **s25 (P2, chart label size <11pt):** оба QuickChart-PNG перегенерированы
  с крупными шрифтами. Математика: ucurve канвас 1100px @ 6.2" → 1px ≈
  0.406pt ⇒ 28px = 11.4pt (ticks/legend/оси; было 15–17px ≈ 6–7pt);
  nolima канвас 980px @ 4.6" → 1px ≈ 0.338pt ⇒ 33px = 11.2pt (было
  16–18px ≈ 5.4–6.1pt). Заголовки осей Y — двухстрочные массивы («точность
  поиска / вставки, %», «% от базовой / точности модели»), иначе rotated
  title не влезает по высоте. Конфиги обновлены in place:
  gen_assets_v3.py::gen_s25_ucurve, gen_assets_v2.py::gen_s25_chart.

### Визуальный цикл

- Iter 1 (chart-PNG до встраивания): nolima PASS; ucurve — двухстрочный
  Y-title подрезан сверху + x-ticks авто-повёрнуты диагонально (съедали
  высоту). Fix: `maxRotation: 0, minRotation: 0` на x-ticks — chart area
  вырос, title влез.
- Iter 2: ucurve перегенерен — title полностью виден, ticks горизонтальны,
  легенда одной строкой. PASS.
- Iter 3 (slide-PNG после rebuild): s29 — title одной строкой, без наезда
  на таблицу, 5-sec PASS (main message = assertion, счёта нет); s14 —
  зазор маркер↔подпись виден на обеих точках, без регрессий; s25 — labels
  обоих чартов визуально ~уровня 12.5pt body-текста, читаемы @50%. PASS.

### Верификация

- pptx = 47 слайдов; lec-02.pdf 47 стр; lec-02-notes.pdf 62 стр
  (как до фикса).
- deck.yaml↔s29-md: id/section/duration_min/type/assertion — 5/5 MATCH.
- Anti-leak grep (visible + notes из pptx, s29/s14/s25): 0 hits — включая
  контрольный паттерн «стало шесть/шесть руч».
- Snapshots: все 47 sNN.png перерендерены (150dpi, той же схемой имён).

---

## Раунд #183 (round 3) — реальные изображения вместо SVG-мемов + s41 fix + s20 fact-check

**Owner-мандат:** «вставь нормальные мемы и картинки из интернета... в заголовке каждого раздела должен быть мем или интересная картинка! + 1-2 внутри раздела».

### Задача A — 6-tier real image acquisition

Все 8 старых SVG-моков (`s01-t0-meme`, `s05a-tokenize-knife`, `s12a-meaning-map`,
`s18a-flashlight`, `s26a-dice`, `s31-gorshochek`, `s33a-matryoshka`,
`s35a-puzzle`) удалены из `build_lec02.py` (файлы на диске оставлены,
просто не референсятся). Заменены на реальные изображения — источники +
статус см. `rendered/assets/web/attribution.md`. Итог: 12/12 целевых
слайдов получили real image (0 failed acquisitions, 1 честный компромисс —
word2vec диаграмма — heatmap вместо идеальной 2D-стрелочной, но реальная и
узнаваемая, см. attribution.md «Попытки, не давшие результата»).

### Задача A1 — `section_divider()` redesign (2 итерации)

- Iter 1: 2-колоночная композиция (текст слева ~6.35", фото справа в Ocean
  box ~5.55×4.55" ≈ 25% площади). PASS для 4/6 (s26a dice, s33a matryoshka
  — однострочная подпись помещается; s01/s28 мем на отдельных слайдах).
  FAIL для s05a/s18a — 2-строчная подпись обрезалась нижней границей box
  (caption height 0.34" недостаточно).
- Iter 2: увеличена `cap_h` до 0.52", line_spacing 1.05, размер шрифта
  10.5→9.5pt. Re-render: s05a/s18a подписи полностью в рамке. PASS 6/6.

### Задача A2/A3 — s01, s08, s10, s28, s31, s36 (content images)

- s01: мем «Well yes, but actually no» (imgflip, кадр Aardman «Пираты!»),
  hero 7.6"×5.79" ≈ 44% площади, свой текст в пустом верхнем поле шаблона.
- s08: та же тема strawberry, второй crop скриншота (полная переписка),
  встроен в левый «Механизм» box — потребовалась 2 итерации подгонки
  размера (изначально shot_h переполнял box; финал 2.4"×1.22").
  Скриншот сначала кадрирован с захватом половины avatar-иконки сверху —
  перекадрирован (offset 100→160px) для чистого верхнего края.
- s10: скриншот GPT Playground «petertodd» glitch-токен (LessWrong,
  тот же корпус, что SolidGoldMagikarp) — потребовалась 1 итерация
  сжатия текстовых блоков в left box для освобождения места (col split
  0.75"/0.65" вместо изначальных 0.75+1.0).
- s28: мем «Well yes, but actually no» переиспользован с другим текстом
  («T=0 = детерминизм?») в масштабе thumbnail 1.42"×1.08" — 2 итерации
  подгонки под узкий зазор (0.85") между экспонатом и рядом плашек.
- s31: реальная иллюстрация Otto Ubbelohde (1909, общественное достояние)
  к «Sweet Porridge»/«Горшочек каши» — прямая замена мока, 1 итерация
  (масштаб 1.55"×0.78", центрирован в свободной зоне box).
- s36: 6 логотипов AI-компаний (LobeHub icons-static-svg, rasterized via
  pymupdf — rsvg-convert недоступен на хосте, PATH проверен явно) —
  recolor в Ocean `#21295C` через замену `fill="currentColor"` перед
  рендером. 1 итерация (сразу легло в существующую колоночную структуру).

### Задача B — s41 композиция (2 итерации)

- Проблема: мост (hero_w=9.9", y=1.35) лежал ПОД карточками 2×2, видна
  была только узкая вертикальная полоса — арка/тросы/опоры обрублены.
- Root cause: исходный PNG 1984×1024 имел ~240px пустого поля сверху —
  контент моста занимал только y=240..1024. Bbox-анализ (`im.getbbox()`)
  подтвердил.
- Fix: (1) обрезка PNG по content-bbox → `s41-bridge-lec3-crop.png`
  (1984×784, ratio 2.53:1); (2) композиция сменена с «фон за карточками»
  на «горизонтальная полоса под заголовком, карточки ниже» — мост теперь
  занимает отдельную зону (bridge_w=7.4", h≈2.92", ~21.6% площади,
  ПОЛНОСТЬЮ читаем: арка, все тросы, обе опоры, оба конечных узла).
  Карточки уплотнены (card_h 2.3→1.62, gap 0.55→0.14) чтобы уместиться
  ниже без второго overflow.
- Iter 1: bridge полностью виден, но cards 2-rows заходили за нижнюю
  границу слайда (7.5") на ~1.6" при исходных размерах карточек.
- Iter 2: card_h/gap уменьшены, anchor-тексты сокращены до 1 строки —
  итоговая нижняя граница карточек 7.34" (в пределах 7.5"). PASS.

### Задача — s20 fact-check fix (сторонний запрос, тот же цикл)

Zheng et al. (EMNLP 2024 Findings) измеряли ТОЛЬКО точность факта (2410
вопросов, 162 роли) — приписанный им вывод про «тон/стиль» в статье
отсутствует. Research-callout box разделён на 2 строки: (1) точная цитата
с цифрами про точность факта, (2) отдельное наблюдение курса про
тон/стиль без привязки к Zheng. Синхронизировано в 3 местах: builder,
`.md` visible body + speaker notes, `.md` visual_brief frontmatter.
deck.yaml проверен — дублирующего content-summary поля для s20 нет
(только `[FACT-CHECK]` marker в chapter_ref, оставлен как есть).

### Верификация

- Full rebuild: 47/47 slides OK. PDF 47 стр (3.8 MB). Notes-PDF
  regenerated: 63 страницы (47 + continuations), positional match
  (см. [#170-4b]).
- Anti-leak grep (visible pptx text): 0 hits — `[VERIFY-DAY-OF]`,
  `[FACT-CHECK]`, `LO[1-9]`, `§X.X`, `→ sNN`, timing-маркеры,
  методические комментарии.
- Deep latin-token scan (полный dec): 243 occurrences / 161 unique — все
  legitimate (модели/бренды/API-параметры уже существовавшие в деке,
  ни одного нового англицизма от моих правок). Точечная проверка только
  добавленного контента (14 затронутых слайдов) — чисто, только brand
  names + established acronyms + literal case words (strawberry/
  cranberry как объект изучения).
- Snapshots: все 47 sNN.png перерендерены (150dpi).
- **Doc-size flag:** этот файл (iteration-log.md) уже 1298+ строк —
...
## v3.3 (issue #183 round 5) — +8-12 новых образов на content-слайды без образа

Прошёл все 47 build_sNN функций + slides/*.md, сверил `visual.primary` /
assertion каждого слайда с фактическим наличием `add_image`/
`_place_image_contain` вызова (не icon-in-ocean-box). Baseline перед этим
раундом: **15 media-rich слайдов** (9 non-divider: s01, s08, s10, s11, s19,
s25, s28, s31, s41 + 6 section-divider illus: s05a, s12a, s18a, s26a, s33a,
s35a).

### Geometry-анализ плотности (выполнен ДО правок, по факту прочтения
### build_sNN + baseline PNG snapshots)

Для каждого кандидата из брифа измерил вертикальный remainder до footer
(y=7.16) или до следующего структурного элемента:

| Слайд | Remainder найден | Решение |
|---|---|---|
| s09 (числа/код) | 0 — 2 колонки + 3-tips row + footer text заполняют слайд до низа | SKIP |
| s11 (стоимость RU-токена) | ~0.25" на всю ширину под chart-box/gold-box | SKIP (недостаточно для читаемого 16:9 мема) |
| **s15 (similarity)** | **~1.35" × 4.75" справа под легендой шкалы** | **ADDED — Spider-Man** |
| s21 (KV-cache) | ~0.15" — схема+2 фазы+vendor-row+gold callout заполняют полностью | SKIP |
| s22 (prompt caching) | ~0.35-0.4" между колонками/gold callout | SKIP |
| **s25 (long-context)** | **~1.0" × 5.15" в верхнем ярусе справа от текста** | **ADDED — needle photo (2-й образ на уже media-rich слайде)** |
| s27 (temperature) | ~0.4" до footer после 3 панелей+callout+top-p/k+live-badge | SKIP |
| s30 (structured outputs) | ~0.4" до footer после 2 колонок+вопрос-залу box | SKIP |
| s32 (reasoning tokens) | ~0.5" до footer, полностью занят 2 ярусами+callout | SKIP |
| s37 (benchmarks) | ~0.2" между 3-й карточкой и gold callout | SKIP |
| **s39 (когда не LLM)** | **~0.7" в правой колонке под текстовым блоком лестницы (после сжатия text_runs h=1.7→1.35)** | **ADDED — Two Buttons (top-panel crop)** |
| **s40 (корреляция≠причинность)** | **~1.2" на всю ширину под gold callout** | **ADDED — xkcd #552** |

### s15 — Spider-Man Pointing at Spider-Man

- Iter 1: скачал `spiderman-pointing-template.jpg` (imgflip, 1wf7pq, 800×450,
  ПРОВЕРЕН визуально — корректный шаблон, не подмена). Сгенерировал
  `spiderman-similarity.jpg` через `gen_memes_v33.py`: добавил белую полосу
  сверху (30% высоты) с подписью «высокое сходство — «похожи», а не факт что
  «об одном»» (26pt). Встроил в ocean_box 8.05,4.92,4.75,1.24 под легендой
  шкалы (свободная зона между легендой и gold callout).
- Inspected iter 1 (snapshots_iter1/s15-18.png): подпись мелкая
  (нечитаема при projector 50%-zoom тесте), персонажи хорошо видны.
- Iter 2 changed: увеличил высоту полосы под текст (30%→34%), убрал часть
  текста для краткости (««похожи» — не факт что «об одном»», 34pt),
  увеличил box 1.24→1.30, картинку 1.10→1.16.
- Re-render iter 2 (snapshots_iter2/s15-18.png): подпись читается чётко,
  персонажи не обрезаны, ocean_box рамка не пережимает картинку.
- Iter 3 (projector 50%-zoom half-size thumbnail test):
  снапшот уменьшен вдвое (`/tmp/half_s-18.png`) — заголовок слайда несёт
  главный тезис, мем читается как метафора «двое похожих указывают друг на
  друга» даже без чтения мелкой подписи. PASS.
- 5-Second Test: PASS — «similarity ≠ релевантность» считывается через
  заголовок + heatmap + failure-карточка; мем — вторичное усиление, не
  перегружает.
- Verdict: accept (3 итерации).

### s25 — needle-in-haystack фото (2-й образ, слайд уже был media-rich)

- Iter 1: скачал `Needle_in_haystack4.jpg` с Wikimedia Commons
  (CC-BY-SA 4.0, 4951×3451, 11MB) → resize до 1200×836 (`PIL.thumbnail`) →
  `needle-haystack-wikimedia.jpg` (234KB). Сжал text_runs top-tier (h=1.6→
  0.85) чтобы освободить зону 7.35,2.62,5.15,0.74 под текстом; встроил
  ocean_box + `_place_image_contain`.
- Inspected iter 1: фото иголки в стоге сена чётко видно, дополняет
  U-кривую слева, не перекрывает текст «U-кривая 2023 распрямилась…».
- Iter 2: full rebuild + re-render — проверил, что усечённый text_runs (h=
  0.85 вместо 1.6) не обрезал текст (2 строки укладываются). PASS,
  никаких артефактов переполнения.
- Iter 3 (projector 50%-zoom): фото различимо как «иголка в сене» даже
  в уменьшенном виде — метафора-якорь работает.
- 5-Second Test: PASS — верхний ярус «Забывание победили ✓ — needle-in-
  a-haystack» теперь имеет буквальный визуальный якорь, не только текст.
- Verdict: accept (3 итерации).

### s39 — Two Buttons (LLM vs обычный код)

- Iter 1: скачал `twobuttons-template.jpg` (imgflip, 1g8my4, 600×908,
  ПРОВЕРЕН — корректный шаблон potty-panel). Сгенерировал
  `twobuttons-llm-vs-code.jpg` (подписи «LLM» / «обычный код» над кнопками
  верхней панели через `gen_memes_v33.py`). Crop верхней панели (0-50%
  высоты) → `twobuttons-llm-vs-code-toponly.jpg` (600×454, только кнопки,
  без нижней панели с потным лицом — экономия места). Сжал text_runs
  правой колонки (лестница) h=1.7→1.35, освободил зону 8.65,6.0,3.9,0.62.
- Inspected iter 1: подпись «обычный код» слегка наезжала на верхний край
  карточки-кнопки.
- Iter 2 changed: сдвинул подписи вниз (top_h*0.22→0.30 для LLM,
  0.20→0.28 для «обычный код»), уменьшил размер шрифта второй подписи
  (28→24-26pt), max_width сужен (0.34→0.30).
- Re-render iter 2: подписи не наезжают на границы карточек, читаемы.
- Iter 3: увеличил inset box 0.62→0.68 (использовал доп. миллиметры
  свободного места до границы правого ocean_box), full rebuild + re-render
  — картинка не обрезана рамкой, пропорции сохранены (`_place_image_contain`).
- 5-Second Test: PASS — заголовок «Когда не LLM — и когда не топ-LLM» +
  decision-tree слева + мем справа усиливают тезис «трудный выбор
  инструмента» без повторения текста.
- Verdict: accept (3 итерации).

### s40 — xkcd #552 «Correlation»

- Iter 1: скачал `https://imgs.xkcd.com/comics/correlation.png` напрямую
  (459×185, grayscale PNG, прямое попадание — комикс буквально про
  «I used to think correlation implied causation… now I don't»). Встроил
  по центру под gold callout (zone y=6.3-7.5, w=12.25 полная ширина),
  начальный размер 2.35" ширина.
- Inspected iter 1: комикс мелковат при 150dpi для 3-панельного текста
  (ожидаемо — это оригинальный текст на английском внутри самого комикса,
  не наша подпись; текст комикса неизбежно останется мелким при таком
  aspect ratio, но узнаваемость формата не страдает).
- Iter 2 changed: увеличил размер 2.35"→2.7" ширина (высота ~1.09"),
  сдвинул чуть выше (6.42→6.38) чтобы не упереться в футер-зону.
- Re-render iter 2: комикс не перекрывает номер страницы (центрирован,
  page_number в правом нижнем углу x=12.15 — комикс заканчивается ~x=10.5).
- Iter 3 (projector 50%-zoom): комикс распознаваем как xkcd-формат
  (чёрно-белые человечки, панели) даже без чтения английского текста —
  русскоязычный gold callout выше уже несёт перевод тезиса. PASS.
- 5-Second Test: PASS — «внимание = корреляция, не причинность» усилено
  прямой отсылкой к классическому «correlation ≠ causation» комиксу.
- Verdict: accept (3 итерации).

### Пропущенные кандидаты (плотные слайды, geometry не позволила)

s09, s11, s21, s22, s27, s30, s32, s37 — детальный geometry-анализ см.
таблицу выше. Для s11/s21/s22/s27/s30 скачаны и (для s11) сгенерированы
mem-assets (см. `attribution.md` § «попытки, не давшие результата») —
готовы для будущего раунда, если содержание слайдов будет сокращено или
переверстано. НЕ форсировал вставку ценой обрезки существующего
контента — per brief «если образ не влезает без потери читаемости —
ПРОПУСТИ».

### s33a chonk chart — 4-я попытка (продолжение v3.2)

3 дополнительных WebSearch/WebFetch попытки в этом раунде (детали в
`attribution.md` § chonk chart) — raw image URL с лицензией так и не
найден (Know Your Meme отдаёт placeholder GIF, поисковики — только
маркетплейс-мерч). Ветеринарная Body Condition Score шкала — рассмотрена
и отклонена как **другой артефакт** (не юмористический мем-формат).
Матрёшка остаётся на s33a без изменений (per brief §9 fallback).

### Финальная верификация v3.3

- Full rebuild: 47/47 slides OK. PPTX 2643 KB.
- PDF regenerated (soffice --headless --convert-to pdf), notes-PDF
  regenerated через `tools/presentation-build/notes_pages_pdf.py
  library/lectures/lec-02` — 63 страницы (47 + continuations, совпадает
  с v3.2 baseline — подтверждает, что новые образы НЕ вызвали overflow
  notes на затронутых слайдах).
- Snapshots: все 47 sNN.png перерендерены (150dpi).
- Anti-leak grep (visible body, extracted через python-pptx): 0 hits на
  `\[VERIFY-DAY-OF\]|\[FACT-CHECK\]|LO[1-9]|§[0-9]|→ s[0-9]+|\(s[0-9][0-9]\)|
  Лектору|Вы здесь`.
- Caption-leak grep (visible body): 0 hits на
  `Wikimedia|скриншот|CC-BY|imgflip|arXiv|LessWrong|Alammar|LobeHub|source|
  источник:` (мем-подписи baked в растровые картинки, не text_frame —
  подтверждено grep на «always has been»/«LLM»/«Spider»/«Stonks»/«Gandalf» —
  0 hits, весь оверлей-текст не извлекается как PPTX text).
- Deep latin-token scan (visible body, `deep_latin_scan.py`): 219
  occurrences / 140 unique — ИДЕНТИЧНО v3.2 baseline (новые образы не
  добавили ни одного нового latin-токена в extractable text, т.к. подписи
  на мемах — растровые, не text runs).
- Media-rich slide count: 15 → **18** (+3 net new media-rich slides:
  s15, s39, s40; s25 получил 2-й образ, но уже был media-rich в baseline).
- Projector-readability (50%-zoom half-size PNG thumbnail test) — все 4
  изменённых слайда PASS: главный визуал/мем остаётся узнаваемым и не
  превращается в шум при уменьшении.

## v3.3 round 2 (issue #183) — увеличение «марок» + образы Группы B (2026-09-06)

Оркестратор провёл независимую визуальную инспекцию round-1 и НЕ принял
результат: (A) все 4 вставки — «марка в пустом контейнере» (0.7–1.5" в
широких боксах); (B) отказ «нет места» опровергнут для 5–6 слайдов
(правило: образ можно ставить, сжимая/сдвигая наименее ценные элементы).

Инструмент: прямые правки `build_lec02.py` + регенерация оверлеев в
`gen_memes_v33_r2.py` (шрифты пересчитаны под итоговую ширину вставки —
round-1 подписи 24–34px давали ~8–9pt на слайде после увеличения).

### Группа A — увеличение round-1 вставок

**s15 — Spider-Man Pointing (iter 1-2):**
- (a) iter1: gold failure-карточка сжата 2.6"→1.68" (текст кончался на
  ~1.45", остальное — пустота), легенда шкалы поднята к карточке, мем-бокс
  увеличен 1.3"→2.42" высоты → образ 1.5"→3.0" ширины. Подпись
  перерисована 34px→52px (~14pt на слайде).
- (b) iter2 (final render inspect): подпись читается, мем не давит heatmap,
  gold-карточка без пустоты. 5-сек тест: PASS («similarity ≠ смысл» +
  мем-якорь считываются мгновенно).

**s25 — иголка в стоге (iter 1-2):**
- (a) iter1: текст верхнего яруса сужен 5.15"→2.72" (место есть — 3 строки),
  фото перенесено на правый фланг полной высоты яруса: 0.9"→2.5" ширины,
  полный кадр. Инспекция: иголка в полном кадре 1200×836 на 2.5" почти
  не видна (тонкая, 30px).
- (b) iter2: умеренный кроп 930×656 (needle-haystack-crop.jpg) — иголка
  ~30% ширины кадра, контекст стога сохранён (НЕ «агрессивный кроп только
  сена» из round-1). Financial render: иголка различима. 5-сек: PASS.

**s39 — Two Buttons (iter 1-2):**
- (a) iter1: лестница правой колонки сдвинута вверх на 0.4" (блоки «10%
  сложных» 2.25→1.85, «90% запросов» 3.2→2.8, текст 4.55→3.9), crop
  подрезан 454px→400px (кнопки + начало руки), этикетки перерисованы
  32/24px→52/38px. Образ 0.9"→2.28" ширины.
- (b) iter2 (final): «LLM» / «обычный код» читаются с проектора (~14.5pt
  и ~10.6pt), образ внутри бокса, без перекрытий. 5-сек: PASS.

**s40 — xkcd #552 (iter 1-2):**
- (a) iter1: gold callout сужен 12.25"→7.55" (текст 2 строки, влезает),
  комикс справа от него 2.7"→4.5" ширины, отступ от нижней грани 0.30"
  (было ~0.03"). Заменён файл на 2x-версию (918×371, скачана с xkcd) —
  чёткость панелей на 150dpi.
- (b) iter2 (final): 3 панели читаемы, диалоги различимы; баланс callout ↔
  комикс — визуальные массы уравнены. 5-сек: PASS.

### Группа B — новые образы (места нашлись сжатием наименее ценного)

**s09 — Math Lady (iter 1-2):**
- (a) iter1: скачан шаблон (попытка 1 `i.imgflip.com/1fyz5c.jpg` — 404;
  попытка 2 KYM CDN — OK, 681×445), оверлей «[123][456][78]?» 64px.
  Вставлен в нижнюю половину колонки «Код» (текст занимал верхнюю
  половину). Инспекция: ПЕРЕКРЫТИЕ — мем наложился на «под все задачи
  сразу» (текст GPT-4 кончается ~3.55", мем начинался 3.42").
- (b) iter2: колонки удлинены 3.30"→3.70", мем сдвинут на y=3.62, нижний
  ряд («Что делать» + 3 карточки + footer) опущен на 0.14–0.34". Чисто,
  без перекрытий, мем ~2.4" ширины. 5-сек: PASS.

**s11 — Always Has Been (iter 1-2):**
- (a) iter1: верхний правый бокс расширен 2.2"→3.15" высоты, gold-бокс
  «Что делать» опущен 4.15→5.05 (нижняя граница 7.4", влезает), подписи
  мема перерисованы 34/30px→54/48px. Инспекция: ПЕРЕКРЫТИЕ — мем
  наложился на строку «разрыв сокращается, но не исчезает».
- (b) iter2: мем сдвинут y=3.02→3.22, высота 1.72"→1.55" → ширина 2.76".
  Чисто. 5-сек: PASS («русский токен дороже? — always has been» прямо
  дублирует тезис слайда).

**s22 — Stonks (iter 1-2, опциональный — ВСТРОЕН):**
- (a) iter1: бары кейса укорочены пропорционально (4.6"→3.1" и 0.82"→0.55",
  соотношение −82% сохранено), подписи сдвинуты, Stonks 1.65" ширины
  справа от баров (зона была пуста).
- (b) iter2 (final): «stonks» label читается, образ контекстный (сразу
  после «−82%»), не «марка в пустоте» — примыкает к барам. 5-сек: PASS.

**s30 — Gandalf (iter 1-3):**
- (a) оверлей: iter1 76px 2 строки — «ПРОЙДЁТ» обрезан нижней гранью
  кадра; iter2 62px — одна строка, но касается краёв (Н и Ё впритык);
  iter3 54px — влезает с полями. (3 итерации на PIL-оверлее.)
- (b) слайд iter1: «Вопрос залу» сдвинут влево (0.55..9.15), Gandalf
  2.87" ширины на правом фланге (9.57..12.44, y 5.92..7.32, отступ снизу
  0.18"). Подпись решена ОСТАВИТЬ (5-сек тест: без подписи связь
  «Gandalf → маскирование токенов» не мгновенна; с подписью — прямая).
- (c) iter2 (final): читается, без перекрытий. PASS.

**s32 — Джокер жжёт деньги (iter 1-2):**
- (a) поиск: 3 попытки Wikimedia («May Day NYC» — тёмная толпа; «Burning
  fake money» — деньги не читаются; «Burning Paper Money» — пламя в
  темноте) — все отклонены по 5-сек тесту как нечитаемые. Tier-4:
  YouTube maxres thumbnail сцены из The Dark Knight — иконический кадр.
  Кроп леттербокса + бейджа «1080p FULL HD» (1280×720 → 1090×610).
- (b) iter1: вставлен 1.86" в правый верхний угол верхнего контейнера
  (зона x 9.2–12.6 / y 1.4–2.5 была пуста), ровно над «×3–10».
- (c) iter2: увеличен до ~1.9" (контейнер позволял), финальный рендер —
  кадр читается (Джокер + горящая гора), прямое усиление тезиса. PASS.

**s37 — Press X to Doubt (iter 1-2):**
- (a) iter1: попытка 1 `i.imgflip.com/1ii4oc.jpg` (из брифа) — оказался
  ДРУГИМ мемом (Trump Bill Signing); попытка 2 — корректный
  `1txerc.jpg` (800×450) через imgflip-страницу шаблона. Текст карточки 2
  сужен 11.0"→8.1" (2 строки, влезает), мем 2.28" в правой трети.
- (b) iter2 (zoom-inspect + final): образ внутри границ карточки, «X
  Doubt» читается. 5-сек: PASS.

### Подтверждённые скипы
- **s21, s27** — подтверждены оркестратором в брифе round 2, не тронуты.

### Финальная верификация v3.3 r2

- Full rebuild: 47/47 OK, PPTX 3658 KB; PDF + notes-PDF (63 стр.,
  совпадает с baseline — образы не вызвали overflow notes) перегенерены;
  все 47 snapshots перерендерены 150dpi.
- Anti-leak grep (extracted visible text, 607 text frames): **0 hits**.
- Caption-leak grep (`Wikimedia|CC-BY|imgflip|xkcd|скриншот|источник:`):
  **0 hits** (все оверлеи baked в растр).
- Deep latin scan: **219 occurrences / 140 unique — дельта против
  baseline v3.3 = 0** (ни одного нового латинского токена в text runs).

...
def build_s03(p):
    ...
    terminal_card(s, 0.55, y, 12.23, 3.4, [
        ...
        ("- Критерий приёмки: заявка реально уходит — форма отправлена руками", CODE_FG),
        ("  из собранной страницы, а не только «тест зелёный»", CODE_FG),
...
def build_s43(p):
    ...
    terminal_card(s, 0.55, y, half_w, h, [
        ...
        ("", CODE_FG),
        ("  0031 — та же запись, что в кейсе 2.1:", GOLD),
        ("  она переехала сюда из DECISIONS.md", GOLD),
    ], size=10.8)
    rx = 0.55 + half_w + 0.28
    rw = 12.23 - half_w - 0.28
    terminal_card(s, rx, y, rw, h, [
    ...
    text_box(s, 0.55, y2, 12.23, 7.0 - y2,
             text="Источник практики: ADR — Майкл Найгард, 2011; adr-tools. Сила "
                  ...
                  "нет. Содержимое записей — иллюстративное: номера 0031/0187 и "
                  "встречная пара Amends / Amended by. Снимок демо-репозитория есть "
                  "(doc/adr/, коммит 3b4b289), но он про другую сцену: записей там "
                  "три, встречных ссылок между ними нет, adr-tools не установлен.",

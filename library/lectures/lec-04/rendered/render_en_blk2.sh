#!/bin/bash
# EN-Sync Block 2 (issue #172 / #162) — build + render selected pages of the EN deck.
# Usage: render_en_blk2.sh [page1 page2 ...]   (no pages = all)
set -e
REND=/tmp/lec04-en-block2/library/lectures/lec-04/rendered
...
OUT=/tmp/claude-999/lec04-enb2-snap
cd "$REND"
python3 build_lec04_en.py
rm -rf "$OUT"; mkdir -p "$OUT" "$REND/snapshots-en-blk2"
timeout 300 $SOFF --headless -env:UserInstallation=file:///tmp/claude-999/loprofile_lec04enb2 \
  --convert-to pdf --outdir "$OUT" "$REND/lec-04-en.pptx" >/dev/null 2>&1
PAGES="$*"
python3 - "$OUT/lec-04-en.pdf" "$REND/snapshots-en-blk2" "$PAGES" <<'PY'
import sys, pymupdf

"""
Full 30-slide build of Лекции 3 «Архитектуры AI-систем: агенты, RAG, API».

Source-of-truth: deck.yaml v1 + chapter v1.1 finalized (~22450 слов) +
slides/*.md (30 файлов с readable speaker notes 150-300 слов).
...
Build via: python3 build_v1.py — generates lec-03.pptx.
"""
import re
...
def build_s02(p):
    ...
    text_box(s, x=0.78, y=5.30, w=7.5, h=0.5,
             text="Курс «Применение AI» · 75 минут",
...
def build_s04(p):
    ...
    # Up arrow alongside the ladder showing direction of climb
    up_arrow(s, 8.30, 2.55, 0.34, 3.95, fill=COVER_OUTLINE)
    text_box(s, 8.18, 6.52, 0.60, 0.28, "выше", size=10, italic=True,
...
def build_s06(p):
    ...
    gold_callout(s, 0.55, 5.20, 12.25, 0.92,
...
def build_s13(p):
    """case_study — RAG fails at scale + Air Canada revisited."""
    s = blank(p)
    slide_title(s, "Провал RAG на масштабе.", size=27)
    gold_callout(s, 0.55, 1.12, 12.25, 0.92,
                 "«Система вернула что-то» ≠ «система вернула правильное». У RAG нет встроенного сигнала «не нашёл» — он всегда возвращает k ближайших, даже если они нерелевантны.",
                 size=14.5)
    # left — 3 failure cases
    lx, lw = 0.55, 6.55
    ocean_box(s, lx, 2.20, lw, 3.62)
    cases = [
    ...
    for nm, body in cases:
        text_box(s, lx + 0.26, yy, lw - 0.52, 0.32, nm,
                 size=15, bold=True, color=MID)
        text_box(s, lx + 0.26, yy + 0.34, lw - 0.52, 0.82, body,
                 size=12.5, color=DEEP, line_spacing=1.14)
        yy += 1.18
    # right — Air Canada revisited
    rx, rw = 7.35, 5.45
    ocean_box(s, rx, 2.20, rw, 3.62, fill=TEAL_TINT, stroke=TEAL, stroke_pt=2.0)
    text_box(s, rx + 0.28, 2.42, rw - 0.56, 0.34, "Air Canada revisited",
             size=15, bold=True, color=TEAL)
    text_box(s, rx + 0.28, 2.80, rw - 0.56, 1.25,
             ...
             size=13, color=DEEP, line_spacing=1.16)
    text_box(s, rx + 0.28, 4.10, rw - 0.56, 1.65,
             ...
             size=13, color=DEEP, line_spacing=1.16)
    footer(s, "Документированные классы провалов (Barnett et al. 2024; Air Canada — McCarthy Tétrault 2024). Кейсы — illustrative. · RAG вернул ответ — как узнать, что он правильный?")
...
def build_s15(p):
    ...
    footer(s, "Полный спектр методов настройки поведения (SFT / DPO / RFT) — в главе. В нарративе: full FT — почти никогда; PEFT — рабочая лошадка 2026.")
...
def build_s24(p):
    """schema_architecture — data-in-chain map + 2 facts + rule."""
    s = blank(p)
    slide_title(s, "Кто видит данные в цепочке.", size=27)
    text_box(s, 0.55, 1.14, 12.25, 0.55,
             "ZDR (Zero Data Retention) — провайдер не сохраняет содержимое после обработки. Least-privilege — каждому компоненту только строго необходимые права.",
             size=13.5, italic=True, color=MID, line_spacing=1.15)
    # chain map
    cx, cyy, cw, chh = 0.55, 1.92, 12.25, 1.55
    ocean_box(s, cx, cyy, cw, chh)
    chain = ["ваши данные", "агент", "инструмент", "внешний API", "провайдер", "подрядчики"]
    n = len(chain)
    node_w = 1.72
    ...
    for i, lab in enumerate(chain):
        col = MID if 0 < i < 5 else LIGHT
        filled_rect(s, nx, ny, node_w, 0.62, col, radius=True, radius_adj=0.16)
        text_box(s, nx, ny, node_w, 0.62, lab, size=12, bold=True,
                 color=WHITE, align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
        if i < n - 1:
            ax = nx + node_w + 0.02
            right_arrow(s, ax, ny + 0.16, gap - 0.04, 0.30, fill=LIGHT)
        nx += node_w + gap
    text_box(s, cx + 0.25, cyy + chh - 0.36, cw - 0.5, 0.3,
             "каждое звено — отдельная граница доверия и отдельная retention-политика",
             size=11.5, italic=True, color=LIGHT, align=PP_ALIGN.CENTER)
    # 2 fact panels
    fy, fh = 3.65, 1.65
    ocean_box(s, 0.55, fy, 6.05, fh)
    text_box(s, 0.83, fy + 0.16, 5.55, 0.36, "Судебный приказ переживает вашу политику",
             size=14, bold=True, color=MID)
    text_box(s, 0.83, fy + 0.55, 5.55, 1.0,
             "суд может обязать сохранять все логи как доказательства поверх договорной политики «30 дней»",
             size=12.5, color=DEEP, line_spacing=1.16)
    ocean_box(s, 6.75, fy, 6.05, fh)
    text_box(s, 7.03, fy + 0.16, 5.55, 0.36, "ZDR покрывает не всё",
             size=14, bold=True, color=MID)
    text_box(s, 7.03, fy + 0.55, 5.55, 1.0,
             "вне ZDR: third-party-интеграции, Files, batch, MCP-коннектор — а агент с инструментами это и есть third-party",
             size=12.5, color=DEEP, line_spacing=1.16)
    gold_callout(s, 0.55, 5.48, 12.25, 0.92,
                 "«У нас ZDR» ≠ «нигде ничего не хранится». Контрмера — карта данных по каждой фиче, не галочка. Регулируемое — не слать без ZDR/BAA, рассмотреть on-prem.",
                 size=14)
    footer(s, "BAA — Business Associate Agreement, договор об обработке данных. Конкретная хронология retention — в главе. Статус ZDR — перепроверить ко дню лекции.")
...
def build_s25(p):
    ...
    text_box(s, 0.55, 1.14, 12.25, 0.55,
             ...
             size=13.5, italic=True, color=MID, line_spacing=1.15)
    # left — mechanism + 2 cases
    lx, ly, lw, lh = 0.55, 1.92, 6.55, 3.95
    ocean_box(s, lx, ly, lw, lh)
    # mechanism flow
    flow = ["недоверенный контент\n(issue / тикет / страница)", "контекст модели",
            "исполняется\nкак команда"]
    fw = (lw - 0.5 - 2 * 0.30) / 3
    fx = lx + 0.25
    for i, t in enumerate(flow):
        filled_rect(s, fx, ly + 0.24, fw, 0.92,
                    (GOLD if i == 2 else MID), radius=True, radius_adj=0.12)
        text_box(s, fx, ly + 0.24, fw, 0.92, t, size=10.5, bold=True,
                 color=(DEEP if i == 2 else WHITE), align=PP_ALIGN.CENTER,
                 anchor=MSO_ANCHOR.MIDDLE, line_spacing=1.05)
        if i < 2:
            right_arrow(s, fx + fw + 0.02, ly + 0.50, 0.26, 0.40, fill=LIGHT)
        fx += fw + 0.30
    text_box(s, lx + 0.26, ly + 1.40, lw - 0.52, 0.78,
             "GitHub MCP heist: issue с инструкцией + переизбыточный токен → ассистент выгрузил приватные репозитории",
             size=12, color=DEEP, line_spacing=1.14)
    text_box(s, lx + 0.26, ly + 2.20, lw - 0.52, 0.55,
             "Tool poisoning / rug-pull: инструмент меняет своё определение после установки",
             size=12, color=DEEP, line_spacing=1.14)
    filled_rect(s, lx + 0.26, ly + 2.95, lw - 0.52, 0.78, GOLD_TINT,
                stroke=GOLD, stroke_pt=1.5, radius=True, radius_adj=0.10)
    text_box(s, lx + 0.44, ly + 3.02, lw - 0.88, 0.64,
    ...
    for i, (t, b) in enumerate(rules):
        ...
        ry += 0.80
    footer(s, "Полная CVE-хронология MCP-инцидентов — в главе/notes, не на видимом слое.")
...
def build_s26(p):
    ...
    for i, (num, label, trig, col, isgold) in enumerate(steps):
        ...
        if trig:
            ...
            tgy = y - trig_h - 0.01
            text_box(s, x + 0.30, tgy, full_w - 0.30, trig_h,
...
def build_s27(p):
    """schema_matrix — decision matrix 7 axes × 7 architectures. PEAK density."""
    s = blank(p)
    slide_title(s, "Матрица выбора.", size=26, y=0.34, h=0.62)
    text_box(s, 0.55, 0.96, 12.25, 0.32,
             "Матрица — структура для аргумента, не сумма баллов. Цвет: gold = сильная сторона, teal = слабая.",
             size=12.5, italic=True, color=MID)
    cols = ["Ось ↓ \\ Арх →", "Код", "Промпт", "RAG", "PEFT", "Длинн. контекст", "Агент", "Multi"]
    col_w = [2.55] + [1.41] * 7
    rows = [
        ("Объём знания", [("н/п", "n"), ("мало", "n"), ("большой", "g"), ("повед.", "n"), ("средн.", "n"), ("больш.", "g"), ("оч.больш.", "g")]),
        ("Частота изменений", [("н/п", "n"), ("редко", "t"), ("часто", "g"), ("редко", "t"), ("средн.", "n"), ("часто", "g"), ("часто", "g")]),
        ("Свежесть / провенанс", [("детерм.", "g"), ("нет", "t"), ("да·цит.", "g"), ("нет", "t"), ("частич.", "n"), ("да", "g"), ("да", "g")]),
        ("Стоимость (отн.)", [("мин", "g"), ("низк.", "g"), ("средн.", "n"), ("раз.выс.", "t"), ("средн.", "n"), ("×чата", "t"), ("×много", "t")]),
        ("Латентность", [("мин", "g"), ("низк.", "g"), ("+retr.", "n"), ("низк.", "g"), ("выше", "n"), ("высок.", "t"), ("оч.выс.", "t")]),
        ("Аудируемость", [("высок.", "g"), ("средн.", "n"), ("ср-выс.", "g"), ("низк.", "t"), ("средн.", "n"), ("низк.", "t"), ("оч.низ.", "t")]),
        ("Риск недетерм./петель", [("нет", "g"), ("низк.", "g"), ("низк.", "g"), ("низк.", "g"), ("низк.", "g"), ("высок.", "t"), ("оч.выс.", "t")]),
    ]
    tx, ty = 0.55, 1.40
    hh = 0.52
    rh = 0.50
    # Ocean motif wrapper behind the matrix (drawn first, behind cells)
    ocean_box(s, 0.40, 1.26, 12.55, hh + 7 * rh + 0.28)
    # header
    cx = tx
    for j, c in enumerate(cols):
        filled_rect(s, cx, ty, col_w[j], hh, MID if j else DEEP, radius=False)
        text_box(s, cx + 0.05, ty, col_w[j] - 0.10, hh, c,
                 size=(11 if j else 11), bold=True, color=WHITE,
    ...
    for ri, (axis, cells) in enumerate(rows):
        ...
        for j, (txt, sem) in enumerate(cells):
            if sem == "g":
                bg, fg = GOLD_TINT, DEEP
            elif sem == "t":
                bg, fg = TEAL_TINT, TEAL
            else:
                bg, fg = WHITE, SLATE
            filled_rect(s, cx, yy, col_w[j + 1], rh, bg, stroke=SOFT_GREY,
                        stroke_pt=0.75)
            text_box(s, cx + 0.03, yy, col_w[j + 1] - 0.06, rh, txt,
                     size=11, bold=(sem == "g"), color=fg,
                     align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE,
                     line_spacing=0.95)
            cx += col_w[j + 1]
        yy += rh
    gold_callout(s, 0.55, yy + 0.12, 12.25, 1.0,
                 "Детерминированная, верифицируемая, повторяемая задача (фикс. цена, политика, парсинг, валидация по схеме, арифметика, маршрутизация) → обычный код, без ИИ. ИИ добавил бы только недетерминизм + стоимость + латентность + поверхность для prompt injection.",
                 size=13)
    footer(s, "Матрица — структура для аргумента: назови решающую ось, обоснуй по ней, проверь остальные на блокеры.")
...
def build_s30(p):
    """summary — bridge + homework + Q&A."""
    s = blank(p)
    slide_title(s, "Мост к отраслям + задание.", size=27)
    # bridge
    bx, by, bw, bh = 0.55, 1.35, 12.25, 1.55
    ocean_box(s, bx, by, bw, bh)
    text_box(s, bx + 0.30, by + 0.18, bw - 0.6, 0.42,
    ...
    text_box(s, bx + 0.30, by + 0.70, bw - 0.6, 0.78,
             "Дальше — отраслевые Лекции 4–17: в каждой один вопрос в новой обёртке — «какая архитектура и почему именно она, и при каком условии ответ был бы другим»",
             size=13, color=DEEP, line_spacing=1.16)
    # homework block
    hx, hy, hw, hh = 0.55, 3.10, 7.85, 2.85
    ocean_box(s, hx, hy, hw, hh, fill=GOLD_TINT, stroke=GOLD, stroke_pt=2.0)
    icon(s, "clipboard-list", hx + 0.26, hy + 0.24, 0.52, "gold")
    text_box(s, hx + 0.92, hy + 0.26, hw - 1.1, 0.5, "Задание — Семинар 3",
             size=18, bold=True, color=DEEP, anchor=MSO_ANCHOR.MIDDLE)
    text_box(s, hx + 0.30, hy + 0.92, hw - 0.6, 0.5,
             "«Архитектурный выбор: чат / агент / RAG / API — 3 кейса»",
             size=14, bold=True, color=MID, line_spacing=1.1)
    text_box(s, hx + 0.30, hy + 1.45, hw - 0.6, 1.3,
             "Применить восьмишаговый чек-лист к 3 кейсам; для каждого: архитектура + ≥2 причины по осям матрицы + ≥1 условие смены выбора + критерий «здесь это было бы НЕ так». Mini-apply задача B (s28) — разминка.",
             size=12, color=DEEP, line_spacing=1.18)
    # Q&A
    qx, qy, qw, qh = 8.65, 3.10, 4.15, 2.85
    ocean_box(s, qx, qy, qw, qh, fill=TEAL_TINT, stroke=TEAL, stroke_pt=2.0)
    text_box(s, qx, qy + 0.55, qw, 1.1, "Q&A",
             size=60, bold=True, color=TEAL, align=PP_ALIGN.CENTER)
    text_box(s, qx, qy + 1.85, qw, 0.55, "Спасибо за внимание",
             size=16, italic=True, color=DEEP, align=PP_ALIGN.CENTER)

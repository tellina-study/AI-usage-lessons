#!/usr/bin/env python3
"""Reproducible chart generation for Лекция 4 v4 deck (QuickChart API → PNG).

Ocean palette LOCKED: deep #21295C / mid #065A82 / light #1C7293 /
teal #028090 / gold #F0AB00 / surface #F4F7FA / grid #E5EAF0.

v4 charts (functional, embedded via band builders):
  c01  s01  METR perception-gap (predict -24 / believe -20 / actual +19) diverging
  c20  s20  GitClear before→after (clones / refactor / churn) grouped
  c21  s21  SWE-bench Verified ~88 vs Pro ~64
  c24  s24  Meta coverage↑(32/5.3) vs mutation↓(2.4/15) grouped
  c33  s33  DORA both halves (+7.5 docs / -7.2 stability) diverging
  c39  s39  Anthropic quiz 67 (без AI) vs 50 (с AI) bar

Run: python3 gen_charts_v4.py
"""
import json
...
GRID = "#E5EAF0"
RED = "#C0503A"   # NOT palette — avoid; use TEAL/GOLD for negative accents
...
def datalabels(fmt="v", size=22, color=DEEP, off=6, anchor="end", align="end"):
    return {
        "anchor": anchor, "align": align, "offset": off,
        "color": color, "font": {"size": size, "weight": "bold"},
        "formatter": None,
    }
        ...
        "chart": {
            ...
            "options": {
                "plugins": {
                    ...
                    "title": {
                        "display": True,
                        "text": "Ожидание ускорения vs факт замедления, %",
            ...
            "options": {
                "plugins": {
                    "legend": {"labels": {"font": {"size": 20}},
                               "position": "top"},
                    "title": {"display": True,
                              "text": "GitClear · 211 млн строк · маркеры техдолга, %",
        ...
        "chart": {
            ...
            "options": {
                "plugins": {
                    "legend": {"display": False},
                    "title": {"display": True,
                              "text": "SWE-bench: разрыв ~24 п.п., % решённых",
...
# ── c24 — Meta coverage↑ (32 vs 5.3) but mutation↓ (2.4 vs 15) ──
def c24_meta():
    spec = {
        ...
        "chart": {
            "type": "bar",
            "data": {
                "labels": ["Покрытие классов", "Убито мутантов"],
                "datasets": [
                    {"label": "LLM-генерация", "data": [32, 2.4],
                     "backgroundColor": GOLD, "borderRadius": 6},
                    {"label": "Узко-целевой метод", "data": [5.3, 15],
            ...
            "options": {
                "plugins": {
                    "legend": {"labels": {"font": {"size": 19}},
                               "position": "top"},
                    "title": {"display": True,
                              "text": "Meta · больше покрытия ≠ больше найденных дефектов, %",
...
def c33_dora():
    spec = {
        ...
        "chart": {
            "type": "bar",
            "data": {
                "labels": ["Документация", "Стабильность\nдоставки"],
                "datasets": [{
                    "data": [7.5, -7.2],
...
# ── c39 — Anthropic quiz: 67 (без AI) vs 50 (с AI) ──
def c39_anthropic():
    spec = {
        ...
        "chart": {
            ...
            "options": {
                "plugins": {
                    "legend": {"display": False},
                    "title": {"display": True,
                              "text": "Квиз на понимание, % (RCT, n=52)",
                              "font": {"size": 23, "weight": "bold"},
                              "color": DEEP, "padding": 14},
                    "datalabels": {"anchor": "center", "align": "center",
                                   "color": "white",

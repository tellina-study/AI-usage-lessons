...
def build_s04(p):
    ...
    # Phase 6.5: title font reduced 20pt → 16pt so it fits one line in 5.6" container.
...
def build_s05a(p):
    ...
    text_box(s, x=left_x + 0.3, y=card_y + mono_size + 1.4, w=left_w - 0.6, h=0.4,
             text="инициалы — заполняются при публикации",
             size=11, italic=True, color=LIGHT, align=PP_ALIGN.CENTER)
    items = [
        ("lucide-briefcase-blue.png", "Опыт с AI", "10+ лет",
         "исследовательская и инженерная практика — ML / NLP / генеративные модели.", TEAL),
        ("lucide-lightbulb-blue.png", "Почему этот курс важен", None,
         "разрыв между шумом про AI и реальной инженерной практикой — заполняем именно его.", MID),
        ("lucide-users-blue.png", "О себе", None,
         "соавтор материалов по AI для инженерных вузов; читаю серьёзно, но без пафоса.", MID),
    ]
...
def build_s05b(p):
    ...
    box_y = 2.05
    box_h = 4.7
    # Single rounded box — both takeaway and central question inside (P1-G fix)
    ocean_box(s, right_x, box_y, right_w, box_h, stroke=TEAL)
    # Top half — main takeaway
    text_runs(s, x=right_x + 0.35, y=box_y + 0.35, w=right_w - 0.7, h=2.6, runs=[
        ...
        {"text": "этот разрыв.", "size": 24, "color": MID, "bold": True},
    ], line_spacing=1.35)
    # Bottom half — central question; «нет» as deep-bold-italic, not gold (P1-G no-red)
    text_runs(s, x=right_x + 0.35, y=box_y + 3.10, w=right_w - 0.7, h=1.4, runs=[
        {"text": "Где AI ", "size": 24, "color": DEEP, "bold": True},
        {"text": "работает", "size": 24, "color": MID, "bold": True},
        {"text": ", где —\n", "size": 24, "color": DEEP, "bold": True},
        {"text": "нет", "size": 24, "color": DEEP, "bold": True, "italic": True},
        {"text": ", и как это понять?", "size": 24, "color": DEEP, "bold": True},
    ], line_spacing=1.3)
    # Funnel disclaimer (P1-J)
    text_box(s, x=funnel_x, y=funnel_y + funnel_h + 0.05, w=funnel_w, h=0.4,
             text="Иллюстрация принципа, не реальная статистика. Сама статистика «5–10% доходят» — в стейксе наверху.",
             size=10, italic=True, color=LIGHT, line_spacing=1.25, align=PP_ALIGN.CENTER)
    speaker_notes(s, "Центральная рамка курса. Возвращаемся в s14, s18, s27. Стейкс обновлён v5: Gartner окт 2024 (80% workforce upskill GenAI by 2027); CNews/Vedomosti/Intellectual Analytics март 2026 (90% не доходят до full industrial deployment). Funnel — иллюстрация принципа (100/-90%/10), не реальная статистика; настоящая цифра в стейксе. Single rounded box (P1-G fix: убран thin-line divider). Tone — исследовательский / навигационный.")
...
def build_s09(p):
    ...
    tiles = [
        ...
        ("46%", "кода у юзеров Copilot", "GitHub Copilot 2025", "telemetry · Java 61%", MID),
        ("$244–390B", "AI-рынок", "Statista / McKinsey 2025", "разные методологии оценки", TEAL),
    ]
    ...
    footer_sources(s, "OpenAI 2026  ·  Stack Overflow Dev Survey 2025  ·  GitHub Copilot 2025 (telemetry)  ·  Statista / McKinsey 2025  ·  CNews/Vedomosti 2026.")
...
def build_s10(p):
    ...
    s = blank(p)
    slide_title(s, "В AI побеждает изобретательность: $5.6M marginal, $589B потерь капитализации Nvidia за один день.")
    ...
    anchors = [
        ("26 декабря 2024", "DeepSeek-V3", "$5.6M", "marginal training run only",
         "FULL_INFRA", MID),  # special marker for v1 caveat callout
    ...
    for i, (date, name, num, label, body, color) in enumerate(anchors):
        ...
        text_box(s, x=x + 0.2, y=card_y + 1.05, w=card_w - 0.4, h=0.95,
                 text=num, size=44, bold=True, color=color, align=PP_ALIGN.CENTER,
                 line_spacing=1.0)
        # Label — larger now for v1 ("marginal training run only" emphasized)
        label_size = 14 if i == 0 else 13
        text_box(s, x=x + 0.2, y=card_y + 1.95, w=card_w - 0.4, h=0.40,
                 text=label, size=label_size, bold=True, color=DEEP,
                 align=PP_ALIGN.CENTER, line_spacing=1.2)
        if body == "FULL_INFRA":
            # Separate visually-prominent caveat callout for full infra cost
            # Gold-tinted band inside card with stronger weight
            cv_y = card_y + 2.45
            filled_rect(s, x + 0.20, cv_y, card_w - 0.4, 0.80, GOLD_TINT,
                        stroke=GOLD, stroke_pt=1.5, radius=True, radius_adj=0.12)
            text_box(s, x=x + 0.25, y=cv_y + 0.08, w=card_w - 0.5, h=0.28,
                     text="FULL INFRA COST",
                     size=10, bold=True, color=GOLD, align=PP_ALIGN.CENTER)
            text_box(s, x=x + 0.25, y=cv_y + 0.35, w=card_w - 0.5, h=0.40,
                     text="$1.3 — 1.6 млрд",
                     size=18, bold=True, color=DEEP, align=PP_ALIGN.CENTER,
                     line_spacing=1.0)
        else:
            text_box(s, x=x + 0.2, y=card_y + 2.50, w=card_w - 0.4, h=0.80,
...
def build_s11(p):
    ...
    right_w = 13.333 - 0.55 - right_x
    text_runs(s, x=right_x, y=1.65, w=right_w, h=4.0, runs=[
    ...
    # Anti-pattern emphasis callout (P2: «слои, не альтернативы»)
    filled_rect(s, right_x, 5.70, right_w, 0.65, GOLD_TINT, stroke=GOLD,
                stroke_pt=1.5, radius=True, radius_adj=0.10)
    text_runs(s, x=right_x + 0.20, y=5.78, w=right_w - 0.40, h=0.50, runs=[
        {"text": "СЛОИ, НЕ АЛЬТЕРНАТИВЫ:  ", "size": 12, "bold": True, "color": GOLD},
        {"text": "каждый следующий ", "size": 13, "color": DEEP, "bold": True},
        {"text": "включает", "size": 13, "color": DEEP, "bold": True, "italic": True},
        {"text": " предыдущий.", "size": 13, "color": DEEP, "bold": True},
    ], line_spacing=1.25)
...
def build_s12(p):
    ...
    # P2-1 v4: footer-tax «Demo: live + видео-backup. Код: assets/code/three-ways/.» удалён со слайда.
    # Информация про backup и путь к коду переехала в speaker notes (anti-pattern #14 footer-tax).

    speaker_notes(s, "Hybrid live + video. Зафиксированная задача: «HTML-страница с графиком продаж за 2025». 30 сек screencast model → 30 сек screencast chat → 2 мин live agent (Claude Code). 2 мин разбор таблицы. Чему учит: тот же AI («одна модель внутри») даёт разные результаты в зависимости от обвязки. Выбор обвязки = инженерное решение под задачу. Backup: 3 скриншота + полное видео всех трёх способов. Код демо: `library/lectures/lec-01/assets/code/three-ways/`. Задача протестирована до 14.04.2026.")
...
def build_s14(p):
    """s14 — Chat case: hero «диалоговая обвязка» — large case + definition (no duplicate bar chart)."""
    s = blank(p)
    slide_title(s, "Чат = модель + UI + память. Большинство откатившихся пилотов выбрали чат там, где нужна модель или агент.")

    # Big hero case card (full width)
    case_x, case_y, case_w, case_h = 0.55, 1.55, 12.25, 4.10
    ocean_box(s, case_x, case_y, case_w, case_h)

    # Left half — case
    add_image(s, ASSETS / "icons/lucide-message-square-blue.png",
              x=case_x + 0.40, y=case_y + 0.40, w=0.85, h=0.85)
    text_box(s, x=case_x + 1.45, y=case_y + 0.45, w=4.5, h=0.4,
             text="КЕЙС  ·  диалоговая обвязка LLM",
             size=14, bold=True, color=MID)
    text_box(s, x=case_x + 0.40, y=case_y + 1.45, w=5.55, h=1.6,
             text="Разобрать непонятный нормативный документ и собрать чек-лист требований",
             size=22, bold=True, color=DEEP, line_spacing=1.25)
    text_runs(s, x=case_x + 0.40, y=case_y + 3.10, w=5.55, h=0.85, runs=[
        {"text": "Решение:  ", "size": 14, "color": LIGHT, "italic": True},
        {"text": "ЧАТ", "size": 16, "bold": True, "color": MID},
        {"text": "  ·  ", "size": 14, "color": LIGHT},
        {"text": "не нужна модель ", "size": 12, "color": LIGHT, "italic": True},
        {"text": "(нет потока)", "size": 12, "color": DEEP},
        {"text": "  ·  ", "size": 14, "color": LIGHT, "newpara": False},
        {"text": "не нужен агент ", "size": 12, "color": LIGHT, "italic": True},
        {"text": "(нет tool use)", "size": 12, "color": DEEP},
    ], line_spacing=1.5)

    # Vertical separator
    filled_rect(s, case_x + 6.10, case_y + 0.40, 0.025, case_h - 0.80, fill=LIGHT)

    # Right half — definition + key feature
    text_box(s, x=case_x + 6.45, y=case_y + 0.45, w=5.4, h=0.4,
             text="ЧТО ДЕЛАЕТ ЭТУ ЗАДАЧУ ЧАТОВОЙ",
             size=14, bold=True, color=TEAL)
    text_runs(s, x=case_x + 6.45, y=case_y + 1.05, w=5.4, h=2.7, runs=[
        {"text": "Память контекста.  ", "size": 16, "bold": True, "color": DEEP},
        {"text": "История диалога — в окне модели; уточнения накапливаются.",
         "size": 13, "color": LIGHT, "italic": True},
        {"text": "Итеративное уточнение.  ", "size": 16, "bold": True, "color": DEEP, "newpara": True},
        {"text": "Вопрос → ответ → «копни глубже п. 3» → ответ. До приемлемого результата за 5-10 ходов.",
         "size": 13, "color": LIGHT, "italic": True},
        {"text": "Без потока и без инструментов.  ", "size": 16, "bold": True, "color": DEEP, "newpara": True},
        {"text": "Один документ, один разработчик, разовая задача — типовой чатовый кейс.",
         "size": 13, "color": LIGHT, "italic": True},
    ], line_spacing=1.5)

    # Gold callback to central question
    gold_callout(s, 0.55, 5.85, 12.25, 0.85,
                 "«Где AI работает» = выбор типа взаимодействия под задачу. Чек-лист в s18.",
                 size=15, bold=True)

    footer_sources(s, "Dam et al. 2024 (LLM-чатботы)  ·  типология взаимодействия — глава §3.3.")
    speaker_notes(s, "Чат — модель + UI + память диалога. Кейс ТЗ — типовой для чата. Память контекста + итеративное уточнение — два ключевых свойства, которые модель в одиночку не даёт. Большинство откатов = чат там, где нужна модель (поток) или агент (tool use). Возврат к central question. Chapter §3.3. Bar chart с долями LLM удалён — он уже был на s04, дубль убивал внимание (Phase 7 student-simulator).")
...
def build_s15(p):
    ...
    # Unified Ocean teal (LIGHT #1C7293) strokes — visual motif consistency (P1-H fix).
    # Winning card (РОЛЬ A) gets gold dot + slightly thicker stroke as marker.
    cols = [
        ("ПЛОХОЙ ПРОМПТ", LIGHT, LIGHT, False,
         "«Расскажи про AI в медицине»",
         "Расплывчатый текст уровня обзорной статьи."),
        ("РОЛЬ A — McKinsey", MID, LIGHT, True,  # winning card → gold marker
        ...
        ("РОЛЬ B — контраст", TEAL, LIGHT, False,
         "«Ты — не-специалист, далёкий от технологий. Тот же вопрос.»",
         "Радикально другой стиль и приоритеты — упрощения, метафоры."),
    ]
    for i, (name, header_color, stroke_color, is_winner, prompt, result) in enumerate(cols):
        x = 0.55 + i * (col_w + col_gap)
        # Winning card — thicker stroke
        stroke_pt = 2.0 if is_winner else 1.5
        ocean_box(s, x, col_y, col_w, col_h, stroke=stroke_color, stroke_pt=stroke_pt)
        # Gold dot for winner (above header)
        if is_winner:
            dot = filled_rect(s, x + col_w/2 - 0.10, col_y - 0.10, 0.20, 0.20,
                              GOLD, radius=True, radius_adj=0.5)
        text_box(s, x=x + 0.2, y=col_y + 0.18, w=col_w - 0.4, h=0.4,
                 text=name, size=13, bold=True, color=header_color, align=PP_ALIGN.CENTER)
        # Gold underline for winner header
        if is_winner:
            filled_rect(s, x + col_w/2 - 0.8, col_y + 0.55, 1.6, 0.04, fill=GOLD)
        ...
        text_box(s, x=x + 0.25, y=col_y + col_h - 0.7, w=col_w - 0.5, h=0.6,
                 text=result, size=12, bold=True, color=header_color, line_spacing=1.3)
...
def build_s16(p):
    ...
    # 5 level rows — Ocean gradient: levels 1-4 in Ocean shades, level 5 GOLD highlight
    # Decreasing saturation: 1=DEEP, 2=MID, 3=LIGHT, 4=TEAL, 5=GOLD (autonomy peak)
    levels = [
        ("1", "Operator",     "одобряет каждый шаг",   "Claude Code «approve each»",      DEEP),
        ("2", "Collaborator", "работают совместно",    "парное прог. с Cursor",           MID),
        ("3", "Consultant",   "корректирует план",     "Devin предлагает декомпозицию",   LIGHT),
        ("4", "Approver",     "утверждает узлы",       "агент собирает PR → ваш merge",   TEAL),
        ("5", "Observer",     "только наблюдает итог", "AutoGPT на ночь → отчёт",         GOLD),
    ]
    row_h = 0.78  # increased spacing
    rows_start_y = levels_y + 1.00
    for i, (num, name, role, example, color) in enumerate(levels):
        ry = rows_start_y + i * row_h
        # Number badge — Ocean gradient by level, gold for level 5
        badge_color = color
        badge_text_color = DEEP if num == "5" else WHITE
        badge = filled_rect(s, right_x + 0.25, ry + 0.05, 0.55, 0.55, badge_color,
                           radius=True, radius_adj=0.5)
        text_box(s, x=right_x + 0.25, y=ry + 0.05, w=0.55, h=0.55,
                 text=num, size=18, bold=True, color=badge_text_color,
                 align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
        # Slightly larger name for level 5
        name_size = 14 if num == "5" else 13
        text_box(s, x=right_x + 0.90, y=ry + 0.02, w=2.0, h=0.35,
                 text=name, size=name_size, bold=True, color=color)
        text_box(s, x=right_x + 0.90, y=ry + 0.38, w=right_w - 1.15, h=0.32,
                 text=f"{role}  ·  {example}",
...
def build_s17(p):
    ...
    # P2-3 v4: each tile now has a real logo above the text label.
    # logo=None → text-only fallback (Алиса — no LobeHub/simple-icons asset found).
    apps = [
        ("Google Translate", "logo-googletranslate.png"),
        ("DeepL", "logo-deepl.png"),
        ("Grammarly", "logo-grammarly.png"),
        ("Notion AI", "logo-notion.png"),
        ("Copilot inline", "logo-copilot.png"),
        ("Яндекс.Навигатор", "logo-yandex.png"),
        ("Adobe Firefly", "logo-adobefirefly.png"),
        ("Алиса", None),  # fallback — no clean asset, оставлен text-only
    ...
    logo_h = 0.5
    for i, (app, logo) in enumerate(apps):
        row, col = i // 3, i % 3
        ...
        if logo is not None:
            # Logo на верхней половине ячейки
            logo_x = x + (cell_w - logo_h) / 2
            logo_y = y + cell_gap + 0.08
            add_image(s, ASSETS / "icons" / logo,
                      x=logo_x, y=logo_y, w=logo_h, h=logo_h)
            # Текст под лого
            text_box(s, x=x + cell_gap + 0.05, y=y + cell_gap + logo_h + 0.12,
                     w=cell_w - 2*cell_gap - 0.1,
                     h=cell_h - 2*cell_gap - logo_h - 0.15,
                     text=app, size=10, bold=True, color=DEEP,
                     align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.TOP, line_spacing=1.15)
        else:
            # Text-only fallback (centered)
            text_box(s, x=x + cell_gap + 0.05, y=y + cell_gap,
                     w=cell_w - 2*cell_gap - 0.1,
                     h=cell_h - 2*cell_gap,
                     text=app, size=11, bold=True, color=DEEP,
...
def build_s18(p):
    ...
    # Quadrants — single-line archetype labels (Phase 6.5 fix #1: ПРИЛОЖЕНИЕ no longer wraps).
    # Font reduced 14pt → 12pt + slightly wider text box so the longest label fits in one line.
    quads = [
        (0, 0, "Высокий контроль\nВысокий детерм.", "МОДЕЛЬ", MID),
        (0, 1, "Высокий UX-контроль\nСредний выход", "ПРИЛОЖЕНИЕ", TEAL),
...
def build_s21(p):
    ...
    # Right — Vectara HHEM range chart (native shapes — clean labels + axis)
    right_x = left_x + left_w + 0.3
    right_w = 13.333 - 0.55 - right_x
    h_y, h_h = 1.6, 3.55
    ocean_box(s, right_x, h_y, right_w, h_h, stroke=TEAL)
    text_box(s, x=right_x + 0.2, y=h_y + 0.12, w=right_w - 0.4, h=0.35,
             text="VECTARA HHEM — диапазон % галлюцинаций по типам задач",
             size=12, bold=True, color=TEAL, align=PP_ALIGN.CENTER, line_spacing=1.2)

    # 4 task rows: name (left), range bar (right), value text inside bar
    tasks = [
        ("Reasoning models  (o-series, Claude reasoning)", 8.0, 15.0, GOLD),
        ("Open-ended generation",                            5.0, 8.0,  LIGHT),
        ("QA over context  (Claude 4.5, GPT-5)",             1.0, 3.0,  MID),
        ("Standard summarization  (Gemini 2.0 Flash, GPT-5)", 0.4, 1.0,  DEEP),
    ]
    # axis goes 0..16 on right portion of card
    axis_x0 = right_x + 0.30           # name labels left of this
    label_w = 2.55
    bar_x0  = axis_x0 + label_w + 0.10  # bar starts here
    bar_x_end = right_x + right_w - 0.30
    axis_max = 16.0
    bar_span = bar_x_end - bar_x0

    # Axis line + ticks (0,4,8,12,16)
    axis_y = h_y + h_h - 0.55
    filled_rect(s, bar_x0, axis_y, bar_span, 0.012, fill=LIGHT)
    for tick in [0, 4, 8, 12, 16]:
        tx = bar_x0 + (tick / axis_max) * bar_span
        filled_rect(s, tx - 0.005, axis_y, 0.01, 0.07, fill=LIGHT)
        text_box(s, x=tx - 0.25, y=axis_y + 0.10, w=0.5, h=0.25,
                 text=f"{tick}%", size=9, color=LIGHT, italic=True, align=PP_ALIGN.CENTER)

    # Rows
    rows_top = h_y + 0.55
    rows_h   = (axis_y - 0.10) - rows_top
    row_pitch = rows_h / len(tasks)
    bar_h = 0.30
    for i, (name, lo, hi, color) in enumerate(tasks):
        ry = rows_top + i * row_pitch + (row_pitch - bar_h) / 2.0
        # task name (right-aligned label)
        text_box(s, x=axis_x0, y=ry - 0.04, w=label_w, h=bar_h + 0.08,
                 text=name, size=9, color=DEEP, bold=True,
                 align=PP_ALIGN.RIGHT, anchor=MSO_ANCHOR.MIDDLE,
                 line_spacing=1.15)
        # range bar
        x_lo = bar_x0 + (lo / axis_max) * bar_span
        x_hi = bar_x0 + (hi / axis_max) * bar_span
        bar = filled_rect(s, x_lo, ry, max(0.05, x_hi - x_lo), bar_h, color,
                          radius=True, radius_adj=0.5)
        # value text
        if hi - lo >= 2.0:
            txt = f"{lo:g}–{hi:g}%"
            ...
            txt = f"{lo:g}–{hi:g}%"
        text_box(s, x=x_hi + 0.07, y=ry - 0.04, w=1.4, h=bar_h + 0.08,
                 text=txt, size=10, bold=True, color=color,
                 anchor=MSO_ANCHOR.MIDDLE)

    text_box(s, x=right_x + 0.2, y=h_y + h_h - 0.20, w=right_w - 0.4, h=0.20,
             text="Vectara HHEM 2025-26 · github.com/vectara/hallucination-leaderboard",
    ...
    # Retrieval-moment gold callout (P1-E: declared in deck.yaml but invisible in v1)
    rm_y, rm_h = 6.40, 0.55
    filled_rect(s, 0.55, rm_y, 12.25, rm_h, GOLD_TINT, stroke=GOLD,
                stroke_pt=1.5, radius=True, radius_adj=0.10)
    text_runs(s, x=0.85, y=rm_y + 0.10, w=11.55, h=0.35, runs=[
        {"text": "RETRIEVAL-УПРАЖНЕНИЕ:  ", "size": 12, "bold": True, "color": GOLD},
        {"text": "попросите AI 3 статьи с DOI по вашей теме — проверьте каждый в Google Scholar. ",
         "size": 13, "color": DEEP, "bold": True},
        {"text": "Сколько найдётся?", "size": 13, "color": DEEP, "bold": True, "italic": True},
    ])

    # Sopostav data at bottom (smaller, moved below retrieval callout)
    text_box(s, x=0.55, y=7.05, w=12.25, h=0.35,
             text="~38% сотрудников делятся sensitive info с AI без ведома работодателя.  CybSafe & NCA «Oh Behave!» 2024-25.",
...
def build_s22(p):
    ...
    cards = [
        ...
        ("SYCOPHANCY", "УГОДЛИВОСТЬ",
         "RLHF*-разметка учит модель соглашаться с пользователем.",
         "Каноник: GPT-4o, апр 2025 — 25 релиз → 28 rollback → 29 postmortem. «Навязчиво-льстящая».",
         "Самая незаметная",
...
def build_s23(p):
    ...
    # Top — native horizontal bar chart in motif box (clean labels, no overlap)
    chart_x, chart_y = 0.55, 1.55
    chart_w, chart_h = 12.25, 3.15
    ocean_box(s, chart_x, chart_y, chart_w, chart_h, stroke=MID)
    text_box(s, x=chart_x + 0.2, y=chart_y + 0.12, w=chart_w - 0.4, h=0.35,
             text="ARC-AGI-2 — состояние май 2026  ·  оценка vs стоимость",
             size=14, bold=True, color=MID, align=PP_ALIGN.CENTER)

    # 3 rows. label_w | bar | score | cost
    bars = [
        # (label,                                      score%, cost text,         cost color, bar color)
        ("Средний человек",                              60.0, "$50–150 / час",   DEEP, MID),
        ("Refinement (Gemini 3 Pro + Poetiq)",           54.0, "$30 / задачу",    GOLD, GOLD),
        ("Single-model (Opus 4.5 Thinking)",             37.6, "$2.20 / задачу",  TEAL, TEAL),
    ]
    label_w = 3.55
    label_x = chart_x + 0.30
    bar_x0  = label_x + label_w + 0.10
    score_w = 0.95
    cost_w  = 1.85
    bar_x_end = chart_x + chart_w - score_w - cost_w - 0.40
    bar_span = bar_x_end - bar_x0

    rows_top = chart_y + 0.65
    rows_bottom = chart_y + chart_h - 0.55
    row_pitch = (rows_bottom - rows_top) / len(bars)
    bar_h = 0.45
    ...
    for tick in [0, 20, 40, 60, 80, 100]:
        tx = bar_x0 + (tick / 100.0) * bar_span
        filled_rect(s, tx - 0.005, axis_y, 0.01, 0.06, fill=LIGHT)
        text_box(s, x=tx - 0.25, y=axis_y + 0.08, w=0.5, h=0.20,
                 text=f"{tick}", size=8, color=LIGHT, italic=True, align=PP_ALIGN.CENTER)

    for i, (name, score, cost_text, cost_col, bar_col) in enumerate(bars):
        ry = rows_top + i * row_pitch + (row_pitch - bar_h) / 2.0
        # name label (right-aligned)
        text_box(s, x=label_x, y=ry - 0.02, w=label_w, h=bar_h + 0.04,
                 text=name, size=11, color=DEEP, bold=True,
                 align=PP_ALIGN.RIGHT, anchor=MSO_ANCHOR.MIDDLE)
        # bar
        bw = (score / 100.0) * bar_span
        filled_rect(s, bar_x0, ry, max(0.05, bw), bar_h, bar_col,
                    radius=True, radius_adj=0.4)
        # score % to right of bar
        text_box(s, x=bar_x0 + bw + 0.08, y=ry - 0.02, w=score_w, h=bar_h + 0.04,
                 text=f"{score:g}%", size=14, bold=True, color=bar_col,
                 anchor=MSO_ANCHOR.MIDDLE)
        # cost to far right (own column, no overlap)
        text_box(s, x=chart_x + chart_w - cost_w - 0.20, y=ry - 0.02,
                 w=cost_w, h=bar_h + 0.04,
                 text=cost_text, size=11, bold=True, color=cost_col,
    ...
    text_box(s, x=0.55, y=in_y + 0.42, w=12.25, h=0.35,
             text="Состояние май 2026; arcprize.org обновляется — moving target. Chollet 2019 (базовая работа).",
             size=11, italic=True, color=LIGHT)
...
def build_s24(p):
    ...
    # 4 leaders — full names (P1-K fix) — alternating above/below pattern
    leaders = [
        (0.05, "Sam Altman",    "CEO OpenAI",          "~5 лет",      "AI 100sM users", GOLD),
        (0.30, "Dario Amodei",  "CEO Anthropic",       "2-3 года",    "конкурент OpenAI", GOLD),
        (0.65, "Demis Hassabis", "CEO Google DeepMind", "50% в декаде", "Нобель 2024, диверсиф.", MID),
        (0.95, "Yann LeCun",    "AMI Labs, ex-Meta",   "не на LLM",   "AMI Labs $1B март 2026", TEAL),
    ]
...
def build_s25(p):
    ...
    # Reduced from 5 to 3 strongest (P1-B density fix)
    text_runs(s, x=left_x + 0.25, y=col_y + 0.95, w=sub_w - 0.5, h=col_h - 1.1, runs=[
        {"text": "•  Скорость", "size": 14, "bold": True, "color": DEEP},
        {"text": "  Deep Blue 200M поз/сек (1997)", "size": 12, "color": LIGHT, "italic": True, "newpara": True},
        {"text": "•  Распознавание паттернов", "size": 14, "bold": True, "color": DEEP, "newpara": True},
        {"text": "  ResNet 3.57% vs human 5.1% ImageNet", "size": 12, "color": LIGHT, "italic": True, "newpara": True},
        {"text": "•  Объём генерации", "size": 14, "bold": True, "color": DEEP, "newpara": True},
        {"text": "  46% кода у юзеров Copilot", "size": 12, "color": LIGHT, "italic": True, "newpara": True},
    ], line_spacing=1.7)
    ...
    # Reduced from 4 to 3 strongest (P1-B density fix)
    text_runs(s, x=hb_x + 0.25, y=col_y + 0.95, w=sub_w - 0.5, h=col_h - 1.1, runs=[
        {"text": "•  Причинность (Pearl 1→3)", "size": 14, "bold": True, "color": DEEP},
        {"text": "  3 уровня — справа →", "size": 12, "color": LIGHT, "italic": True, "newpara": True},
        {"text": "•  Абстрактное обобщение", "size": 14, "bold": True, "color": DEEP, "newpara": True},
        {"text": "  по 2-3 примерам (ARC-AGI s23)", "size": 12, "color": LIGHT, "italic": True, "newpara": True},
        {"text": "•  Целеполагание", "size": 14, "bold": True, "color": DEEP, "newpara": True},
        {"text": "  AI оптимизирует, человек ставит цель", "size": 12, "color": LIGHT, "italic": True, "newpara": True},
    ], line_spacing=1.7)
    ...
    # 3 levels (top→bottom: 3, 2, 1) — Ocean gradient (deep → mid → light), white text inside
    levels_pearl = [
    ...
    for i, (num, en_name, ru_name, example, ai_status, color) in enumerate(levels_pearl):
        ...
        # Tile (Ocean gradient — solid fill in respective Ocean shade)
        filled_rect(s, right_x + 0.2, ly, right_w - 0.4, lvl_h - 0.05,
                    color, stroke=None, radius=True, radius_adj=0.1)
        # Number badge (gold for top tier, white-on-deep for others)
        badge_color = GOLD if num == "3" else WHITE
        badge_text_color = DEEP if num == "3" else color
        bd = filled_rect(s, right_x + 0.3, ly + 0.15, 0.45, 0.45, badge_color,
                        radius=True, radius_adj=0.5)
        text_box(s, x=right_x + 0.3, y=ly + 0.15, w=0.45, h=0.45,
                 text=num, size=16, bold=True, color=badge_text_color,
                 align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
        text_box(s, x=right_x + 0.85, y=ly + 0.1, w=right_w - 1.05, h=0.32,
                 text=en_name, size=13, bold=True, color=WHITE)
        text_box(s, x=right_x + 0.85, y=ly + 0.4, w=right_w - 1.05, h=0.3,
                 text=ru_name, size=10, italic=True, color=SURFACE)
        text_box(s, x=right_x + 0.3, y=ly + 0.75, w=right_w - 0.5, h=0.55,
                 text=example, size=10, color=WHITE, italic=True, line_spacing=1.3)
        # Status — small at bottom-right
        text_box(s, x=right_x + 0.3, y=ly + lvl_h - 0.35, w=right_w - 0.5, h=0.25,
                 text=ai_status, size=10, bold=True, color=GOLD if num == "3" else SURFACE,
...
def build_s26(p):
    ...
    # P2-1 v4: footer-tax «Точные группы и темы — в каталоге 00-course/...» удалён со слайда (methodist comment).
    # Информация переехала в speaker notes.

    speaker_notes(s, "Карта 17 лекций в 4 блока. Outcome-oriented: «после блока — что вы сможете делать». Зачем roadmap: после 4 разделов лекции 1 студент видит, как фреймворк раскручивается дальше. Без roadmap лекция 1 — оторванный wow-момент. NB course-curator: точные группы и темы вытащить из `00-course/` (Google Drive `1sHXoLaIqCpBRv1IaLjS6lNtBdwI5cPc0`) субагентом course-curator до финальной сборки; на момент пилота показана предварительная декомпозиция.")
...
def build_s27(p):
    ...
    # P2-4 v4: callback split into 2 boxes (gold question + teal-tinted checklist badge) с visible gap.
    # Gold question box
    cq_y = cb_y + 0.65 + mini_h + 0.55
    cq_h = 1.05
    filled_rect(s, left_x + 0.2, cq_y, left_w - 0.4, cq_h, GOLD_TINT,
                stroke=GOLD, stroke_pt=1.5, radius=True, radius_adj=0.12)
    text_box(s, x=left_x + 0.35, y=cq_y + 0.12, w=left_w - 0.7, h=0.32,
             text="Помните вопрос с s5?",
             size=12, italic=True, color=GOLD, align=PP_ALIGN.CENTER)
    text_box(s, x=left_x + 0.35, y=cq_y + 0.42, w=left_w - 0.7, h=0.55,
             text="«Где AI работает, где — нет, и как это понять?»",
             size=14, bold=True, color=DEEP, align=PP_ALIGN.CENTER, line_spacing=1.25)

    # Separate checklist badge (below, with gap)
    cl_y = cq_y + cq_h + 0.18
    filled_rect(s, left_x + 0.4, cl_y, left_w - 0.8, 0.45, SURFACE,
                stroke=LIGHT, stroke_pt=1.0, radius=True, radius_adj=0.3)
    text_box(s, x=left_x + 0.45, y=cl_y + 0.06, w=left_w - 0.9, h=0.35,
...
def build_s29(p):
    """s29 — Q&A with backup provocations. P2-2 v4: provocation вынесена в visible assertion наверху."""
    s = blank(p)

    # P2-2 v4: Top assertion (provocation) — visible, Ocean DEEP
    text_box(s, x=0.55, y=0.55, w=12.25, h=0.9,
             text="Кто после этой лекции изменил мнение о чём-то?",
             size=28, bold=True, color=DEEP,
             align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE,
             line_spacing=1.15)

    # Big Q&A as subtitle — gold (уменьшено с 200pt → 100pt, освобождено место под assertion)
    qa_y = 1.65
    qa_h = 2.4
    text_box(s, x=0.55, y=qa_y, w=12.25, h=qa_h,
             text="Q&A",
             size=130, bold=True, color=GOLD,
             align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE,
             line_spacing=1.0)

    # Subtitle
    text_box(s, x=0.55, y=qa_y + qa_h + 0.05, w=12.25, h=0.45,
    ...
    provocations = [
        ("Backup 1",
         ...
         MID),
        ("Backup 2",
         "«Что из услышанного вы попробуете до семинара?»",
         TEAL),
    ]
    for i, (label, text, color) in enumerate(provocations):
        ...
        text_box(s, x=x + 0.2, y=bp_y + 0.48, w=bp_w - 0.4, h=0.9,
                 text=text, size=13, italic=True, color=DEEP, line_spacing=1.3)
    ...
    speaker_notes(s, "Главная провокация наверху слайда («Кто после этой лекции изменил мнение о чём-то?») — задайте сразу после открытия Q&A, до первой руки. Это самый сильный hook. Открытый Q&A 2 минуты. Если тишина — backup-провокации (универсальный tone, без «ИУ6»): «коллега» вместо «инженер ИУ6», «что из услышанного попробуете» — переход к действию. Финал: благодарность + напоминание задание s28. Если Q&A провисает — вернуться к самым обсуждаемым слайдам (s10 DeepSeek / s23 ARC-AGI / s24 narrow vs general).")

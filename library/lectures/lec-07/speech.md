...
- **[s01 pre-flight, Chester]** За день до лекции открыть на лекторском ноутбуке `mlmed.org/tools/xray/`. Перетащить тестовый рентген из `library/lectures/lec-07/assets/test-xray.png`. Убедиться, что heatmap появляется и в таблице 18 строк с вероятностями. Если страница не загружается — открыть запасной снимок `library/lectures/lec-07/assets/backup/chester-pneumonia-result.png` в отдельной вкладке и переключаться по Alt+Tab.

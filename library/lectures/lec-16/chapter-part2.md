...
**Что это значит для AI.** Foundation models, обученные на 90 годах данных Saudi Aramco (METABRAIN — мы разберём её в §2.2) или на доступной библиотеке скважин SLB Petrel (Lumi — §2.3), **не обобщаются** на новый бассейн с принципиально иной геологией. Бассейн Tarim в Китае не похож на Пермский. Восточно-африканский рифт не похож ни на тот, ни на другой. Pre-salt Brazil в ранней exploration phase — каждый case unique. AI здесь даёт **screening и acceleration существующих workflow** (foundation model отвечает «вот 5 потенциальных drilling locations, проверь их», и **senior geophysicist делает финальную интерпретацию**), а не «замену геолога». И всё, что обещало большее — провалилось публично (см. §2.5 BP+Beyond Limits и §2.6 IBM+Repsol).

**HPC-гонка как ответ на sparse data.** Если данных мало, но физика известна — это означает, что **симуляционные модели становятся ground truth**, и ускорение симуляций даёт material lift. Поэтому в Q3 наблюдается **гонка high-performance computing** (HPC — высокопроизводительные вычисления): Aramco тренирует METABRAIN на собственных HPC-кластерах; Eni построила HPC6 (Top500 #5 в декабре 2024 года); ExxonMobil развернула Discovery 6 на 4 032 NVIDIA Grace Hopper Superchip. Это **капитальные инвестиции $100–400 млн** на одну компанию. И они не «коммодизируются как hyperscaler cloud» — это **strategic infrastructure**, которая определяет конкурентное преимущество в frontier exploration на годы вперёд.

**Профиль AI в Q3.**

- **Физика — ground truth.** Eclipse, INTERSECT, CMG (компании Computer Modelling Group), OpenFOAM — классические пластовые симуляторы.
- **AI — accelerator и screening tool.** Foundation models, ML surrogates, deep learning поверх сейсмики.
...
- **Aramco выручка 2024 ≈ $440 млрд.** $1,8B / $440B = **0,4% выручки.** AI не «спасает» компанию — добавляет полпроцента к полностью оптимизированной операции.
...
2. **0,4% выручки — это правильный baseline.** Если читать «$1,8 млрд» без знаменателя, кажется огромным. С знаменателем — это инкрементальная оптимизация на масштабе огромной компании.
...
- **Microsoft Azure ML + Google Cloud Vertex AI** — hyperscaler stack с домен-tuning.
- **Internal models** Газпром нефти (Cognitive Geo), CNPC, Petrobras (если развёрнуты).

Выбор между Lumi и альтернативой — это выбор не «между AI и не-AI», а между **вертикально-интегрированным стеком (SLB Lumi + Petrel + Delfi)** и **disaggregated stack (own data + own platform + hyperscaler LLM)**. Для крупного NOC второй вариант часто предпочтительнее (контроль данных, vendor independence). Для среднего IOC или сервисного подрядчика — первый.
...
- **Stabroek Block в Гайане.** ExxonMobil — оператор Stabroek, одного из крупнейших offshore нефтяных открытий 21 века. **Stabroek estimate: ~16 миллиардов BOE** (barrels of oil equivalent — единица измерения, включающая нефть и газ в эквиваленте баррелей нефти). **Discovery 6 unlock $1+ млрд value** на первых **6 FPSO** в Stabroek; FPSO (Floating Production Storage and Offloading — плавучая платформа добычи-хранения-выгрузки нефти) — типичная единица offshore production capacity.
...
В 2018 году BP стала **крупнейшим клиентом и инвестором** компании Beyond Limits — стартапа из Глендейла (Калифорния), выделившегося из NASA Jet Propulsion Laboratory с claim'ом, что технология **«cognitive AI»** для исследовательских миссий deep space теперь применима в нефтегазе. BP вложила **$20 млн**; партнёрство объявлялось как стратегическое.
...
**Второй — single-customer concentrated bet.** BP была **anchor investor + anchor customer**. Это значит: продуктовая дорожная карта Beyond Limits **первоначально оптимизировалась под BP**. Когда BP сократил инвестиции (2020 oil crash) — у Beyond Limits не оказалось диверсифицированной customer base. **Vendor pivot оставил BP без commercial product**, а Beyond Limits — без anchor revenue. Структурный double-loss.
...
**Параллель с Aramco METABRAIN.** METABRAIN, начатая в 2024 году, **не повторяет ошибку Beyond Limits** в одном ключевом аспекте: METABRAIN — **внутренний продукт Aramco**, не продаётся внешне. Aramco не нужно искать новых customers; не нужно scale across operators. METABRAIN оптимизируется под **внутренние workflow Aramco** и не сталкивается с проблемой generalization. Это структурный аргумент в пользу **insourcing AI** для крупных NOC, против покупки vertical AI vendors. Российский Cognitive Geo (Газпром нефть) — тот же паттерн.

**Что должен сделать студент с BP+Beyond Limits**: не «никогда не покупать vendor AI», а **читать риск концентрации**. Если вендор имеет 1 anchor customer = 80% revenue — это red flag. Если vendor pivot'ит каждые 2 года — red flag. Если promise = «replace geologist» — read flag. Hop за пределы red flags → due diligence.

**Дополнительный угол анализа — почему именно 2018 год.** BP+Beyond Limits объявлены в 2018 году — в самый пик ML-маркетинговой эйфории. ImageNet results 2012, AlphaGo 2016, ChatGPT-1 в 2018 (хотя для большинства публики ChatGPT появился в 2022 году). В этом окне **каждая нефтегазовая компания** делала AI-партнёрство для пресс-релиза. Beyond Limits сильно отличался тем, что у него была JPL-родословная (что давало credibility в индустрии, ценящей deep-space heritage); этого было достаточно для BP $20M инвестиции **без жёсткой технической due diligence**. К 2025 году такие инвестиции не происходят в той же manner — индустрия научилась требовать **technical pilot with measurable outcomes** перед инвестицией. Это **positive evolution**, но cost обучения был $20M+ в одном кейсе, и аналогичные cases у других компаний в той же эпохе (Shell+C3.ai, Chevron+Microsoft AI partnerships) принесли similar lessons.
...
**Второй — «30 лет data analyzed» звучит впечатляюще, но без specific business question metric выводы не actionable.** Repsol получил от Watson **insights в формате «вот интересные паттерны в ваших данных»**, без привязки к конкретным экономическим решениям («стоит ли бурить вот эту локацию?», «надо ли поднять давление в этой колонне?»). Insights без actionability = pilot purgatory.
...
**Параллель Aramco METABRAIN vs IBM Watson 2014.** METABRAIN существенно мощнее (250B параметров vs ~100M-1B у Watson 2014 архитектур) и обучен на специализированных нефтегазовых данных (90 лет Aramco). Но **сам паттерн внедрения** — «foundation model для геологов» — может повторить ту же ловушку. Различие — Aramco **сама** разрабатывает и **сама** интегрирует с внутренними workflow; у Watson + Repsol было **two-party** integration с misaligned incentives (IBM хотел продать платформу; Repsol хотел business outcomes — разные продукт-требования).
...
- **«Insights без actionability»**. AI генерирует interesting patterns, operator не знает, что с ними делать. Без **explicit business workflow integration** — pilot purgatory.
...
**Critical takeaway для LO3.** Frontier exploration — это **canonical example квадранта**, где **alternative tool (Eclipse / INTERSECT / CMG / OpenFOAM)** часто **лучше** AI tool на core mission (regulatory submission, EOR modelling, long-horizon CCS). AI добавляется **рядом**, не **вместо**. Студент, который пишет «AI заменит пластового симулятора через 5 лет» — упускает структурную картину. Через 5 лет (2030) Eclipse будет **по-прежнему стандартом**, а AI — usefully ускорять отдельные циклы и screening.
...
**Senior engineer + classical interpretation — экономическая reality.** Старший геолог с 25+ годами опыта работы с одной геологической провинцией стоит компании $200–500k/год. Foundation model с обучением + интеграцией + maintenance — $5–20 миллионов/год CapEx + ongoing. Math простой: **5–25 senior geologists** за стоимость одной foundation model инсталляции. Foundation model окупается **только** если она **replicates work**, эквивалентную 5–25 геологам — что в моменте 2026 года **не доказано** ни одним публичным benchmark. Это **economic reality** Q3: AI augmentation **дополняет**, не **замещает** senior expertise — потому что senior expertise структурно дешевле foundation model deployment на per-decision basis.

**Halliburton — incumbent perspective.** Halliburton, исторически второй крупнейший сервисный подрядчик после SLB, использует AI преимущественно как **augmentation поверх classical workflow**: ML-assisted seismic interpretation (где human geophysicist делает финальный QC), automated log analysis (но senior petrophysicist делает interpretation), drilling automation на основе learned patterns от successful wells. Halliburton **намеренно избегает** «foundation model» маркетинга — отчасти из-за того, что foundation model гонка ведут SLB Lumi и крупные NOC, и Halliburton **не имеет capital для конкуренции** с этим. Это **strategic positioning**: «human + classical workflow + ML augmentation» vs «foundation model + auto-interpretation». На горизонте 5 лет — оба подхода могут coexist, и индустрия примет тот, который даст лучший cost-effectiveness, не тот, который более «модный».
...
**Последствие:** ML-суррогаты — **screening tools**, не replacement classical simulators. Операторы **сохраняют classical simulators** как ground truth для регуляторных submissions и для дизайн-решений; ML используется для **acceleration** циклов сценарного анализа.
...
**Gartner 2027 prediction: 40% agentic AI projects будут отменены** из-за cost overruns + poor risk controls. Для нефтегаза — это **прямое предупреждение**: deployment LLM agents в production без strong human-in-loop discipline = risk pattern, который повторит the structure failure модели Watson 2014 эпохи.
